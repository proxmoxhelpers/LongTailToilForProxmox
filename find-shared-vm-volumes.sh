...
# Detects configured multi-guest volume references.
############################################################
...
main() {
    ...
    need_commands pvesm lvs awk sed grep sort uniq basename cut readlink mktemp

    FSV_RAW="$(mktemp "${TMPDIR:-/tmp}/lt-shared-raw.XXXXXX")"
    FSV_KEYS="$(mktemp "${TMPDIR:-/tmp}/lt-shared-keys.XXXXXX")"
    : > "$FSV_RAW"; : > "$FSV_KEYS"
    trap 'rm -f "$FSV_RAW" "$FSV_KEYS"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM

    for FSV_CFG in /etc/pve/qemu-server/*.conf /etc/pve/lxc/*.conf; do
        [ -f "$FSV_CFG" ] || continue
        FSV_VMID="$(basename "$FSV_CFG" .conf)"
        case "$FSV_CFG" in */qemu-server/*) FSV_KIND=qemu ;; *) FSV_KIND=lxc ;; esac

        awk '
        /^(ide|sata|scsi|virtio)[0-9]+: / ||
        /^(efidisk|tpmstate)[0-9]+: / ||
        /^unused[0-9]+: / || /^rootfs: / || /^mp[0-9]+: / {
            key=$1; sub(/:$/, "", key)
            line=$0; sub(/^[^:]+:[[:space:]]*/, "", line)
            split(line,a,",")
            if (a[1] ~ /:/) print key "|" a[1]
        }' "$FSV_CFG" |
        while IFS='|' read -r FSV_SLOT FSV_VOLID; do
            [ -n "$FSV_VOLID" ] || continue
            FSV_UUID="$(volid_lv_uuid "$FSV_VOLID" 2>/dev/null || :)"
            if [ -n "$FSV_UUID" ]; then
                FSV_IDKEY="uuid:$FSV_UUID"
            else
                FSV_PATH="$(resolve_volid_path "$FSV_VOLID" 2>/dev/null || :)"
                FSV_CANON="$(readlink -f "$FSV_PATH" 2>/dev/null || :)"
                if [ -n "$FSV_CANON" ]; then
                    FSV_IDKEY="path:$FSV_CANON"
                else
                    FSV_IDKEY="volid:$FSV_VOLID"
                fi
            fi
            printf '%s|%s|%s|%s|%s\n' \
                "$FSV_IDKEY" "$FSV_VOLID" "$FSV_KIND" "$FSV_VMID" "$FSV_SLOT" >> "$FSV_RAW"
        done
    done

    # A physical identity is shared only when it occurs in more than one guest
    # config, not merely in multiple slots of the same guest.
    awk -F'|' '
        {
            guest=$3 ":" $4
            k=$1 SUBSEP guest
        ...
        END { for (id in guests) if (guests[id] > 1) print id }
    ' "$FSV_RAW" | sort > "$FSV_KEYS"

    case "$OUTPUT_FORMAT" in
        json) printf '['; FSV_FIRST=1 ;;
        tsv) printf 'IDENTITY\tVOLUME_IDS\tREFERENCES\n' ;;
        *) printf '%-42s %-54s %s\n' IDENTITY VOLUME_IDS REFERENCES ;;
    esac

    while IFS= read -r FSV_IDKEY; do
        [ -n "$FSV_IDKEY" ] || continue
        FSV_VOLIDS="$(awk -F'|' -v id="$FSV_IDKEY" '$1==id && !seen[$2]++ {printf "%s%s",sep,$2; sep=","}' "$FSV_RAW")"
        FSV_REFS="$(awk -F'|' -v id="$FSV_IDKEY" '$1==id {printf "%s%s:%s:%s",sep,$3,$4,$5; sep=","}' "$FSV_RAW")"
        case "$OUTPUT_FORMAT" in
            json)
                [ "$FSV_FIRST" -eq 1 ] || printf ','
                FSV_FIRST=0
                printf '{"identity":"%s","volume_ids":"%s","references":"%s"}' \
                    "$(json_escape "$FSV_IDKEY")" "$(json_escape "$FSV_VOLIDS")" "$(json_escape "$FSV_REFS")"
                ;;
            tsv) printf '%s\t%s\t%s\n' "$FSV_IDKEY" "$FSV_VOLIDS" "$FSV_REFS" ;;
            *) printf '%-42s %-54s %s\n' "$FSV_IDKEY" "$FSV_VOLIDS" "$FSV_REFS" ;;
        esac
    done < "$FSV_KEYS"
...
usage() {
    cat <<EOF
find-shared-vm-volumes.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
