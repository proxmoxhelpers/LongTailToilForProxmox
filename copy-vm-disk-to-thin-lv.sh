...
# Convenience VM-disk to thin-LV copy.
############################################################
...
setup() {
    ...
    CVT_COMPLETE=0; CVT_SOURCE_ACTIVATED_BY_US=0; ALLOW_RUNNING=0
...
main() {
    ...
    need_commands qm pvesm lvs lvcreate lvremove lvchange blockdev dd cmp awk date cut head find
    require_qemu_vm "$VMID"
    valid_disk_slot "$SLOT" || usage_error "Invalid or out-of-range disk slot: $SLOT"
    valid_lvm_component "$DEST_VG" || usage_error "Invalid destination VG name: $DEST_VG"
    valid_lvm_component "$DEST_NAME" || usage_error "Invalid destination LV name: $DEST_NAME"
    CVT_VOLID="$(disk_volid "$VMID" "$SLOT")" || refuse "Slot $SLOT is not a storage-backed disk."
    CVT_SOURCE="$(resolve_volid_path "$CVT_VOLID")" || refuse "Cannot resolve source volume."
    CVT_SRC_ID="$(lvm_identity "$CVT_SOURCE")" || refuse "Source disk must be backed by an LVM logical volume."
    CVT_SOURCE_VG="$(printf '%s\n' "$CVT_SRC_ID" | cut -d'|' -f1)"
    CVT_SOURCE_LV="$(printf '%s\n' "$CVT_SRC_ID" | cut -d'|' -f2)"
    CVT_SOURCE="/dev/${CVT_SOURCE_VG}/${CVT_SOURCE_LV}"
    CVT_SIZE="$(lvs --noheadings --units b --nosuffix -o lv_size "${CVT_SOURCE_VG}/${CVT_SOURCE_LV}" 2>/dev/null | awk 'NF {printf "%.0f\n", $1; exit}')"
    [ -n "$CVT_SIZE" ] || refuse "Could not determine source LV size."
    CVT_VM_STATUS="$(qm status "$VMID" 2>/dev/null | awk '{print $2}')"
    if [ "$CVT_VM_STATUS" = running ] && [ "${ALLOW_RUNNING:-0}" -ne 1 ]; then
        refuse "Source VM is running. Stop it first, use a state-aware copy helper, or pass --allow-running to accept crash/application-consistency risk."
    fi
    [ "$CVT_VM_STATUS" != running ] || warn "Source VM is running; --allow-running accepts crash/application-consistency risk while bytes are copied."
    if [ -z "$POOL" ]; then
     CVT_POOLS="$(lvs --noheadings -o lv_name --select 'lv_attr=~^t' "$DEST_VG" 2>/dev/null | awk '{$1=$1;if(NF)print}')"
     [ "$(printf '%s\n' "$CVT_POOLS" | awk 'NF{n++}END{print n+0}')" -eq 1 ] || refuse "Specify --pool because destination VG does not have exactly one thin pool."
     POOL="$CVT_POOLS"
    fi
    valid_lvm_component "$POOL" || usage_error "Invalid thin-pool name: $POOL"
    CVT_POOL_ATTR="$(lvs --noheadings -o lv_attr "$DEST_VG/$POOL" 2>/dev/null | awk '{$1=$1;print;exit}')"
    case "$CVT_POOL_ATTR" in t*) : ;; *) refuse "Destination is not an LVM thin pool: $DEST_VG/$POOL" ;; esac
    ! lvs "$DEST_VG/$DEST_NAME" >/dev/null 2>&1 || refuse "Destination LV already exists."
    info "$VMID/$SLOT -> /dev/$DEST_VG/$DEST_NAME ($CVT_SIZE bytes, thin pool $POOL)"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin copy-vm-disk-to-thin-lv; journal_note source "$CVT_VOLID"; journal_note destination "$DEST_VG/$DEST_NAME"
    trap 'cvt_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cvt_ensure_source_device
    run_mutation lvcreate -V "${CVT_SIZE}B" -T "$DEST_VG/$POOL" -n "$DEST_NAME"
    if [ "$DRYRUN" -eq 0 ]; then
        CVT_DEST_UUID="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        [ -n "${CVT_DEST_UUID}" ] || die "Created destination LV but could not capture its UUID."
    fi
    if [ "$DRYRUN" -eq 1 ]; then print_command dd "if=$CVT_SOURCE" "of=/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=sparse,fsync status=progress; cvt_release_source_device; CVT_COMPLETE=1; return 0; fi
    dd if="$CVT_SOURCE" of="/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=sparse,fsync status=progress
    cmp -n "$CVT_SIZE" "$CVT_SOURCE" "/dev/$DEST_VG/$DEST_NAME"
    cvt_release_source_device
    CVT_COMPLETE=1; ok "Thin-LV VM disk copy created and verified."
}
...
usage() {
    cat <<EOF
copy-vm-disk-to-thin-lv.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    VMID=""; SLOT=""; DEST_VG=""; DEST_NAME=""; POOL=""
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --pool) [ "$#" -ge 2 ] || usage_error "--pool requires a value."; POOL="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; elif [ -z "$DEST_VG" ]; then DEST_VG="$1"; elif [ -z "$DEST_NAME" ]; then DEST_NAME="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$VMID" ] && [ -n "$SLOT" ] && [ -n "$DEST_VG" ] || usage_error "VMID, slot and destination VG are required."
    [ -n "$DEST_NAME" ] || DEST_NAME="vm-${VMID}-${SLOT}-thin-copy"
}

# cvt_ensure_source_device
# Call: cvt_ensure_source_device
# Activates an inactive source LV with -K only for this helper's copy window.
cvt_ensure_source_device() {
    [ -b "$CVT_SOURCE" ] && return 0
    info "Source LV is inactive; temporarily activating it for the block copy..."
    if [ "$DRYRUN" -eq 1 ]; then
        dryrun_cmd lvchange -ay -K "$CVT_SOURCE_VG/$CVT_SOURCE_LV"
        CVT_SOURCE_ACTIVATED_BY_US=1
        dryrun_verify "Source LV would be active and readable for the copy"
        return 0
    fi
    run_lvm_filtered lvchange -ay -K "$CVT_SOURCE_VG/$CVT_SOURCE_LV" || die "Could not activate source LV for block copying: $CVT_SOURCE_VG/$CVT_SOURCE_LV"
    CVT_SOURCE_ACTIVATED_BY_US=1
    [ -b "$CVT_SOURCE" ] || die "Source LV activation succeeded but no block device appeared: $CVT_SOURCE"
}

# cvt_release_source_device
# Call: cvt_release_source_device
# Restores an LV activated by this helper to its original inactive state.
cvt_release_source_device() {
    [ "${CVT_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ] || return 0
    info "Restoring source LV to its original inactive state..."
    if [ "$DRYRUN" -eq 1 ]; then dryrun_cmd lvchange -an "$CVT_SOURCE_VG/$CVT_SOURCE_LV"
    else run_lvm_filtered lvchange -an "$CVT_SOURCE_VG/$CVT_SOURCE_LV" || { warn "Could not restore source LV to inactive state: $CVT_SOURCE_VG/$CVT_SOURCE_LV"; return 1; }; fi
    CVT_SOURCE_ACTIVATED_BY_US=0
}

# cvt_cleanup STATUS
# Call: cvt_cleanup STATUS
# Restores activation state and removes only copy-owned temporary state on exit.
cvt_cleanup() {
    cvt_cleanup_status="$1"
    trap - 0 HUP INT TERM

    # Restore source activation state before doing anything that may fail.
    if [ "${CVT_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ]; then
        if run_lvm_filtered lvchange -an "${CVT_SOURCE_VG}/${CVT_SOURCE_LV}" >/dev/null 2>&1; then
            CVT_SOURCE_ACTIVATED_BY_US=0
        else
            warn "Source LV could not be restored to inactive state: ${CVT_SOURCE_VG}/${CVT_SOURCE_LV}"
        fi
    fi

    # Remove an incomplete destination only when it is still the exact LV
    # created by this invocation. Name reuse never authorizes deletion.
    if [ "${CVT_COMPLETE:-0}" -ne 1 ] && [ -n "${CVT_DEST_UUID:-}" ]; then
        cvt_cleanup_uuid="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        if [ -n "${cvt_cleanup_uuid}" ] && [ "${cvt_cleanup_uuid}" = "${CVT_DEST_UUID}" ]; then
            warn "Removing incomplete destination LV created by this invocation."
            lvremove -y "$DEST_VG/$DEST_NAME" >/dev/null 2>&1 || warn "Could not remove incomplete destination LV."
        elif [ -n "${cvt_cleanup_uuid}" ]; then
            warn "Destination name now refers to a different LV UUID; preserving it."
        fi
    fi

    [ "${cvt_cleanup_status}" -eq 0 ] || exit "${cvt_cleanup_status}"
    return 0
}
