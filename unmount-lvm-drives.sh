...
usage() {
    cat <<EOF
unmount-lvm-drives.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# Call: remove_folder_if_empty TARGET
remove_folder_if_empty() {
    rfie_target="$1"; [ -d "$rfie_target" ] || return 0
    if [ -n "$(find "$rfie_target" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null)" ]; then
        printf '  Folder NOT removed because it contains files: %s\n\n  Contents:\n' "$rfie_target"
        find "$rfie_target" -mindepth 1 -maxdepth 1 -printf '    %f\n' 2>/dev/null || :
        return 0
    fi
    if dryrun_cmd rmdir "$rfie_target"; then
        printf '  Removed empty folder: %s\n' "$rfie_target"
        if dryrun_enabled; then dryrun_verify "$rfie_target would be removed only if empty"; fi
    else
