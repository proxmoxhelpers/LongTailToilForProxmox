...
# Call: validate_destinations ARG1 [ARG2] [ARG3] [ARG4] [ARG5]
validate_destinations() {
...
apply_renumber_plan() {
    ...
    while IFS='|' read -r arp_i arp_volid arp_path arp_vg arp_old arp_new arp_storage arp_temp; do
        [ "$arp_old" = "$arp_new" ] || dryrun_cmd lvrename "$arp_vg" "$arp_old" "$arp_temp"
    done < "$PLAN_FILE"

    while IFS='|' read -r arp_i arp_volid arp_path arp_vg arp_old arp_new arp_storage arp_temp; do
        [ "$arp_old" = "$arp_new" ] || dryrun_cmd lvrename "$arp_vg" "$arp_temp" "$arp_new"
    done < "$PLAN_FILE"

    while IFS='|' read -r arp_i arp_volid arp_path arp_vg arp_old arp_new arp_storage arp_temp; do
        [ "$arp_old" = "$arp_new" ] && continue
        arp_old_volid="${arp_storage}:${arp_old}"; arp_new_volid="${arp_storage}:${arp_new}"
        dryrun_cmd sed -i "s#${arp_old_volid}#${arp_new_volid}#g" "$CONFIG"
    done < "$PLAN_FILE"
}
