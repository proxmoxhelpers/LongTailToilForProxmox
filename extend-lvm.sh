...
############################################################
# extend-lvm.sh
# Grow-only raw LV extension primitive.
############################################################
...
main() {
    ...
    need_commands lvs lvextend grep find awk sed date cut head
    EL_ID="$(lvm_identity "$LV")" || refuse "Logical volume does not exist: $LV"
    EL_VG="$(printf '%s\n' "$EL_ID" | cut -d'|' -f1)"; EL_NAME="$(printf '%s\n' "$EL_ID" | cut -d'|' -f2)"
    EL_UUID="$(printf '%s\n' "$EL_ID" | cut -d'|' -f3)"
    EL_BEFORE="$(lvs --noheadings --units b --nosuffix -o lv_size "$EL_VG/$EL_NAME" | awk '{printf "%.0f",$1}')"
    EL_REFS="$(lv_uuid_reference_count "$EL_UUID")"
    if [ "$EL_REFS" -gt 0 ] && [ "$ALLOW_REFERENCED" -eq 0 ]; then
        refuse "LV UUID $EL_UUID is referenced by $EL_REFS guest storage entry/entries; use resize-vm-disk.sh or pass --allow-referenced explicitly."
    fi
    lvextend --test -L "$SIZE" "$EL_VG/$EL_NAME" >/dev/null 2>&1 || refuse "LVM rejected the requested extension in test mode."
    info "LV: $EL_VG/$EL_NAME"
    info "Current bytes: $EL_BEFORE"
    info "Requested size: $SIZE"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin extend-lvm
    journal_note lv "$EL_VG/$EL_NAME"
    journal_note before_bytes "$EL_BEFORE"
    run_mutation lvextend -L "$SIZE" "$EL_VG/$EL_NAME"
    if [ "$DRYRUN" -eq 0 ]; then
     EL_AFTER="$(lvs --noheadings --units b --nosuffix -o lv_size "$EL_VG/$EL_NAME" | awk '{printf "%.0f",$1}')"
     [ "$EL_AFTER" -gt "$EL_BEFORE" ] || verification_failure "LV did not grow."
     journal_note after_bytes "$EL_AFTER"
     ok "Extended LV to $EL_AFTER bytes."
    fi
}
...
usage() {
    cat <<EOF
extend-lvm.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --allow-referenced) ALLOW_REFERENCED=1; shift ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$LV" ]; then LV="$1"; elif [ -z "$SIZE" ]; then SIZE="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$LV" ] && [ -n "$SIZE" ] || usage_error "LV and target/increment size are required."
