...
usage() {
    cat <<EOF
mount-lvm-drives.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            *) pa_count=$((pa_count + 1)); case "$pa_count" in 1) DEVICE="$1" ;; 2) MOUNT_ROOT="$1" ;; *) usage_error "Multiple mount directories specified." ;; esac ;;
        esac
        shift
    done
