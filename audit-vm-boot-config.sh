...
# Read-only boot order and firmware consistency audit.
############################################################
...
main() {
    ...
    need_commands qm awk sed grep
    require_qemu_vm "$VMID"
    AB_CFG="$(qm config "$VMID")"; AB_ISSUES=0
    AB_BIOS="$(printf '%s\n' "$AB_CFG" | sed -n 's/^bios:[[:space:]]*//p' | head -n1)"
    AB_BOOT="$(printf '%s\n' "$AB_CFG" | sed -n 's/^boot:[[:space:]]*//p' | head -n1)"
    AB_EFI="$(printf '%s\n' "$AB_CFG" | grep -E '^efidisk[0-9]+:' || :)"
    AB_TPM="$(printf '%s\n' "$AB_CFG" | grep -E '^tpmstate[0-9]+:' || :)"
    printf 'VMID: %s\nBIOS: %s\nBoot: %s\n' "$VMID" "${AB_BIOS:-seabios(default)}" "${AB_BOOT:--}"
    if [ "${AB_BIOS:-seabios}" = ovmf ] && [ -z "$AB_EFI" ]; then warn "OVMF is configured without an EFI variables disk."; AB_ISSUES=$((AB_ISSUES + 1)); fi
    if [ "${AB_BIOS:-seabios}" != ovmf ] && [ -n "$AB_EFI" ]; then warn "EFI disk exists while BIOS is not OVMF."; AB_ISSUES=$((AB_ISSUES + 1)); fi
    if [ -n "$AB_BOOT" ]; then
     AB_ORDER="$(printf '%s\n' "$AB_BOOT" | sed -n 's/.*order=\([^,]*\).*/\1/p')"
     AB_OLD_IFS=$IFS; IFS=';'
     for AB_DEV in $AB_ORDER; do
       IFS=$AB_OLD_IFS
       [ -n "$AB_DEV" ] || continue
       if ! printf '%s\n' "$AB_CFG" | grep -qE "^${AB_DEV}:"; then warn "Boot order references missing device: $AB_DEV"; AB_ISSUES=$((AB_ISSUES + 1)); fi
       IFS=';'
     done
     IFS=$AB_OLD_IFS
    fi
    [ "$AB_ISSUES" -eq 0 ] || exit 4
    ok "Boot/firmware configuration is internally consistent."
}
...
usage() {
    cat <<EOF
audit-vm-boot-config.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
