...
# Higher-level stopped-VM storage layout migration.
############################################################
...
# mvsl_cleanup STATUS
# On abnormal real execution, reverse any selected slot that currently points
# at the requested destination back to the source storage recorded in the
# immutable preflight plan. Rollback is best-effort and is verified per slot.

main() {
    [ "$APP_ELEVATED" = "true" ] || self_elevate "$@"
    need_commands qm pvesm awk sed grep mktemp date

    require_qemu_vm "$VMID"
    [ "$(guest_status "$VMID" qemu)" = stopped ] ||
        refuse "VM must be stopped for multi-disk layout migration."
    MVSL_CFG="/etc/pve/qemu-server/${VMID}.conf"
    qemu_config_has_snapshots "$MVSL_CFG" &&
        refuse "VM configuration contains snapshots; multi-disk migration refuses snapshot-bearing configs."

    pvesm status 2>/dev/null | awk -v s="$DEST" '$1==s {found=1} END{exit !found}' ||
        refuse "Destination storage is unavailable: $DEST"

    MVSL_PLAN="$(mktemp "${TMPDIR:-/tmp}/lt-migrate-plan.XXXXXX")"
    MVSL_DONE="$(mktemp "${TMPDIR:-/tmp}/lt-migrate-done.XXXXXX")"
    : > "$MVSL_PLAN"
    : > "$MVSL_DONE"
    MVSL_COMPLETE=0
    MVSL_MUTATION_STARTED=0
    trap 'mvsl_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM

    for MVSL_SLOT in $(qm config "$VMID" | awk -F: '/^(ide|sata|scsi|virtio)[0-9]+:/ {print $1}'); do
        [ -z "$SLOT" ] || [ "$MVSL_SLOT" = "$SLOT" ] || continue
        valid_disk_slot "$MVSL_SLOT" || refuse "Unexpected disk slot in VM config: $MVSL_SLOT"
        MVSL_VOLID="$(disk_volid "$VMID" "$MVSL_SLOT" || :)"
        [ -n "$MVSL_VOLID" ] || continue
        MVSL_SRC="${MVSL_VOLID%%:*}"
        [ "$MVSL_SRC" = "$DEST" ] && continue
        printf '%s|%s|%s\n' "$MVSL_SLOT" "$MVSL_SRC" "$MVSL_VOLID" >> "$MVSL_PLAN"
    done

    MVSL_COUNT="$(awk 'END{print NR+0}' "$MVSL_PLAN")"
    [ "$MVSL_COUNT" -gt 0 ] || {
        MVSL_COMPLETE=1
        ok "All selected disks are already on $DEST."
        return 0
    }

    info "Planned disk moves:"
    cat "$MVSL_PLAN"
    [ "$PREFLIGHT" -eq 0 ] || {
        MVSL_COMPLETE=1
        return 0
    ...
    journal_begin migrate-vm-storage-layout
    journal_note vmid "$VMID"
    journal_note destination "$DEST"
    journal_note disks "$MVSL_COUNT"

    if [ "$DRYRUN" -eq 1 ]; then
        while IFS='|' read -r MVSL_SLOT MVSL_SRC MVSL_VOLID; do
            print_command qm move_disk "$VMID" "$MVSL_SLOT" "$DEST" --delete 1
        done < "$MVSL_PLAN"
        MVSL_COMPLETE=1
        return 0
    fi

    MVSL_MUTATION_STARTED=1
    while IFS='|' read -r MVSL_SLOT MVSL_SRC MVSL_VOLID; do
        info "Moving $MVSL_SLOT: $MVSL_SRC -> $DEST"
        qm move_disk "$VMID" "$MVSL_SLOT" "$DEST" --delete 1 ||
            verification_failure "Move failed for $MVSL_SLOT; rollback will be attempted."

        # Record immediately after qm returns success, before postcondition
        # verification, so later failures are covered by the exit trap.
        printf '%s|%s\n' "$MVSL_SLOT" "$MVSL_SRC" >> "$MVSL_DONE"
        journal_note "moved_$MVSL_SLOT" "$MVSL_SRC->$DEST"

        MVSL_NEW="$(disk_volid "$VMID" "$MVSL_SLOT" || :)"
        [ -n "$MVSL_NEW" ] &&
        [ "${MVSL_NEW%%:*}" = "$DEST" ] ||
            verification_failure "Post-move storage verification failed for $MVSL_SLOT; rollback will be attempted."
    done < "$MVSL_PLAN"

    MVSL_COMPLETE=1
    ok "Moved $MVSL_COUNT disk(s) to $DEST and verified each resulting storage prefix."
}
...
usage() {
    cat <<EOF
migrate-vm-storage-layout.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# mvsl_cleanup
# Rolls back completed storage moves in reverse order after an abnormal exit.
# Call: mvsl_cleanup STATUS
mvsl_cleanup() {
    mvsl_cleanup_status="$1"
    trap - 0 HUP INT TERM

    if [ "${MVSL_COMPLETE:-0}" -ne 1 ] &&
       [ "${MVSL_MUTATION_STARTED:-0}" -eq 1 ] &&
       [ "${DRYRUN:-0}" -eq 0 ] &&
       [ -f "${MVSL_PLAN:-}" ]; then
        warn "Migration did not complete; attempting reverse-order rollback."
        awk '{ line[NR]=$0 } END { for (i=NR; i>=1; i--) print line[i] }' "$MVSL_PLAN" |
        while IFS='|' read -r mvsl_r_slot mvsl_r_src mvsl_r_old_volid; do
            [ -n "$mvsl_r_slot" ] || continue
            mvsl_r_now="$(disk_volid "$VMID" "$mvsl_r_slot" 2>/dev/null || :)"
            [ -n "$mvsl_r_now" ] || {
                warn "Rollback cannot inspect $mvsl_r_slot; manual inspection required."
                continue
            }
            [ "${mvsl_r_now%%:*}" = "$DEST" ] || continue
            if qm move_disk "$VMID" "$mvsl_r_slot" "$mvsl_r_src" --delete 1 >/dev/null 2>&1; then
                mvsl_r_after="$(disk_volid "$VMID" "$mvsl_r_slot" 2>/dev/null || :)"
                [ "${mvsl_r_after%%:*}" = "$mvsl_r_src" ] ||
                    warn "Rollback verification failed for $mvsl_r_slot -> $mvsl_r_src."
            else
                warn "Rollback failed for $mvsl_r_slot -> $mvsl_r_src."
            fi
        done
    fi

    rm -f "${MVSL_PLAN:-}" "${MVSL_DONE:-}"
    [ "$mvsl_cleanup_status" -eq 0 ] || exit "$mvsl_cleanup_status"
}
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            --quiet) QUIET=1; shift ;;
            --slot)
                [ "$#" -ge 2 ] || usage_error "--slot requires a value."
                SLOT="$2"; shift 2
                ;;
    ...
    [ -n "$VMID" ] && [ -n "$DEST" ] || usage_error "VMID and destination storage are required."
    case "$VMID" in *[!0-9]*|"") usage_error "VMID must be numeric." ;; esac
    [ -z "$SLOT" ] || valid_disk_slot "$SLOT" ||
        usage_error "--slot must be a valid QEMU disk slot."
    single_line_value "$DEST" || usage_error "Destination storage must be a single-line value."
    case "$DEST" in *[!A-Za-z0-9._-]*|"") usage_error "Invalid Proxmox storage ID: $DEST" ;; esac
}
