...
# Read-only partition bounds and content-signature verifier.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs lvchange partx blkid blockdev awk sed grep mktemp cut
    VDC_ACTIVATIONS="$(mktemp "${TMPDIR:-/var/tmp}/lt-content-activations.XXXXXX")"; : > "$VDC_ACTIVATIONS"
    trap 'vdc_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    VDC_RAW="$(vdc_resolve_selector "$SELECTOR")" || refuse "Cannot resolve disk selector: $SELECTOR"
    if prepare_readable_path "$VDC_RAW" "$VDC_ACTIVATIONS"; then VDC_PATH="$READABLE_PATH"; else
        VDC_RC=$?; [ "$VDC_RC" -ne 2 ] || refuse "Selected LV is inactive; dry-run/preflight will not change activation state."
        refuse "Selected device is not readable: $VDC_RAW"
    fi
    VDC_SECTOR="$(blockdev --getss "$VDC_PATH")"; VDC_BYTES="$(blockdev --getsize64 "$VDC_PATH")"
    printf 'Device: %s\nBytes: %s\nSector size: %s\n' "$VDC_PATH" "$VDC_BYTES" "$VDC_SECTOR"
    VDC_COUNT=0; VDC_ISSUES=0; VDC_PREV_END=0
    while read -r VDC_NR VDC_START VDC_SECTORS VDC_TYPE; do
        [ -n "${VDC_NR:-}" ] || continue
        VDC_COUNT=$((VDC_COUNT + 1))
        VDC_END=$((VDC_START + VDC_SECTORS))
        if [ "$VDC_START" -lt "$VDC_PREV_END" ]; then warn "Partition $VDC_NR overlaps a preceding partition."; VDC_ISSUES=$((VDC_ISSUES + 1)); fi
        VDC_PREV_END="$VDC_END"
        if [ "$VDC_END" -gt $((VDC_BYTES / VDC_SECTOR)) ]; then warn "Partition $VDC_NR extends beyond the block device."; VDC_ISSUES=$((VDC_ISSUES + 1)); fi
        VDC_OFFSET=$((VDC_START * VDC_SECTOR)); VDC_LEN=$((VDC_SECTORS * VDC_SECTOR))
        VDC_FS="$(blkid -p -o value -s TYPE -O "$VDC_OFFSET" -S "$VDC_LEN" "$VDC_PATH" 2>/dev/null || :)"
        printf 'part%-3s start=%-12s sectors=%-12s table=%-24s content=%s\n' "$VDC_NR" "$VDC_START" "$VDC_SECTORS" "${VDC_TYPE:--}" "${VDC_FS:--}"
    done <<EOF
    $(partx --show --noheadings -o NR,START,SECTORS,TYPE "$VDC_PATH" 2>/dev/null || :)
EOF
    if [ "$VDC_COUNT" -eq 0 ]; then
        VDC_WHOLE="$(blkid -p -o value -s TYPE "$VDC_PATH" 2>/dev/null || :)"
        printf 'No partitions detected; whole-device content: %s\n' "${VDC_WHOLE:--}"
    fi
    [ "$VDC_ISSUES" -eq 0 ] || exit 4
    ok "Disk content bounds check passed."
}
...
usage() {
    cat <<EOF
verify-vm-disk-content.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
      ...
      *) [ -z "$SELECTOR" ] || usage_error "One disk selector is accepted."; SELECTOR="$1"; shift ;;
     esac
    done
    [ -n "$SELECTOR" ] || usage_error "A disk selector is required."
}

# vdc_resolve_selector SELECTOR
# Call: vdc_resolve_selector SELECTOR
# Resolves a VM disk selector to one exact configured slot and backing volume.
vdc_resolve_selector() {
    vdc_resolve_selector_selector="$1"
    case "$vdc_resolve_selector_selector" in
      /*) printf '%s\n' "$vdc_resolve_selector_selector" ;;
      *:*) vdc_resolve_selector_vmid="${vdc_resolve_selector_selector%%:*}"; vdc_resolve_selector_slot="${vdc_resolve_selector_selector#*:}"
           require_qemu_vm "$vdc_resolve_selector_vmid"
           valid_disk_slot "$vdc_resolve_selector_slot" || usage_error "Invalid or out-of-range disk slot: $vdc_resolve_selector_slot"
           vdc_resolve_selector_volid="$(disk_volid "$vdc_resolve_selector_vmid" "$vdc_resolve_selector_slot")" || return 1
           resolve_volid_path "$vdc_resolve_selector_volid" ;;
      *) return 1 ;;
    esac
}

# vdc_cleanup STATUS
# Restores temporary inactive-LV activation used for content inspection.
# Call: vdc_cleanup STATUS
vdc_cleanup() {
    vdc_cleanup_status="$1"
    trap - 0 HUP INT TERM
    restore_tracked_activations "${VDC_ACTIVATIONS:-}"
    rm -f "${VDC_ACTIVATIONS:-}" 2>/dev/null || :
    [ "$vdc_cleanup_status" -eq 0 ] || exit "$vdc_cleanup_status"
    return 0
}
