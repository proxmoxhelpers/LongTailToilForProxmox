...
############################################################
# show-thin-snapshot-tree.sh
# Shows LVM-thin origin/child lineage.
############################################################
...
main() {
    ...
    need_commands lvs awk sed sort
    STS_INPUT="$(lvs --noheadings --separator '|' -o vg_name,lv_name,lv_uuid,pool_lv,origin,lv_attr 2>/dev/null | sed 's/^[[:space:]]*//;s/[[:space:]]*$//;s/[[:space:]]*|[[:space:]]*/|/g')"
    if [ -n "$VG" ]; then STS_INPUT="$(printf '%s\n' "$STS_INPUT" | awk -F'|' -v vg="$VG" '$1==vg')"; fi
    case "$OUTPUT_FORMAT" in
     json) printf '['; STS_FIRST=1 ;;
     tsv) printf 'VG\tLV\tPOOL\tORIGIN\tUUID\tATTR\n' ;;
     *) printf '%-18s %-34s %-20s %-34s %-16s %s\n' VG LV POOL ORIGIN UUID ATTR ;;
    esac
    printf '%s\n' "$STS_INPUT" | while IFS='|' read -r STS_VG STS_LV STS_UUID STS_POOL STS_ORIGIN STS_ATTR; do
     [ -n "$STS_LV" ] || continue
     case "$OUTPUT_FORMAT" in
      json)
       [ "$STS_FIRST" -eq 1 ] || printf ','
       STS_FIRST=0
       printf '{"vg":"%s","lv":"%s","pool":"%s","origin":"%s","uuid":"%s","attr":"%s"}' "$(json_escape "$STS_VG")" "$(json_escape "$STS_LV")" "$(json_escape "$STS_POOL")" "$(json_escape "$STS_ORIGIN")" "$(json_escape "$STS_UUID")" "$(json_escape "$STS_ATTR")"
       ;;
      tsv) printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$STS_VG" "$STS_LV" "${STS_POOL:--}" "${STS_ORIGIN:--}" "$STS_UUID" "$STS_ATTR" ;;
      *) printf '%-18s %-34s %-20s %-34s %-16s %s\n' "$STS_VG" "$STS_LV" "${STS_POOL:--}" "${STS_ORIGIN:--}" "$STS_UUID" "$STS_ATTR" ;;
     esac
    done
...
usage() {
    cat <<EOF
show-thin-snapshot-tree.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  show-thin-snapshot-tree.sh [VG|--vg VG] [--json|--tsv]

DESCRIPTION
  Read-only inventory of thin snapshot ancestry. Each LV is shown with its
  pool and direct origin so base/template lineage remains visible.
EOF
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --vg) [ "$#" -ge 2 ] || usage_error "--vg requires a value."; VG="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) [ -z "$VG" ] || usage_error "Unexpected argument: $1"; VG="$1"; shift ;;
     esac
    done
    [ -z "$VG" ] || valid_lvm_component "$VG" || usage_error "Invalid volume-group name: $VG"
}
