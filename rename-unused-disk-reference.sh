...
# UUID-safe config-only unusedN reference rename.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs cp awk grep mktemp date readlink cut head find
    require_qemu_vm "$VMID"; [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    RUR_CFG="/etc/pve/qemu-server/${VMID}.conf"
    ! qemu_config_has_snapshots "$RUR_CFG" || refuse "VM snapshots are present; direct config editing is refused."
    valid_unused_slot "$SLOT" || usage_error "Slot must be unused0..unused255."
    single_line_value "$NEW_VOLID" || usage_error "New volume ID must be a single line."
    RUR_OLD_VALUE="$(sed -n "s/^${SLOT}:[[:space:]]*//p" "$RUR_CFG" | head -n1)"; [ -n "$RUR_OLD_VALUE" ] || refuse "$SLOT is not configured."
    RUR_OLD_VOLID="${RUR_OLD_VALUE%%,*}"
    RUR_OLD_PATH="$(resolve_volid_path "$RUR_OLD_VOLID" || :)"; RUR_NEW_PATH="$(resolve_volid_path "$NEW_VOLID" || :)"
    [ -n "$RUR_OLD_PATH" ] && [ -n "$RUR_NEW_PATH" ] || refuse "Both old and new volume IDs must resolve."
    RUR_OLD_ID="$(lvm_identity "$RUR_OLD_PATH" || :)"; RUR_NEW_ID="$(lvm_identity "$RUR_NEW_PATH" || :)"
    if [ -n "$RUR_OLD_ID" ] && [ -n "$RUR_NEW_ID" ]; then
     RUR_OLD_UUID="$(printf '%s\n' "$RUR_OLD_ID" | cut -d'|' -f3)"; RUR_NEW_UUID="$(printf '%s\n' "$RUR_NEW_ID" | cut -d'|' -f3)"
     [ "$RUR_OLD_UUID" = "$RUR_NEW_UUID" ] || refuse "New volume ID resolves to a different LV UUID."
    else
     [ "$(readlink -f "$RUR_OLD_PATH")" = "$(readlink -f "$RUR_NEW_PATH")" ] || refuse "Old and new volume IDs do not resolve to the same backing path."
    fi
    RUR_NEW_VALUE="$NEW_VOLID${RUR_OLD_VALUE#"$RUR_OLD_VOLID"}"
    info "$SLOT: $RUR_OLD_VALUE -> $RUR_NEW_VALUE"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin rename-unused-disk-reference; journal_note vmid "$VMID"; journal_note slot "$SLOT"; journal_note old "$RUR_OLD_VOLID"; journal_note new "$NEW_VOLID"
    RUR_BACKUP="/root/${VMID}.conf.before-unused-reference-rename.$(date +%Y%m%d-%H%M%S)"
    RUR_TMP="$(mktemp "${TMPDIR:-/tmp}/lt-unused-ref.XXXXXX")"; trap 'rm -f "$RUR_TMP"' 0 HUP INT TERM
    awk -v slot="$SLOT" -v value="$RUR_NEW_VALUE" 'BEGIN{done=0} $0 ~ ("^"slot":") {print slot": "value; done=1; next} {print} END{if(!done)exit 5}' "$RUR_CFG" > "$RUR_TMP" || die "Could not prepare config rewrite."
    if [ "$DRYRUN" -eq 1 ]; then print_command cp "$RUR_CFG" "$RUR_BACKUP"; printf '[DRYRUN] rewrite %s only; no storage deletion API\n' "$SLOT"; return 0; fi
    cp "$RUR_CFG" "$RUR_BACKUP"
    RUR_TX_ACTIVE=1
    trap 'rur_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cat "$RUR_TMP" > "$RUR_CFG"
    if ! qm config "$VMID" >/dev/null 2>&1 || ! grep -qF "$SLOT: $RUR_NEW_VALUE" "$RUR_CFG"; then
        cat "$RUR_BACKUP" > "$RUR_CFG"
        RUR_TX_ACTIVE=0
        trap - 0 HUP INT TERM
        verification_failure "Config validation failed; backup restored."
    fi
    RUR_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    journal_note backup "$RUR_BACKUP"; ok "Unused reference renamed without freeing storage."
}
...
usage() {
    cat <<EOF
rename-unused-disk-reference.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
      ...
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; elif [ -z "$NEW_VOLID" ]; then NEW_VOLID="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$VMID" ] && [ -n "$SLOT" ] && [ -n "$NEW_VOLID" ] || usage_error "VMID, unusedN and new volume ID are required."
    valid_unused_slot "$SLOT" || usage_error "Slot must be unused0..unused255."
}
...
# rur_restore_on_exit
# Restores the original config after an interrupted unused-reference rename.
rur_restore_on_exit() {
    rur_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${RUR_TX_ACTIVE:-0}" -eq 1 ] && [ -n "${RUR_BACKUP:-}" ] && [ -f "$RUR_BACKUP" ]; then
        cat "$RUR_BACKUP" > "$RUR_CFG" || :
        warn "Restored VM config after interrupted unused-reference rename."
    fi
    [ "$rur_restore_status" -eq 0 ] || exit "$rur_restore_status"
    return 0
}
