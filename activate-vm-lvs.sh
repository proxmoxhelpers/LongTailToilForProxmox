...
############################################################
# activate-vm-lvs.sh
# Activates referenced guest LVs when explicitly requested.
############################################################
...
main() {
    ...
    need_commands qm pct pvesm lvs lvchange awk sed grep date cut head find sort mktemp
    AVL_KIND="$(guest_kind "$VMID")" || refuse "Guest $VMID does not exist."
    AVL_CFG="$(guest_config_path "$VMID")"
    AVL_PLAN="$(mktemp "${TMPDIR:-/var/tmp}/lt-activate-plan.XXXXXX")"
    AVL_DONE="$(mktemp "${TMPDIR:-/var/tmp}/lt-activate-done.XXXXXX")"
    : > "$AVL_PLAN"; : > "$AVL_DONE"
    trap 'avl_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    ...
    # Build the complete physical-LV plan before the first activation. Storage
    # aliases are deduplicated by LV UUID.
    while IFS= read -r AVL_VOLID; do
        [ -n "$AVL_VOLID" ] || continue
        AVL_PATH="$(resolve_volid_path "$AVL_VOLID" || :)"
        if [ -z "$AVL_PATH" ]; then warn "Skipping unresolved storage reference: $AVL_VOLID"; continue; fi
        AVL_ID="$(lvm_identity "$AVL_PATH" || :)"
        if [ -z "$AVL_ID" ]; then continue; fi
        AVL_VG="$(printf '%s\n' "$AVL_ID" | cut -d'|' -f1)"
        AVL_LV="$(printf '%s\n' "$AVL_ID" | cut -d'|' -f2)"
        AVL_UUID="$(printf '%s\n' "$AVL_ID" | cut -d'|' -f3)"
        AVL_ATTR="$(printf '%s\n' "$AVL_ID" | cut -d'|' -f7)"
        grep -qF "|$AVL_UUID|" "$AVL_PLAN" 2>/dev/null && continue
        case "$AVL_ATTR" in
            ????a*) info "Already active: $AVL_VG/$AVL_LV"; printf '%s|%s|%s|active\n' "$AVL_VG" "$AVL_LV" "$AVL_UUID" >> "$AVL_PLAN" ;;
            *) printf '%s|%s|%s|inactive\n' "$AVL_VG" "$AVL_LV" "$AVL_UUID" >> "$AVL_PLAN" ;;
        esac
    done <<EOF
$(config_volids "$AVL_CFG" | sort -u)
EOF

    AVL_COUNT="$(awk -F'|' '$4=="inactive"{n++} END{print n+0}' "$AVL_PLAN")"
    while IFS='|' read -r AVL_VG AVL_LV AVL_UUID AVL_STATE; do
        [ "$AVL_STATE" = "inactive" ] || continue
        info "Activate: $AVL_VG/$AVL_LV"
        if [ "$PREFLIGHT" -eq 1 ]; then continue; fi
        if [ "$DRYRUN" -eq 1 ]; then
            print_command lvchange -ay -K "$AVL_VG/$AVL_LV"
            continue
        fi
        lvchange -ay -K "$AVL_VG/$AVL_LV"
        AVL_NOW="$(lvs --noheadings -o lv_attr "$AVL_VG/$AVL_LV" 2>/dev/null | awk '{$1=$1;print;exit}')"
        case "$AVL_NOW" in ????a*) : ;; *) verification_failure "LV did not become active: $AVL_VG/$AVL_LV" ;; esac
        printf '%s|%s|%s\n' "$AVL_VG" "$AVL_LV" "$AVL_UUID" >> "$AVL_DONE"
    done < "$AVL_PLAN"

    AVL_COMPLETE=1
    ok "Activation plan contains $AVL_COUNT inactive referenced LV(s)."
}
...
usage() {
    cat <<EOF
activate-vm-lvs.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# avl_cleanup STATUS
# Rolls back only activations completed by this invocation after an abnormal exit.
# Call: avl_cleanup STATUS
avl_cleanup() {
    avl_cleanup_status="$1"
    trap - 0 HUP INT TERM
    if [ "${AVL_COMPLETE:-0}" -ne 1 ] && [ -s "${AVL_DONE:-}" ]; then
        warn "Activation did not complete; restoring LVs activated by this invocation."
        awk '{line[NR]=$0} END{for(i=NR;i>=1;i--)print line[i]}' "$AVL_DONE" |
        while IFS='|' read -r avl_cleanup_vg avl_cleanup_lv avl_cleanup_uuid; do
            avl_cleanup_now="$(lvs --noheadings --separator '|' -o lv_uuid,lv_attr "$avl_cleanup_vg/$avl_cleanup_lv" 2>/dev/null |
                head -n1 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//;s/[[:space:]]*|[[:space:]]*/|/g')"
            avl_cleanup_now_uuid="${avl_cleanup_now%%|*}"
            [ "$avl_cleanup_now_uuid" = "$avl_cleanup_uuid" ] || continue
            lvchange -an "$avl_cleanup_vg/$avl_cleanup_lv" >/dev/null 2>&1 ||
                warn "Could not roll back activation: $avl_cleanup_vg/$avl_cleanup_lv"
        done
    fi
    rm -f "${AVL_PLAN:-}" "${AVL_DONE:-}" 2>/dev/null || :
    [ "$avl_cleanup_status" -eq 0 ] || exit "$avl_cleanup_status"
    return 0
}
