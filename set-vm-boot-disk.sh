...
usage() {
    cat <<EOF
set-vm-boot-disk.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# Call: build_boot_order [ARG...]
build_boot_order() {
    bbo_boot="$(qm config "$VMID" | sed -n 's/^boot:[[:space:]]*//p' | head -n1)"
    bbo_order="$(printf '%s\n' "$bbo_boot" | sed -n 's/.*order=\([^,]*\).*/\1/p')"
    NEW="$SLOT"
    bbo_old_ifs="$IFS"; IFS=';'; set -- $bbo_order; IFS="$bbo_old_ifs"
    for bbo_item in "$@"; do
        if [ -n "$bbo_item" ] && [ "$bbo_item" != "$SLOT" ]; then NEW="${NEW};${bbo_item}"; fi
    done
    return 0
}
