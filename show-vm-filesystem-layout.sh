...
# Per-VM read-only partition/filesystem summary.
############################################################
...
main() {
    ...
    if [ -n "$SLOT" ]; then valid_disk_slot "$SLOT" || usage_error "Invalid or out-of-range disk slot: $SLOT"; fi
    SVFL_ACTIVATIONS="$(mktemp "${TMPDIR:-/var/tmp}/lt-layout-activations.XXXXXX")"; : > "$SVFL_ACTIVATIONS"
    trap 'svfl_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    printf '%-10s %-8s %-12s %-12s %-24s %s\n' SLOT PART START SIZE TABLE_HINT CONTENT
    for SVFL_SLOT in $(qm config "$VMID" | awk -F: '/^(ide|sata|scsi|virtio)[0-9]+:/ {print $1}'); do
        [ -z "$SLOT" ] || [ "$SVFL_SLOT" = "$SLOT" ] || continue
        SVFL_VOLID="$(disk_volid "$VMID" "$SVFL_SLOT" || :)"; [ -n "$SVFL_VOLID" ] || continue
        SVFL_RAW="$(resolve_volid_path "$SVFL_VOLID" || :)"; [ -n "$SVFL_RAW" ] || continue
        if prepare_readable_path "$SVFL_RAW" "$SVFL_ACTIVATIONS"; then SVFL_PATH="$READABLE_PATH"; else
            SVFL_RC=$?; [ "$SVFL_RC" -ne 2 ] || refuse "$SVFL_SLOT is inactive; dry-run/preflight will not activate it."
            refuse "Cannot read $SVFL_SLOT backing device: $SVFL_RAW"
        fi
        SVFL_SS="$(blockdev --getss "$SVFL_PATH")"
        SVFL_FOUND=0
        while read -r SVFL_NR SVFL_START SVFL_SECTORS SVFL_TYPE; do
            [ -n "${SVFL_NR:-}" ] || continue; SVFL_FOUND=1
            SVFL_OFF=$((SVFL_START * SVFL_SS)); SVFL_LEN=$((SVFL_SECTORS * SVFL_SS))
            SVFL_CONTENT="$(blkid -p -o value -s TYPE -O "$SVFL_OFF" -S "$SVFL_LEN" "$SVFL_PATH" 2>/dev/null || :)"
            printf '%-10s part%-4s %-12s %-12s %-24s %s\n' "$SVFL_SLOT" "$SVFL_NR" "$SVFL_START" "$SVFL_SECTORS" "${SVFL_TYPE:--}" "${SVFL_CONTENT:--}"
        done <<EOF
$(partx --show --noheadings -o NR,START,SECTORS,TYPE "$SVFL_PATH" 2>/dev/null || :)
EOF
        if [ "$SVFL_FOUND" -eq 0 ]; then
            SVFL_CONTENT="$(blkid -p -o value -s TYPE "$SVFL_PATH" 2>/dev/null || :)"
            printf '%-10s %-8s %-12s %-12s %-24s %s\n' "$SVFL_SLOT" whole 0 "$(blockdev --getsz "$SVFL_PATH")" - "${SVFL_CONTENT:--}"
        fi
    done
}
...
usage() {
    cat <<EOF
show-vm-filesystem-layout.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# svfl_cleanup STATUS
# Restores temporary inactive-LV activations used for layout inspection.
# Call: svfl_cleanup STATUS
svfl_cleanup() {
    svfl_cleanup_status="$1"
    trap - 0 HUP INT TERM
    restore_tracked_activations "${SVFL_ACTIVATIONS:-}"
    rm -f "${SVFL_ACTIVATIONS:-}" 2>/dev/null || :
    [ "$svfl_cleanup_status" -eq 0 ] || exit "$svfl_cleanup_status"
    return 0
}
