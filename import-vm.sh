...
############################################################
# import-vm.sh
# Restores and verifies a single-file LongTail VM archive.
############################################################
...
main() {
    ...
    need_commands tar grep awk sha256sum sort uniq sed
    [ -f "$ARCHIVE" ] || refuse "Archive not found: $ARCHIVE"
    ivm_validate_archive "$ARCHIVE"
    set -- "$ARCHIVE"
    [ -z "$TARGET_VMID" ] || set -- "$@" --vmid "$TARGET_VMID"
    [ -z "$STORAGE" ] || set -- "$@" --storage "$STORAGE"
    [ "$START_GUEST" -eq 0 ] || set -- "$@" --start
    [ "$VERIFY_CONTENT" -eq 1 ] || set -- "$@" --no-content-verify
    [ "$QUIET" -eq 0 ] || set -- "$@" --quiet
    if [ "$PREFLIGHT" -eq 1 ]; then set -- "$@" --preflight
    elif [ "$DRYRUN" -eq 1 ]; then set -- "$@" --dryrun
    fi
    tar -xOf "$ARCHIVE" ltvm/restore.sh | /bin/sh -s -- "$@"
}
...
usage() {
    cat <<EOF
import-vm.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# ivm_validate_archive ARCHIVE
# Call: ivm_validate_archive ARCHIVE
...
ivm_validate_archive() {
    ivm_validate_archive_file="$1"
    IVM_LISTING="$(tar -tf "$ivm_validate_archive_file" 2>/dev/null)" || refuse "Archive is not a readable tar file."
    printf '%s\n' "$IVM_LISTING" | while IFS= read -r ivm_validate_archive_member; do
        case "${ivm_validate_archive_member}" in /*|..|../*|*/..|*/../*) exit 8 ;; esac
        case "${ivm_validate_archive_member}" in ltvm|ltvm/*) : ;; *) exit 8 ;; esac
        printf '%s\n' "${ivm_validate_archive_member}" | grep -Eq '^[A-Za-z0-9._/-]+$' || exit 8
    done || refuse "Archive contains an unsafe path."
    [ -z "$(printf '%s\n' "$IVM_LISTING" | sort | uniq -d)" ] || refuse "Archive contains duplicate member paths."
    IVM_TYPES="$(tar -tvf "$ivm_validate_archive_file" 2>/dev/null)" || refuse "Archive member metadata cannot be read."
    printf '%s\n' "$IVM_TYPES" | awk 'NF && substr($1,1,1)!="-" && substr($1,1,1)!="d"{bad=1} END{exit bad}' || refuse "Archive contains a symlink, hardlink, device, FIFO, or other non-regular member."
    for ivm_validate_archive_required in ltvm/manifest.tsv ltvm/config.conf ltvm/storage-map.tsv ltvm/checksums.sha256 ltvm/restore.sh; do
        [ "$(printf '%s\n' "$IVM_LISTING" | grep -Fxc "$ivm_validate_archive_required" || :)" -eq 1 ] || refuse "Archive must contain exactly one ${ivm_validate_archive_required}."
    done
    IVM_CHECKSUMS="$(tar -xOf "$ivm_validate_archive_file" ltvm/checksums.sha256 2>/dev/null)" || refuse "Archive checksum list cannot be read."
    [ -n "$IVM_CHECKSUMS" ] || refuse "Archive checksum list is empty."
    printf '%s\n' "$IVM_CHECKSUMS" | grep -Eq '^[[:xdigit:]]{64}  ltvm/[A-Za-z0-9._/-]+$' || refuse "Archive checksum list has invalid syntax."
    [ "$(printf '%s\n' "$IVM_CHECKSUMS" | awk '$2=="ltvm/restore.sh"{n++} END{print n+0}')" -eq 1 ] || refuse "Embedded restore program is not covered exactly once by archive checksums."
    printf '%s\n' "$IVM_CHECKSUMS" | while IFS= read -r ivm_validate_archive_line; do
        ivm_validate_archive_expected="${ivm_validate_archive_line%% *}"
        ivm_validate_archive_member="${ivm_validate_archive_line#*  }"
        printf '%s\n' "${ivm_validate_archive_expected}" | grep -Eq '^[0-9a-f]{64}$' || exit 8
        printf '%s\n' "${ivm_validate_archive_member}" | grep -Eq '^ltvm/[A-Za-z0-9._/-]+$' || exit 8
        case "${ivm_validate_archive_member}" in ltvm/*) : ;; *) exit 8 ;; esac
        case "${ivm_validate_archive_member}" in *'/../'*|../*|*/..|..|/*) exit 8 ;; esac
        [ "$(printf '%s\n' "$IVM_LISTING" | grep -Fxc "${ivm_validate_archive_member}" || :)" -eq 1 ] || exit 8
        ivm_validate_archive_actual="$(tar -xOf "$ivm_validate_archive_file" "${ivm_validate_archive_member}" 2>/dev/null | sha256sum | awk '{print $1}')"
        [ "${ivm_validate_archive_actual}" = "${ivm_validate_archive_expected}" ] || exit 9
    done || refuse "Archive checksum paths or content verification failed."
    printf '%s\n' "$IVM_LISTING" | while IFS= read -r ivm_validate_archive_member; do
        case "$ivm_validate_archive_member" in */|ltvm/checksums.sha256) continue ;; esac
        printf '%s\n' "$IVM_CHECKSUMS" | awk -v m="$ivm_validate_archive_member" '$2==m{n++} END{exit !(n==1)}' || exit 10
...
parse_arguments() {
    ARCHIVE=""; TARGET_VMID=""; STORAGE=""; START_GUEST=0; VERIFY_CONTENT=1
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      *) [ -z "$ARCHIVE" ] || usage_error "Only one archive path is accepted."; ARCHIVE="$1"; shift ;;
     esac
    done
    [ -n "$ARCHIVE" ] || usage_error "Archive path is required."
