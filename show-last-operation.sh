...
main() {
    ...
    need_commands cat stat readlink
    SOL_ROOT="${LONGTAILTOIL_JOURNAL_DIR:-/var/tmp/longtailtoil-operations}"
    [ -d "$SOL_ROOT" ] || refuse "No LongTail operation journal directory exists: $SOL_ROOT"
    [ ! -L "$SOL_ROOT" ] || refuse "Journal root must not be a symlink: $SOL_ROOT"
    SOL_OWNER="$(stat -Lc '%u' "$SOL_ROOT" 2>/dev/null || :)"
    [ "$SOL_OWNER" = "0" ] || refuse "Journal root is not root-owned: $SOL_ROOT"

    if [ -n "$OPID" ]; then
        case "$OPID" in *[!A-Za-z0-9._-]*|"") usage_error "Invalid operation ID." ;; esac
        SOL_FILE="$SOL_ROOT/$OPID.journal"
        [ ! -L "$SOL_FILE" ] || refuse "Named journal must be a regular file, not a symlink."
    else
        [ -L "$SOL_ROOT/latest" ] || refuse "No latest operation journal symlink is available."
        SOL_LINK="$(readlink "$SOL_ROOT/latest" 2>/dev/null || :)"
        case "$SOL_LINK" in
            *[!A-Za-z0-9._-]*|"") refuse "Latest journal link has an unsafe target." ;;
        esac
        SOL_FILE="$SOL_ROOT/$SOL_LINK"
    fi

    [ -f "$SOL_FILE" ] && [ ! -L "$SOL_FILE" ] ||
        refuse "Operation journal not found as a regular file: $SOL_FILE"
    [ "$(stat -Lc '%u' "$SOL_FILE" 2>/dev/null || :)" = "0" ] ||
        refuse "Operation journal is not root-owned: $SOL_FILE"
    cat "$SOL_FILE"
}
...
usage() {
    cat <<EOF
show-last-operation.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
      ...
      *) [ -z "$OPID" ] || usage_error "Only one operation ID is accepted."; OPID="$1"; shift ;;
     esac
    done
}
