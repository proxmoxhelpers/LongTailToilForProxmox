...
# Compact one-guest storage topology view.
############################################################
...
main() {
    ...
    need_commands qm pct pvesm lvs awk sed grep basename
    SVM_KIND="$(guest_kind "$VMID")" || refuse "Guest $VMID does not exist."
    SVM_CFG="$(guest_config_path "$VMID")"
    if [ "$OUTPUT_FORMAT" = text ]; then
        printf '%-10s %-38s %-38s %-14s %-14s %-12s %-16s\n' SLOT VOLUME PATH SIZE POOL ORIGIN UUID
    elif [ "$OUTPUT_FORMAT" = json ]; then printf '['; SVM_FIRST=1; fi
    while IFS='|' read -r SVM_SLOT SVM_VALUE; do
        [ -n "$SVM_SLOT" ] || continue
        SVM_VOLID="${SVM_VALUE%%,*}"
        case "$SVM_VOLID" in *:*) : ;; *) continue ;; esac
        SVM_PATH="$(resolve_volid_path "$SVM_VOLID" || :)"
        SVM_IDENTITY=""
        [ -z "$SVM_PATH" ] || SVM_IDENTITY="$(lvm_identity "$SVM_PATH" || :)"
        if [ -n "$SVM_IDENTITY" ]; then
            SVM_UUID="$(printf '%s\n' "$SVM_IDENTITY" | cut -d'|' -f3)"
            SVM_SIZE="$(printf '%s\n' "$SVM_IDENTITY" | cut -d'|' -f4)"
            SVM_POOL="$(printf '%s\n' "$SVM_IDENTITY" | cut -d'|' -f5)"
            SVM_ORIGIN="$(printf '%s\n' "$SVM_IDENTITY" | cut -d'|' -f6)"
        else
            SVM_UUID="-"; SVM_SIZE="-"; SVM_POOL="-"; SVM_ORIGIN="-"
        fi
        case "$OUTPUT_FORMAT" in
            json)
                [ "$SVM_FIRST" -eq 1 ] || printf ','
                SVM_FIRST=0
                printf '{"slot":"%s","volume":"%s","path":"%s","size":"%s","pool":"%s","origin":"%s","uuid":"%s"}' \
                    "$(json_escape "$SVM_SLOT")" "$(json_escape "$SVM_VOLID")" "$(json_escape "${SVM_PATH:--}")" \
                    "$(json_escape "$SVM_SIZE")" "$(json_escape "${SVM_POOL:--}")" "$(json_escape "${SVM_ORIGIN:--}")" "$(json_escape "$SVM_UUID")"
                ;;
            tsv) printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$SVM_SLOT" "$SVM_VOLID" "${SVM_PATH:--}" "$SVM_SIZE" "${SVM_POOL:--}" "${SVM_ORIGIN:--}" "$SVM_UUID" ;;
            *) printf '%-10s %-38s %-38s %-14s %-14s %-12s %-16s\n' "$SVM_SLOT" "$SVM_VOLID" "${SVM_PATH:--}" "$SVM_SIZE" "${SVM_POOL:--}" "${SVM_ORIGIN:--}" "$SVM_UUID" ;;
        esac
    done <<EOF
    $(awk '
    /^(ide|sata|scsi|virtio)[0-9]+: / || /^(efidisk|tpmstate)[0-9]+: / || /^unused[0-9]+: / || /^rootfs: / || /^mp[0-9]+: / {
      key=$1; sub(/:$/, "", key); line=$0; sub(/^[^:]+:[[:space:]]*/, "", line); print key "|" line
    }' "$SVM_CFG")
EOF
...
usage() {
    cat <<EOF
show-vm-storage-map.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            *) [ -z "$VMID" ] || usage_error "Only one VMID is accepted."; VMID="$1"; shift ;;
        esac
    done
