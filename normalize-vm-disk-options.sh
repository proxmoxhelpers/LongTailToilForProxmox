...
# Conservative disk-option compatibility cleanup.
############################################################
...
main() {
    ...
    need_commands qm awk sed grep date
    require_qemu_vm "$VMID"; [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    if [ -n "$SLOT" ]; then valid_disk_slot "$SLOT" || usage_error "Invalid or out-of-range slot: $SLOT"; fi
    NVO_CHANGED=0
    NVO_ROLLBACK="$(mktemp "${TMPDIR:-/var/tmp}/lt-normalize-options.XXXXXX")"
    trap 'rm -f "$NVO_ROLLBACK"' 0 HUP INT TERM
    for NVO_SLOT in $(qm config "$VMID" | awk -F: '/^(ide|sata|scsi|virtio)[0-9]+:/ {print $1}'); do
     [ -z "$SLOT" ] || [ "$NVO_SLOT" = "$SLOT" ] || continue
     NVO_OLD="$(disk_value "$VMID" "$NVO_SLOT")"; NVO_NEW="$NVO_OLD"
     case "$NVO_SLOT" in ide*|sata*) NVO_NEW="$(printf '%s\n' "$NVO_NEW" | sed 's/,iothread=[^,]*//g')" ;; esac
     if [ "$NVO_NEW" != "$NVO_OLD" ]; then
       NVO_CHANGED=$((NVO_CHANGED + 1)); warn "Removing incompatible iothread from $NVO_SLOT."
       if [ "$PREFLIGHT" -eq 0 ]; then
           if [ "$DRYRUN" -eq 1 ]; then
               run_mutation qm set "$VMID" "--$NVO_SLOT" "$NVO_NEW"
           else
               printf '%s|%s\n' "$NVO_SLOT" "$NVO_OLD" >> "$NVO_ROLLBACK"
               NVO_TX_ACTIVE=1
               trap 'nvo_restore_on_exit' 0
               trap 'exit 129' HUP
               trap 'exit 130' INT
               trap 'exit 143' TERM
               qm set "$VMID" "--$NVO_SLOT" "$NVO_NEW"
               [ "$(disk_value "$VMID" "$NVO_SLOT")" = "$NVO_NEW" ] ||
                   verification_failure "Normalized value did not stick at $NVO_SLOT."
           fi
       fi
     fi
    done
    if [ "$PREFLIGHT" -eq 0 ] && [ "$DRYRUN" -eq 0 ]; then
        qm config "$VMID" >/dev/null || verification_failure "Proxmox cannot parse VM after normalization."
        NVO_TX_ACTIVE=0
        trap - 0 HUP INT TERM
        rm -f "$NVO_ROLLBACK"
    fi
    ok "Disk option normalization changes: $NVO_CHANGED"
}
...
usage() {
    cat <<EOF
normalize-vm-disk-options.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# nvo_restore_on_exit
# Restores every slot already changed by this invocation after an abnormal exit.
nvo_restore_on_exit() {
    nvo_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${NVO_TX_ACTIVE:-0}" -eq 1 ] && [ -f "${NVO_ROLLBACK:-}" ]; then
        while IFS='|' read -r nvo_restore_slot nvo_restore_value; do
            [ -n "$nvo_restore_slot" ] || continue
            qm set "$VMID" "--$nvo_restore_slot" "$nvo_restore_value" >/dev/null 2>&1 || :
        done < "$NVO_ROLLBACK"
        warn "Restored disk options after interrupted normalization."
    fi
    rm -f "${NVO_ROLLBACK:-}" 2>/dev/null || :
    [ "$nvo_restore_status" -eq 0 ] || exit "$nvo_restore_status"
    return 0
}
