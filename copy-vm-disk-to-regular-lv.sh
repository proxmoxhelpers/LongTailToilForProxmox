...
############################################################
# copy-vm-disk-to-regular-lv.sh
# Convenience VM-disk to regular-LV copy.
############################################################
...
setup() {
    ...
    CVR_COMPLETE=0; CVR_SOURCE_ACTIVATED_BY_US=0; ALLOW_RUNNING=0
...
main() {
    ...
    need_commands qm pvesm lvs lvcreate lvremove lvchange blockdev dd cmp date cut head find awk
    ...
    CVR_VOLID="$(disk_volid "$VMID" "$SLOT")" || refuse "Slot $SLOT is not a storage-backed disk."
    CVR_SOURCE="$(resolve_volid_path "$CVR_VOLID")" || refuse "Cannot resolve source volume."
    CVR_SRC_ID="$(lvm_identity "$CVR_SOURCE")" || refuse "Source disk must be backed by an LVM logical volume."
    CVR_SOURCE_VG="$(printf '%s\n' "$CVR_SRC_ID" | cut -d'|' -f1)"
    CVR_SOURCE_LV="$(printf '%s\n' "$CVR_SRC_ID" | cut -d'|' -f2)"
    CVR_SOURCE="/dev/${CVR_SOURCE_VG}/${CVR_SOURCE_LV}"
    CVR_SIZE="$(lvs --noheadings --units b --nosuffix -o lv_size "${CVR_SOURCE_VG}/${CVR_SOURCE_LV}" 2>/dev/null | awk 'NF {printf "%.0f\n", $1; exit}')"
    [ -n "$CVR_SIZE" ] || refuse "Could not determine source LV size."
    CVR_VM_STATUS="$(qm status "$VMID" 2>/dev/null | awk '{print $2}')"
    if [ "$CVR_VM_STATUS" = running ] && [ "${ALLOW_RUNNING:-0}" -ne 1 ]; then
    ...
    [ "$CVR_VM_STATUS" != running ] || warn "Source VM is running; --allow-running accepts crash/application-consistency risk while bytes are copied."
    ! lvs "$DEST_VG/$DEST_NAME" >/dev/null 2>&1 || refuse "Destination LV already exists."
    info "$VMID/$SLOT -> /dev/$DEST_VG/$DEST_NAME ($CVR_SIZE bytes, regular LV)"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin copy-vm-disk-to-regular-lv; journal_note source "$CVR_VOLID"; journal_note destination "$DEST_VG/$DEST_NAME"
    trap 'cvr_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cvr_ensure_source_device
    run_mutation lvcreate -L "${CVR_SIZE}B" -n "$DEST_NAME" "$DEST_VG"
    if [ "$DRYRUN" -eq 0 ]; then
        CVR_DEST_UUID="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        [ -n "${CVR_DEST_UUID}" ] || die "Created destination LV but could not capture its UUID."
    fi
    if [ "$DRYRUN" -eq 1 ]; then print_command dd "if=$CVR_SOURCE" "of=/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=fsync status=progress; cvr_release_source_device; CVR_COMPLETE=1; return 0; fi
    dd if="$CVR_SOURCE" of="/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=fsync status=progress
    cmp -n "$CVR_SIZE" "$CVR_SOURCE" "/dev/$DEST_VG/$DEST_NAME"
    cvr_release_source_device
    CVR_COMPLETE=1; ok "Regular-LV VM disk copy created and verified."
}
...
usage() {
    cat <<EOF
copy-vm-disk-to-regular-lv.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  copy-vm-disk-to-regular-lv.sh <VMID> <slot> <destination-VG> [new-LV-name] [--allow-running] [dryrun|--preflight]

DESCRIPTION
  Creates an independent regular-LV copy of a VM disk and verifies every byte.
  Regular destinations never use sparse dd writes. No VM configuration changes.
...
parse_arguments() {
    ...
    [ -n "$DEST_NAME" ] || DEST_NAME="vm-${VMID}-${SLOT}-regular-copy"
}

# cvr_ensure_source_device
# Call: cvr_ensure_source_device
# Activates an inactive source LV with -K only for this helper's copy window.
cvr_ensure_source_device() {
    [ -b "$CVR_SOURCE" ] && return 0
    info "Source LV is inactive; temporarily activating it for the block copy..."
    if [ "$DRYRUN" -eq 1 ]; then
        dryrun_cmd lvchange -ay -K "$CVR_SOURCE_VG/$CVR_SOURCE_LV"
        CVR_SOURCE_ACTIVATED_BY_US=1
        dryrun_verify "Source LV would be active and readable for the copy"
        return 0
    fi
    run_lvm_filtered lvchange -ay -K "$CVR_SOURCE_VG/$CVR_SOURCE_LV" || die "Could not activate source LV for block copying: $CVR_SOURCE_VG/$CVR_SOURCE_LV"
    CVR_SOURCE_ACTIVATED_BY_US=1
    [ -b "$CVR_SOURCE" ] || die "Source LV activation succeeded but no block device appeared: $CVR_SOURCE"
}

# cvr_release_source_device
# Call: cvr_release_source_device
# Restores an LV activated by this helper to its original inactive state.
cvr_release_source_device() {
    [ "${CVR_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ] || return 0
    info "Restoring source LV to its original inactive state..."
    if [ "$DRYRUN" -eq 1 ]; then dryrun_cmd lvchange -an "$CVR_SOURCE_VG/$CVR_SOURCE_LV"
    else run_lvm_filtered lvchange -an "$CVR_SOURCE_VG/$CVR_SOURCE_LV" || { warn "Could not restore source LV to inactive state: $CVR_SOURCE_VG/$CVR_SOURCE_LV"; return 1; }; fi
    CVR_SOURCE_ACTIVATED_BY_US=0
}

# cvr_cleanup STATUS
# Call: cvr_cleanup STATUS
# Restores activation state and removes only copy-owned temporary state on exit.
cvr_cleanup() {
    cvr_cleanup_status="$1"
    trap - 0 HUP INT TERM

    # Restore source activation state before doing anything that may fail.
    if [ "${CVR_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ]; then
        if run_lvm_filtered lvchange -an "${CVR_SOURCE_VG}/${CVR_SOURCE_LV}" >/dev/null 2>&1; then
            CVR_SOURCE_ACTIVATED_BY_US=0
        else
            warn "Source LV could not be restored to inactive state: ${CVR_SOURCE_VG}/${CVR_SOURCE_LV}"
        fi
    fi
    ...
    if [ "${CVR_COMPLETE:-0}" -ne 1 ] && [ -n "${CVR_DEST_UUID:-}" ]; then
        cvr_cleanup_uuid="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        if [ -n "${cvr_cleanup_uuid}" ] && [ "${cvr_cleanup_uuid}" = "${CVR_DEST_UUID}" ]; then
        ...
        elif [ -n "${cvr_cleanup_uuid}" ]; then
            warn "Destination name now refers to a different LV UUID; preserving it."
        fi
    fi

    [ "${cvr_cleanup_status}" -eq 0 ] || exit "${cvr_cleanup_status}"
    return 0
}
