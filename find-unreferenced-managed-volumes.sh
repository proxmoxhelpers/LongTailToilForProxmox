...
# Narrow managed-LV orphan inventory.
############################################################
...
main() {
    ...
    need_commands lvs awk sed

    FUM_COUNT=0
    printf '%-24s %-36s %-38s %-38s\n' VG LV UUID PATH
    while IFS='|' read -r FUM_VG FUM_LV FUM_UUID; do
        [ -n "$FUM_LV" ] || continue
        [ -z "$VG" ] || [ "$FUM_VG" = "$VG" ] || continue
        FUM_NUM="$(printf '%s\n' "$FUM_LV" |
            sed -n 's/^\(vm\|base\)-[0-9][0-9]*-disk-\([0-9][0-9]*\)$/\2/p')"
        [ -n "$FUM_NUM" ] || continue

        # Reference decisions are made from the physical LV UUID, not from the
        # textual storage ID or LV name, so Proxmox storage aliases are covered.
        [ "$(lv_uuid_reference_count "$FUM_UUID")" -eq 0 ] || continue
        FUM_PATH="/dev/$FUM_VG/$FUM_LV"
        printf '%-24s %-36s %-38s %-38s\n' "$FUM_VG" "$FUM_LV" "$FUM_UUID" "$FUM_PATH"
        FUM_COUNT=$((FUM_COUNT + 1))
    done <<EOF
$(lvs --noheadings --separator '|' -o vg_name,lv_name,lv_uuid 2>/dev/null |
  sed 's/^[[:space:]]*//;s/[[:space:]]*$//;s/[[:space:]]*|[[:space:]]*/|/g')
EOF
    info "Unreferenced managed volumes: $FUM_COUNT"
}
...
usage() {
    cat <<EOF
find-unreferenced-managed-volumes.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
