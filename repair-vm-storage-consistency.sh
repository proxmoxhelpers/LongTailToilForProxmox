...
# Conservative unambiguous storage-name repair workflow.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs lvrename cp awk sed grep mktemp date cut head find sort
    require_qemu_vm "$VMID"; [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    RVSC_CFG="/etc/pve/qemu-server/${VMID}.conf"
    ! qemu_config_has_snapshots "$RVSC_CFG" || refuse "VM snapshots are present; direct config repair is refused."
    RVSC_PLAN="$(mktemp "${TMPDIR:-/tmp}/lt-repair-plan.XXXXXX")"
    RVSC_DONE="$(mktemp "${TMPDIR:-/tmp}/lt-repair-done.XXXXXX")"
    RVSC_REVERSE="$(mktemp "${TMPDIR:-/tmp}/lt-repair-reverse.XXXXXX")"
    RVSC_REWRITE_TMP="$(mktemp "${TMPDIR:-/tmp}/lt-repair-rewrite.XXXXXX")"
    RVSC_MUTATING=0; RVSC_COMPLETE=0; : > "$RVSC_PLAN"; : > "$RVSC_DONE"
    trap 'rvsc_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    while IFS='|' read -r RVSC_SLOT RVSC_VALUE; do
     [ -n "$RVSC_SLOT" ] || continue
     RVSC_VOLID="${RVSC_VALUE%%,*}"; case "$RVSC_VOLID" in *:*) : ;; *) continue ;; esac
     RVSC_PATH="$(resolve_volid_path "$RVSC_VOLID" || :)"; [ -n "$RVSC_PATH" ] || continue
     RVSC_ID="$(lvm_identity "$RVSC_PATH" || :)"; [ -n "$RVSC_ID" ] || continue
     RVSC_VG="$(printf '%s\n' "$RVSC_ID" | cut -d'|' -f1)"; RVSC_LV="$(printf '%s\n' "$RVSC_ID" | cut -d'|' -f2)"
     RVSC_UUID="$(printf '%s\n' "$RVSC_ID" | cut -d'|' -f3)"
     RVSC_FAMILY="$(printf '%s\n' "$RVSC_LV" | sed -n 's/^\(vm\|base\)-[0-9][0-9]*-disk-[0-9][0-9]*$/\1/p')"
     RVSC_NUM="$(printf '%s\n' "$RVSC_LV" | sed -n 's/^\(vm\|base\)-[0-9][0-9]*-disk-\([0-9][0-9]*\)$/\2/p')"
     RVSC_EMBED="$(printf '%s\n' "$RVSC_LV" | sed -n 's/^\(vm\|base\)-\([0-9][0-9]*\)-disk-[0-9][0-9]*$/\2/p')"
     [ -n "$RVSC_FAMILY" ] && [ "$RVSC_EMBED" != "$VMID" ] || continue
     [ "$(volid_reference_count "$RVSC_VOLID")" -eq 1 ] || refuse "$RVSC_VOLID is shared/referenced by more than this VM; repair refuses to rename it."
     RVSC_NEW="${RVSC_FAMILY}-${VMID}-disk-${RVSC_NUM}"
     ! lvs "$RVSC_VG/$RVSC_NEW" >/dev/null 2>&1 || refuse "Exact corrected name already exists: $RVSC_VG/$RVSC_NEW"
     RVSC_STORAGE="${RVSC_VOLID%%:*}"; RVSC_NEW_VOLID="$RVSC_STORAGE:$RVSC_NEW"
     printf '%s|%s|%s|%s|%s|%s|%s\n' "$RVSC_SLOT" "$RVSC_VOLID" "$RVSC_NEW_VOLID" "$RVSC_VG" "$RVSC_LV" "$RVSC_NEW" "$RVSC_UUID" >> "$RVSC_PLAN"
    done <<EOF
$(awk -F': ' '/^(ide|sata|scsi|virtio)[0-9]+:|^(efidisk|tpmstate)[0-9]+:|^unused[0-9]+:/ {print $1 "|" $2}' "$RVSC_CFG")
EOF
    RVSC_COUNT="$(awk 'END{print NR+0}' "$RVSC_PLAN")"
    if [ "$RVSC_COUNT" -eq 0 ]; then ok "No unambiguous managed-name VMID mismatches found."; return 0; fi
    info "Unambiguous repairs:"
    while IFS='|' read -r RVSC_SLOT RVSC_OLD_VOLID RVSC_NEW_VOLID RVSC_VG RVSC_OLD RVSC_NEW RVSC_UUID; do
        printf '%s: %s -> %s (UUID %s)\n' "$RVSC_SLOT" "$RVSC_OLD_VOLID" "$RVSC_NEW_VOLID" "$RVSC_UUID"
    done < "$RVSC_PLAN"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin repair-vm-storage-consistency; journal_note vmid "$VMID"; journal_note repairs "$RVSC_COUNT"
    RVSC_BACKUP="/root/${VMID}.conf.before-storage-repair.$(date +%Y%m%d-%H%M%S)"
    if [ "$DRYRUN" -eq 1 ]; then
     print_command cp "$RVSC_CFG" "$RVSC_BACKUP"
     while IFS='|' read -r RVSC_SLOT RVSC_OLD_VOLID RVSC_NEW_VOLID RVSC_VG RVSC_OLD RVSC_NEW RVSC_UUID; do
         print_command lvrename "$RVSC_VG" "$RVSC_OLD" "$RVSC_NEW"
         printf '[DRYRUN] rewrite exact volume reference %s -> %s in %s\n' "$RVSC_OLD_VOLID" "$RVSC_NEW_VOLID" "$RVSC_CFG"
     done < "$RVSC_PLAN"
     return 0
    fi
    cp "$RVSC_CFG" "$RVSC_BACKUP"; RVSC_MUTATING=1
    while IFS='|' read -r RVSC_SLOT RVSC_OLD_VOLID RVSC_NEW_VOLID RVSC_VG RVSC_OLD RVSC_NEW RVSC_UUID; do
     if ! lvrename "$RVSC_VG" "$RVSC_OLD" "$RVSC_NEW"; then verification_failure "LV rename failed; UUID-safe rollback will run."; fi
     printf '%s|%s|%s|%s|%s|%s|%s\n' "$RVSC_SLOT" "$RVSC_OLD_VOLID" "$RVSC_NEW_VOLID" "$RVSC_VG" "$RVSC_OLD" "$RVSC_NEW" "$RVSC_UUID" >> "$RVSC_DONE"
     RVSC_CURRENT="$(rvsc_lv_name_by_uuid "$RVSC_VG" "$RVSC_UUID" || :)"
     [ "$RVSC_CURRENT" = "$RVSC_NEW" ] || verification_failure "Renamed LV UUID cannot be found at expected name; rollback will run."
     rvsc_rewrite_literal "$RVSC_CFG" "$RVSC_OLD_VOLID" "$RVSC_NEW_VOLID" || verification_failure "Config rewrite failed; UUID-safe rollback will run."
    done < "$RVSC_PLAN"
    qm config "$VMID" >/dev/null 2>&1 || verification_failure "Config validation failed; UUID-safe rollback will run."
    while IFS='|' read -r RVSC_SLOT RVSC_OLD_VOLID RVSC_NEW_VOLID RVSC_VG RVSC_OLD RVSC_NEW RVSC_UUID; do
        [ "$(rvsc_lv_name_by_uuid "$RVSC_VG" "$RVSC_UUID" || :)" = "$RVSC_NEW" ] || verification_failure "Postcondition failed for LV UUID $RVSC_UUID."
        grep -qF "$RVSC_NEW_VOLID" "$RVSC_CFG" || verification_failure "Postcondition failed for config reference $RVSC_NEW_VOLID."
    done < "$RVSC_PLAN"
    RVSC_COMPLETE=1; journal_note backup "$RVSC_BACKUP"; ok "Applied and verified $RVSC_COUNT exact managed-name repair(s). Backup: $RVSC_BACKUP"
}
...
usage() {
    cat <<EOF
repair-vm-storage-consistency.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# rvsc_lv_name_by_uuid VG UUID
# Call: rvsc_lv_name_by_uuid VG UUID
# Prints the unique LV name carrying UUID inside VG.
rvsc_lv_name_by_uuid() {
    rvsc_lv_name_by_uuid_vg="$1"; rvsc_lv_name_by_uuid_uuid="$2"
    lvs --noheadings --separator '|' -o lv_name,lv_uuid "$rvsc_lv_name_by_uuid_vg" 2>/dev/null |
      awk -F'|' -v u="$rvsc_lv_name_by_uuid_uuid" '{gsub(/^[ \t]+|[ \t]+$/,"",$1);gsub(/^[ \t]+|[ \t]+$/,"",$2);if($2==u){n++;name=$1}} END{if(n==1)print name;else exit 1}'
}

# rvsc_rewrite_literal FILE OLD NEW
# Call: rvsc_rewrite_literal FILE OLD NEW
# Replaces OLD as a literal byte string, avoiding sed-regex interpretation.
rvsc_rewrite_literal() {
    rvsc_rewrite_literal_file="$1"; rvsc_rewrite_literal_old="$2"; rvsc_rewrite_literal_new="$3"
    awk -v old="$rvsc_rewrite_literal_old" -v new="$rvsc_rewrite_literal_new" '
      {
        line=$0
        while ((p=index(line,old))>0) line=substr(line,1,p-1) new substr(line,p+length(old))
        print line
      }
    ' "$rvsc_rewrite_literal_file" > "$RVSC_REWRITE_TMP" || return 1
    cat "$RVSC_REWRITE_TMP" > "$rvsc_rewrite_literal_file"
}

# rvsc_rollback
# Call: rvsc_rollback
# Rolls completed UUID-tracked renames back in reverse order and keeps config
# references synchronized with each successful reverse rename.
rvsc_rollback() {
    [ -s "${RVSC_DONE:-}" ] || return 0
    awk '{line[NR]=$0} END{for(i=NR;i>=1;i--)print line[i]}' "$RVSC_DONE" > "$RVSC_REVERSE" || return 1
    rvsc_rollback_failed=0
    while IFS='|' read -r rvsc_rollback_slot rvsc_rollback_old_volid rvsc_rollback_new_volid rvsc_rollback_vg rvsc_rollback_old rvsc_rollback_new rvsc_rollback_uuid; do
        rvsc_rollback_current="$(rvsc_lv_name_by_uuid "$rvsc_rollback_vg" "$rvsc_rollback_uuid" || :)"
        case "$rvsc_rollback_current" in
          "$rvsc_rollback_new")
            if lvrename "$rvsc_rollback_vg" "$rvsc_rollback_new" "$rvsc_rollback_old" >/dev/null 2>&1; then
                rvsc_rewrite_literal "$RVSC_CFG" "$rvsc_rollback_new_volid" "$rvsc_rollback_old_volid" || rvsc_rollback_failed=1
            else
                warn "Rollback could not rename UUID $rvsc_rollback_uuid from $rvsc_rollback_new to $rvsc_rollback_old."
                rvsc_rollback_failed=1
            fi
            ;;
          "$rvsc_rollback_old")
            rvsc_rewrite_literal "$RVSC_CFG" "$rvsc_rollback_new_volid" "$rvsc_rollback_old_volid" || rvsc_rollback_failed=1
            ;;
          *)
            warn "Rollback cannot locate UUID $rvsc_rollback_uuid at either expected LV name."
            rvsc_rollback_failed=1
            ;;
        esac
    done < "$RVSC_REVERSE"
    if [ "$rvsc_rollback_failed" -eq 0 ]; then
        cat "$RVSC_BACKUP" > "$RVSC_CFG"
        qm config "$VMID" >/dev/null 2>&1 || rvsc_rollback_failed=1
    fi
    [ "$rvsc_rollback_failed" -eq 0 ]
}

# rvsc_cleanup STATUS
# Call: rvsc_cleanup STATUS
# Executes rollback on abnormal exit after mutation has begun, then removes only
# temporary plan files. The root-owned recovery backup is intentionally retained.
rvsc_cleanup() {
    rvsc_cleanup_status="$1"; trap - 0 HUP INT TERM
    if [ "${RVSC_MUTATING:-0}" -eq 1 ] && [ "${RVSC_COMPLETE:-0}" -ne 1 ]; then
        warn "Repair did not complete; attempting UUID-safe reverse-order rollback."
        rvsc_rollback || warn "ROLLBACK FAILED: inspect $RVSC_CFG and backup ${RVSC_BACKUP:-unknown} before retrying."
    fi
    rm -f "${RVSC_PLAN:-}" "${RVSC_DONE:-}" "${RVSC_REVERSE:-}" "${RVSC_REWRITE_TMP:-}" 2>/dev/null || :
    [ "$rvsc_cleanup_status" -eq 0 ] || exit "$rvsc_cleanup_status"
}
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
      ...
      *) [ -z "$VMID" ] || usage_error "One VMID is required."; VMID="$1"; shift ;;
     esac
    done
