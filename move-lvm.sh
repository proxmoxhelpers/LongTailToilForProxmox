...
validate_source_and_destination() {
    ...
    vsad_rel="${DESTINATION#/dev/}"; DEST_VG="${vsad_rel%%/*}"; DEST_LV="${vsad_rel#*/}"
...
# Call: assert_not_in_use LV
assert_not_in_use() {
    aniu_real="$(readlink -f "$1")"
    aniu_open="$(dmsetup info -c --noheadings -o open "$aniu_real" 2>/dev/null | tr -d '[:space:]' || :)"
    case "$aniu_open" in ''|*[!0-9]*) return 0 ;; esac
    [ "$aniu_open" -eq 0 ] || die "Source LV is open/in use (device-mapper open count: $aniu_open). Stop any VM/CT or process using it first."
}

# validate_no_dependents
# Refuses cross-VG deletion when source is an origin for dependent LVs.
# Call: validate_no_dependents ARG1 [ARG2]
validate_no_dependents() {
...
# Call: run_embedded_copy_lvm [ARG...]
run_embedded_copy_lvm() {
...
# Call: select_destination_allocation ARG1 [ARG2]
select_destination_allocation() {
