...
# Copy-and-verify conversion to an independent thin LV.
############################################################
...
setup() {
    ...
    CLT_COMPLETE=0; CLT_SOURCE_ACTIVATED_BY_US=0
...
main() {
    ...
    need_commands lvs vgs lvcreate lvremove lvchange blockdev dd cmp awk sed grep basename date cut head find
    ...
    CLT_SRC_ID="$(lvm_identity "$SOURCE")" || refuse "Source LV does not exist."
    CLT_SOURCE_VG="$(printf '%s\n' "$CLT_SRC_ID" | cut -d'|' -f1)"
    CLT_SOURCE_LV="$(printf '%s\n' "$CLT_SRC_ID" | cut -d'|' -f2)"
    SOURCE="/dev/$CLT_SOURCE_VG/$CLT_SOURCE_LV"
    CLT_SIZE="$(lvs --noheadings --units b --nosuffix -o lv_size "$CLT_SOURCE_VG/$CLT_SOURCE_LV" 2>/dev/null | awk 'NF {printf "%.0f\n", $1; exit}')"
    [ -n "$CLT_SIZE" ] || refuse "Could not determine source LV size."
    if [ -z "$POOL" ]; then
     CLT_POOLS="$(lvs --noheadings -o lv_name --select 'lv_attr=~^t' "$DEST_VG" 2>/dev/null | awk '{$1=$1; if(NF)print}')"
     CLT_COUNT="$(printf '%s\n' "$CLT_POOLS" | awk 'NF{n++} END{print n+0}')"
     [ "$CLT_COUNT" -eq 1 ] || refuse "Destination VG must have exactly one thin pool unless --pool is specified."
     POOL="$CLT_POOLS"
    fi
    valid_lvm_component "$POOL" || usage_error "Invalid thin-pool name: $POOL"
    CLT_POOL_ATTR="$(lvs --noheadings -o lv_attr "$DEST_VG/$POOL" 2>/dev/null | awk '{$1=$1;print;exit}')"
    case "$CLT_POOL_ATTR" in t*) : ;; *) refuse "Destination is not an LVM thin pool: $DEST_VG/$POOL" ;; esac
    ! lvs "$DEST_VG/$DEST_NAME" >/dev/null 2>&1 || refuse "Destination LV already exists: $DEST_VG/$DEST_NAME"
    info "Independent thin copy: $SOURCE -> /dev/$DEST_VG/$DEST_NAME ($CLT_SIZE bytes)"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin convert-lv-to-thin; journal_note source "$SOURCE"; journal_note destination "$DEST_VG/$DEST_NAME"
    trap 'clt_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    clt_ensure_source_device
    run_mutation lvcreate -V "${CLT_SIZE}B" -T "$DEST_VG/$POOL" -n "$DEST_NAME"
    if [ "$DRYRUN" -eq 0 ]; then
        CLT_DEST_UUID="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        [ -n "${CLT_DEST_UUID}" ] || die "Created destination LV but could not capture its UUID."
    fi
    if [ "$DRYRUN" -eq 1 ]; then print_command dd "if=$SOURCE" "of=/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=sparse,fsync status=progress; clt_release_source_device; CLT_COMPLETE=1; return 0; fi
    dd if="$SOURCE" of="/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=sparse,fsync status=progress
    cmp -n "$CLT_SIZE" "$SOURCE" "/dev/$DEST_VG/$DEST_NAME"
    clt_release_source_device
    CLT_COMPLETE=1
    ok "Independent thin LV created and byte-verified."
}
...
usage() {
    cat <<EOF
convert-lv-to-thin.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
      ...
      *) if [ -z "$SOURCE" ]; then SOURCE="$1"; elif [ -z "$DEST_VG" ]; then DEST_VG="$1"; elif [ -z "$DEST_NAME" ]; then DEST_NAME="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$SOURCE" ] && [ -n "$DEST_VG" ] || usage_error "Source LV and destination VG are required."
    [ -n "$DEST_NAME" ] || DEST_NAME="$(basename "$SOURCE")-thin"
}

# clt_ensure_source_device
# Call: clt_ensure_source_device
...
clt_ensure_source_device() {
    ...
    if [ "$DRYRUN" -eq 1 ]; then
        dryrun_cmd lvchange -ay -K "$CLT_SOURCE_VG/$CLT_SOURCE_LV"
        CLT_SOURCE_ACTIVATED_BY_US=1
        dryrun_verify "Source LV would be active and readable for the copy"
        return 0
    fi
    run_lvm_filtered lvchange -ay -K "$CLT_SOURCE_VG/$CLT_SOURCE_LV" || die "Could not activate source LV for block copying: $CLT_SOURCE_VG/$CLT_SOURCE_LV"
    CLT_SOURCE_ACTIVATED_BY_US=1
    [ -b "$SOURCE" ] || die "Source LV activation succeeded but no block device appeared: $SOURCE"
}

# clt_release_source_device
# Call: clt_release_source_device
# Restores an LV activated by this helper to its original inactive state.
clt_release_source_device() {
    [ "${CLT_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ] || return 0
    info "Restoring source LV to its original inactive state..."
    if [ "$DRYRUN" -eq 1 ]; then dryrun_cmd lvchange -an "$CLT_SOURCE_VG/$CLT_SOURCE_LV"
    else run_lvm_filtered lvchange -an "$CLT_SOURCE_VG/$CLT_SOURCE_LV" || { warn "Could not restore source LV to inactive state: $CLT_SOURCE_VG/$CLT_SOURCE_LV"; return 1; }; fi
    CLT_SOURCE_ACTIVATED_BY_US=0
}

# clt_cleanup STATUS
# Call: clt_cleanup STATUS
# Restores activation state and removes only conversion-owned temporary LVs on exit.
clt_cleanup() {
    clt_cleanup_status="$1"
    trap - 0 HUP INT TERM

    # Restore source activation state before doing anything that may fail.
    if [ "${CLT_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ]; then
        if run_lvm_filtered lvchange -an "${CLT_SOURCE_VG}/${CLT_SOURCE_LV}" >/dev/null 2>&1; then
            CLT_SOURCE_ACTIVATED_BY_US=0
        else
            warn "Source LV could not be restored to inactive state: ${CLT_SOURCE_VG}/${CLT_SOURCE_LV}"
        fi
    fi
    ...
    if [ "${CLT_COMPLETE:-0}" -ne 1 ] && [ -n "${CLT_DEST_UUID:-}" ]; then
        clt_cleanup_uuid="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        if [ -n "${clt_cleanup_uuid}" ] && [ "${clt_cleanup_uuid}" = "${CLT_DEST_UUID}" ]; then
        ...
        elif [ -n "${clt_cleanup_uuid}" ]; then
            warn "Destination name now refers to a different LV UUID; preserving it."
        fi
    fi

    [ "${clt_cleanup_status}" -eq 0 ] || exit "${clt_cleanup_status}"
    return 0
}
