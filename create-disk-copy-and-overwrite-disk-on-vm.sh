...
# Prints/finalizes the command result and performs normal completion cleanup.
    ...
    if [ "$DEST_EXISTS" -eq 1 ]; then print_banner "Destination disk overwritten with independent copy"
    else print_banner "Destination disk created with independent copy"; fi
    printf 'Source:          %s\n' "$SOURCE_PATH"
...
select_destination_allocation() {
    if [ "$DEST_EXISTS" -eq 0 ]; then
        DEST_VG="$SOURCE_VG"
        sda_map="$(unique_storage_for_lv "$SOURCE_PATH")"
        STORAGE_ID="${sda_map%%|*}"
        DEST_STORAGE_TYPE="${sda_map#*|}"
        if [ "$DEST_STORAGE_TYPE" = "lvmthin" ]; then
            DEST_POOL="$SOURCE_POOL"
            [ -n "$DEST_POOL" ] || die "Source storage is lvmthin but the source LV has no thin-pool metadata."
        else
            DEST_POOL=""
        fi
        return 0
...
# select_new_disk_name
# Chooses a collision-free PREFIX-DESTVMID-disk-N name in the destination VG.
select_new_disk_name() {
    NEW_VG="$DEST_VG"
...
# print_plan
# Summarizes the staged/final replacement, displaced-disk policy, destination state mode and boot intent before mutation.
print_plan() {
    print_banner "Overwrite/add VM disk with independent copy"
    printf 'Source LV:              %s\n' "$SOURCE_PATH"
...
# create_destination
# Creates the staged replacement LV and records its stable UUID before any destination mutation.
create_destination() {
    ...
    if ! dryrun_enabled; then
        ...
        NEW_UUID="$(lvs --noheadings -o lv_uuid "${DEST_VG}/${NEW_LV_NAME}" 2>/dev/null | trim)"
        [ -n "$NEW_UUID" ] || die "Could not record the replacement LV UUID."
    else
        NEW_UUID="dryrun-new-copy"
        dryrun_verify "Replacement LV would exist with sufficient size and a stable LV UUID"
    fi
}

# verify_storage_mapping
# Proves the staged Proxmox volume ID resolves to the newly created replacement LV UUID.
verify_storage_mapping() {
    if dryrun_enabled; then
        dryrun_verify "Proxmox storage $STORAGE_ID would resolve $NEW_VOLID to replacement LV UUID $NEW_UUID"
        return 0
    fi
    vsm_path="$(pvesm path "$NEW_VOLID" 2>/dev/null || :)"
    [ -n "$vsm_path" ] || die "Proxmox cannot resolve $NEW_VOLID."
    vsm_uuid="$(lvs --noheadings -o lv_uuid "$vsm_path" 2>/dev/null | trim || :)"
    [ -n "$vsm_uuid" ] || die "Proxmox resolved $NEW_VOLID, but the resolved path is not an identifiable LVM LV."
    [ "$vsm_uuid" = "$NEW_UUID" ] || {
        printf 'Expected LV UUID: %s\nResolved LV UUID: %s\nResolved path:    %s\n' "$NEW_UUID" "$vsm_uuid" "$vsm_path" >&2
...
ensure_source_device() {
    ...
    if dryrun_enabled; then
        dryrun_cmd lvchange -ay -K "${SOURCE_VG}/${SOURCE_LV}"
        SOURCE_ACTIVATED_BY_US=1
        dryrun_verify "Source LV would be active and readable for the copy"
        return 0
    fi
    run_lvm_filtered lvchange -ay -K "${SOURCE_VG}/${SOURCE_LV}" || die "Could not activate source LV for block copying: ${SOURCE_VG}/${SOURCE_LV}"
    SOURCE_ACTIVATED_BY_US=1
...
replace_destination_disk() {
    if [ "$DEST_EXISTS" -eq 0 ]; then
        ...
        info "Renaming staged replacement to disk-$DEST_DISK_NUMBER..."
        ...
        if ! dryrun_enabled; then
            ...
            [ "$rdd_final_uuid" = "$NEW_UUID" ] || die "New replacement UUID changed or final rename did not land on the intended LV."
        fi
        ...
        if ! dryrun_cmd qm set "$DEST_VM" "--${DEST_SLOT}" "$NEW_VOLID"; then
            ...
            die "Could not attach the new replacement disk."
        fi
        NEW_ATTACHED=1
        return 0
    fi
    ...
    info "Renaming staged replacement to the original disk number disk-$DEST_DISK_NUMBER..."
    if dryrun_enabled; then dryrun_cmd lvrename "$NEW_VG" "$TEMP_LV_NAME" "$FINAL_LV_NAME"
    else run_lvm_filtered lvrename "$NEW_VG" "$TEMP_LV_NAME" "$FINAL_LV_NAME"; fi
    REPLACEMENT_FINALIZED=1
    NEW_LV_NAME="$FINAL_LV_NAME"; NEW_LV_PATH="$FINAL_LV_PATH"; NEW_VOLID="$FINAL_VOLID"
...
# delete_archived_disk
# Deletes the displaced archived LV only after the replacement is attached and verified, when requested.
delete_archived_disk() {
...
# verify_result
# Verifies final disk identity, archived/deleted old-disk state, guest state, and optional boot order.
verify_result() {
    if dryrun_enabled; then
        if [ "$DEST_EXISTS" -eq 1 ]; then
...
# cleanup_on_exit
# Transaction trap: invokes UUID-safe rollback and restores guest/source activation state after failure.
cleanup_on_exit() {
    coe_status=$?
    trap - 0 HUP INT TERM
    set +e
    ...
    if ! dryrun_enabled && [ "$CREATED" -eq 1 ] && [ "$NEW_ATTACHED" -eq 0 ] && [ "$COMPLETE" -eq 0 ] && [ "$ROLLBACK_FAILED" -eq 0 ]; then
        if ! guest_volume_references | grep -F "|$NEW_VOLID" >/dev/null 2>&1; then
            warn "Removing unattached replacement disk: $NEW_LV_PATH"
