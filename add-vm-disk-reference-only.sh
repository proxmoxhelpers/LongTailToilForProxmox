...
# Advanced recovery primitive for config-only reference insertion.
############################################################
...
main() {
    ...
    need_commands qm pvesm cp grep mktemp date
    require_qemu_vm "$VMID"
    AVR_CFG="/etc/pve/qemu-server/${VMID}.conf"
    [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM $VMID must be stopped."
    ! qemu_config_has_snapshots "$AVR_CFG" || refuse "VM snapshots are present; direct config editing is refused."
    valid_qemu_storage_slot "$SLOT" || usage_error "Unsupported or out-of-range slot: $SLOT"
    single_line_value "$VOLID" || usage_error "Volume ID must be a single line."
    single_line_value "$OPTIONS" || usage_error "--options must be a single line."
    ! grep -qE "^${SLOT}:" "$AVR_CFG" || refuse "Slot $SLOT is already configured."
    AVR_PATH="$(resolve_volid_path "$VOLID" || :)"; [ -n "$AVR_PATH" ] || refuse "pvesm cannot resolve $VOLID."
    AVR_REFS="$(volid_reference_count "$VOLID")"
    if [ "$AVR_REFS" -gt 0 ] && [ "$ALLOW_SHARED" -eq 0 ]; then refuse "$VOLID is already referenced by $AVR_REFS guest config(s); pass --allow-shared only when intentional."; fi
    AVR_VALUE="$VOLID"; [ -z "$OPTIONS" ] || AVR_VALUE="$AVR_VALUE,$OPTIONS"
    info "Config-only add: $SLOT: $AVR_VALUE"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin add-vm-disk-reference-only; journal_note vmid "$VMID"; journal_note slot "$SLOT"; journal_note volume "$VOLID"
    AVR_BACKUP="/root/${VMID}.conf.before-reference-add.$(date +%Y%m%d-%H%M%S)"
    if [ "$DRYRUN" -eq 1 ]; then print_command cp "$AVR_CFG" "$AVR_BACKUP"; printf '[DRYRUN] append %s: %s to %s\n' "$SLOT" "$AVR_VALUE" "$AVR_CFG"; return 0; fi
    cp "$AVR_CFG" "$AVR_BACKUP"
    AVR_TX_ACTIVE=1
    trap 'avr_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    printf '%s: %s\n' "$SLOT" "$AVR_VALUE" >> "$AVR_CFG"
    if ! qm config "$VMID" >/dev/null 2>&1 || ! grep -qF "$SLOT: $AVR_VALUE" "$AVR_CFG"; then
     cat "$AVR_BACKUP" > "$AVR_CFG"
     AVR_TX_ACTIVE=0
     trap - 0 HUP INT TERM
    ...
    AVR_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    journal_note backup "$AVR_BACKUP"
    ok "Added config reference only. No storage allocation/free API was called. Backup: $AVR_BACKUP"
}
...
usage() {
    cat <<EOF
add-vm-disk-reference-only.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    VMID=""; SLOT=""; VOLID=""; OPTIONS=""; ALLOW_SHARED=0
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --options) [ "$#" -ge 2 ] || usage_error "--options requires a comma option string."; OPTIONS="$2"; shift 2 ;;
      --allow-shared) ALLOW_SHARED=1; shift ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; elif [ -z "$VOLID" ]; then VOLID="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$VMID" ] && [ -n "$SLOT" ] && [ -n "$VOLID" ] || usage_error "VMID, slot and Proxmox volume ID are required."
}



# avr_restore_on_exit
# Call: installed as EXIT trap after the config backup is created.
# Restores the original config on an abnormal exit before transaction commit.
avr_restore_on_exit() {
    avr_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${AVR_TX_ACTIVE:-0}" -eq 1 ] && [ -n "${AVR_BACKUP:-}" ] && [ -f "$AVR_BACKUP" ]; then
        cat "$AVR_BACKUP" > "$AVR_CFG" || :
        warn "Restored VM config after interrupted reference add."
    fi
    [ "$avr_restore_status" -eq 0 ] || exit "$avr_restore_status"
    return 0
}
