...
# Richer best-effort VM rebuild from existing managed volumes.
############################################################
...
# rve_config_safe_to_remove
# Succeeds only when every storage reference in the newly created config is one
# of the exact recovery-plan volume IDs. This prevents cleanup from deleting a
# VM config that acquired unrelated storage concurrently.

# rve_cleanup STATUS
# Removes only the newly created config on incomplete apply. Existing volumes
# are never freed by this helper.

main() {
    [ "$APP_ELEVATED" = "true" ] || self_elevate "$@"
    need_commands qm pvesm lvs awk sed grep sort mktemp date readlink

    [ ! -e "/etc/pve/qemu-server/${VMID}.conf" ] &&
    [ ! -e "/etc/pve/lxc/${VMID}.conf" ] ||
        refuse "VMID $VMID already exists."

    RVE_PLAN="$(mktemp "${TMPDIR:-/tmp}/lt-rebuild-plan.XXXXXX")"
    : > "$RVE_PLAN"
    RVE_COMPLETE=0
    RVE_CREATE_STARTED=0
    trap 'rve_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM

    while IFS='|' read -r RVE_VG RVE_LV RVE_UUID; do
        [ -n "$RVE_LV" ] || continue
        RVE_NUM="$(printf '%s\n' "$RVE_LV" |
            sed -n "s/^\\(vm\\|base\\)-${VMID}-disk-\\([0-9][0-9]*\\)$/\\2/p")"
        [ -n "$RVE_NUM" ] || continue

        RVE_VOLID="$(rve_find_volid "$RVE_VG" "$RVE_LV" "$RVE_UUID" || :)"
        [ -n "$RVE_VOLID" ] || {
            warn "No unique Proxmox storage mapping found for $RVE_VG/$RVE_LV"
            continue
        }
        [ "$(lv_uuid_reference_count "$RVE_UUID")" -eq 0 ] ||
            refuse "$RVE_VG/$RVE_LV is already referenced by another guest, possibly through a storage alias."
        printf '%s|%s|%s\n' "$RVE_NUM" "$RVE_VOLID" "$RVE_VG/$RVE_LV" >> "$RVE_PLAN"
    done <<EOF
$(lvs --noheadings --separator '|' -o vg_name,lv_name,lv_uuid 2>/dev/null |
  sed 's/^[[:space:]]*//;s/[[:space:]]*$//;s/[[:space:]]*|[[:space:]]*/|/g')
EOF

    sort -t'|' -k1,1n "$RVE_PLAN" -o "$RVE_PLAN"
    RVE_COUNT="$(awk 'END{print NR+0}' "$RVE_PLAN")"
    [ "$RVE_COUNT" -gt 0 ] ||
        refuse "No uniquely mapped unreferenced managed volumes were found for VMID $VMID."

    RVE_DUP="$(awk -F'|' 'seen[$1]++ {print $1; exit}' "$RVE_PLAN")"
    [ -z "$RVE_DUP" ] ||
        refuse "More than one unreferenced managed LV claims disk-$RVE_DUP; recovery ordering is ambiguous."

    info "Recovery plan for VM $VMID ($NAME):"
    cat "$RVE_PLAN"
    [ "$PREFLIGHT" -eq 0 ] || {
        RVE_COMPLETE=1
        return 0
    ...
    journal_begin rebuild-vm-from-existing-disks
    journal_note vmid "$VMID"
    journal_note disks "$RVE_COUNT"

    if [ "$DRYRUN" -eq 1 ]; then
        print_command qm create "$VMID" --name "$NAME" --memory "$MEMORY" --cores "$CORES" --scsihw virtio-scsi-single
        RVE_I=0
        while IFS='|' read -r RVE_NUM RVE_VOLID RVE_PATH; do
            print_command qm set "$VMID" "--scsi$RVE_I" "$RVE_VOLID"
            RVE_I=$((RVE_I + 1))
        done < "$RVE_PLAN"
        print_command qm set "$VMID" --boot order=scsi0
        RVE_COMPLETE=1
        return 0
    fi

    RVE_CREATE_STARTED=1
    if [ "$OVMF" -eq 1 ]; then
        qm create "$VMID" --name "$NAME" --memory "$MEMORY" --cores "$CORES" \
            --scsihw virtio-scsi-single --bios ovmf
    else
        qm create "$VMID" --name "$NAME" --memory "$MEMORY" --cores "$CORES" \
    ...
    RVE_I=0
    while IFS='|' read -r RVE_NUM RVE_VOLID RVE_PATH; do
        qm set "$VMID" "--scsi$RVE_I" "$RVE_VOLID" ||
            verification_failure "Could not attach $RVE_VOLID to scsi$RVE_I."
        [ "$(disk_volid "$VMID" "scsi$RVE_I")" = "$RVE_VOLID" ] ||
            verification_failure "Attached volume did not verify at scsi$RVE_I."
        RVE_I=$((RVE_I + 1))
    done < "$RVE_PLAN"

    qm set "$VMID" --boot order=scsi0 >/dev/null ||
        verification_failure "Could not set recovery VM boot order."
    qm config "$VMID" >/dev/null 2>&1 ||
        verification_failure "New recovery VM config does not parse."

    RVE_COMPLETE=1
    ok "Created VM $VMID with $RVE_COUNT recovered disk(s). Review firmware, machine, CPU, NICs and boot semantics before starting."
}
...
usage() {
    cat <<EOF
rebuild-vm-from-existing-disks.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# rve_config_safe_to_remove
# Confirms an incomplete recovery config references only volumes from this invocation's immutable plan.
# Call: rve_config_safe_to_remove
rve_config_safe_to_remove() {
    rve_c_cfg="/etc/pve/qemu-server/${VMID}.conf"
    [ -f "$rve_c_cfg" ] || return 0
    config_volids "$rve_c_cfg" qemu |
    while IFS= read -r rve_c_volid; do
        [ -n "$rve_c_volid" ] || continue
        awk -F'|' -v v="$rve_c_volid" '$2==v {found=1} END{exit !found}' "$RVE_PLAN" || exit 1
    done
}

# rve_cleanup
# Removes only a provably invocation-owned incomplete guest configuration after an abnormal exit.
# Call: rve_cleanup STATUS
rve_cleanup() {
    rve_cleanup_status="$1"
    trap - 0 HUP INT TERM
    if [ "${RVE_COMPLETE:-0}" -ne 1 ] &&
       [ "${RVE_CREATE_STARTED:-0}" -eq 1 ] &&
       [ "${DRYRUN:-0}" -eq 0 ]; then
        if rve_config_safe_to_remove; then
            rm -f "/etc/pve/qemu-server/${VMID}.conf" ||
                warn "Could not remove incomplete recovery VM config."
        else
            warn "Incomplete recovery config contains an unexpected storage reference; preserving it for operator inspection."
        fi
    fi
    rm -f "${RVE_PLAN:-}"
    [ "$rve_cleanup_status" -eq 0 ] || exit "$rve_cleanup_status"
}
...
parse_arguments() {
    VMID=""; NAME=""; MEMORY=2048; CORES=2; APPLY=0; OVMF=0
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            --memory) [ "$#" -ge 2 ] || usage_error "--memory requires MiB."; MEMORY="$2"; shift 2 ;;
            --cores) [ "$#" -ge 2 ] || usage_error "--cores requires a number."; CORES="$2"; shift 2 ;;
            --ovmf) OVMF=1; shift ;;
            --apply) APPLY=1; shift ;;
    ...
    case "$MEMORY" in *[!0-9]*|"") usage_error "--memory must be a positive integer MiB value." ;; esac
    case "$CORES" in *[!0-9]*|"") usage_error "--cores must be a positive integer." ;; esac
    [ "$MEMORY" -gt 0 ] || usage_error "--memory must be greater than zero."
    [ "$CORES" -gt 0 ] || usage_error "--cores must be greater than zero."
    [ -n "$NAME" ] || NAME="recovered-$VMID"
    single_line_value "$NAME" || usage_error "--name must be a single-line value."
    [ "$APPLY" -eq 1 ] || DRYRUN=1
}

# rve_find_volid VG LV UUID
# Call: rve_find_volid VG LV UUID
# Prints a unique storage:LV mapping whose resolved LVM UUID matches.
rve_find_volid() {
    rve_find_volid_vg="$1"; rve_find_volid_lv="$2"; rve_find_volid_uuid="$3"; rve_find_volid_found=""
    for rve_find_volid_storage in $(pvesm status 2>/dev/null | awk 'NR>1{print $1}'); do
        rve_find_volid_candidate="$rve_find_volid_storage:$rve_find_volid_lv"
        rve_find_volid_path="$(resolve_volid_path "$rve_find_volid_candidate" || :)"; [ -n "$rve_find_volid_path" ] || continue
        rve_find_volid_identity="$(lvm_identity "$rve_find_volid_path" || :)"; [ -n "$rve_find_volid_identity" ] || continue
        rve_find_volid_cuuid="$(printf '%s\n' "$rve_find_volid_identity" | cut -d'|' -f3)"
        [ "$rve_find_volid_cuuid" = "$rve_find_volid_uuid" ] || continue
        [ -z "$rve_find_volid_found" ] || return 1
        rve_find_volid_found="$rve_find_volid_candidate"
    done
    [ -n "$rve_find_volid_found" ] || return 1
    printf '%s\n' "$rve_find_volid_found"
}
