...
############################################################
# resize-vm-disk.sh
# Grow-only Proxmox VM disk resize.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs numfmt awk sed grep date cut
    require_qemu_vm "$VMID"
    valid_disk_slot "$SLOT" || usage_error "Invalid or out-of-range disk slot: $SLOT"
    RVD_VOLID="$(disk_volid "$VMID" "$SLOT")" || refuse "Slot $SLOT is not a storage-backed disk."
    RVD_PATH="$(resolve_volid_path "$RVD_VOLID")" || refuse "Cannot resolve $RVD_VOLID."
    RVD_ID="$(lvm_identity "$RVD_PATH" || :)"
    [ -n "$RVD_ID" ] || refuse "resize-vm-disk.sh currently requires an LVM-backed Proxmox disk."
    RVD_VG="$(printf '%s\n' "$RVD_ID" | cut -d'|' -f1)"
    RVD_LV="$(printf '%s\n' "$RVD_ID" | cut -d'|' -f2)"
    RVD_UUID="$(printf '%s\n' "$RVD_ID" | cut -d'|' -f3)"
    RVD_BEFORE="$(lvs --noheadings --units b --nosuffix -o lv_size "$RVD_VG/$RVD_LV" 2>/dev/null | awk 'NF{printf "%.0f\n",$1;exit}')"
    [ -n "$RVD_BEFORE" ] || refuse "Could not determine current LVM size."

    case "$SIZE" in
        +*)
            RVD_INCREMENT="${SIZE#+}"
            [ -n "$RVD_INCREMENT" ] || usage_error "Empty size increment."
            RVD_INCREMENT_BYTES="$(numfmt --from=iec "$RVD_INCREMENT" 2>/dev/null || :)"
            case "$RVD_INCREMENT_BYTES" in ''|*[!0-9]*) usage_error "Increment must be an IEC size such as +8G." ;; esac
            [ "$RVD_INCREMENT_BYTES" -gt 0 ] || usage_error "Increment must be greater than zero."
            RVD_EXPECTED=$((RVD_BEFORE + RVD_INCREMENT_BYTES))
            ;;
        *)
            RVD_TARGET="$(numfmt --from=iec "$SIZE" 2>/dev/null || :)"
            case "$RVD_TARGET" in ''|*[!0-9]*) usage_error "Size must be an IEC value such as 64G or +8G." ;; esac
            [ "$RVD_TARGET" -gt "$RVD_BEFORE" ] || refuse "Requested absolute size is not larger than current size ($RVD_BEFORE bytes)."
            RVD_EXPECTED="$RVD_TARGET"
            ;;
    esac

    info "VM $VMID $SLOT: $RVD_VOLID"
    info "Current bytes: $RVD_BEFORE"
    info "Requested size: $SIZE"
    info "Minimum expected bytes: $RVD_EXPECTED"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin resize-vm-disk
    journal_note vmid "$VMID"; journal_note slot "$SLOT"; journal_note lv_uuid "$RVD_UUID"; journal_note before_bytes "$RVD_BEFORE"
    run_mutation qm resize "$VMID" "$SLOT" "$SIZE"
    if [ "$DRYRUN" -eq 0 ]; then
        RVD_AFTER_UUID="$(lvs --noheadings -o lv_uuid "$RVD_VG/$RVD_LV" 2>/dev/null | awk '{$1=$1;print;exit}')"
        [ "$RVD_AFTER_UUID" = "$RVD_UUID" ] || verification_failure "Backing LV UUID changed unexpectedly."
        RVD_AFTER="$(lvs --noheadings --units b --nosuffix -o lv_size "$RVD_VG/$RVD_LV" 2>/dev/null | awk 'NF{printf "%.0f\n",$1;exit}')"
        [ -n "$RVD_AFTER" ] || verification_failure "Could not read resulting LVM size."
        [ "$RVD_AFTER" -ge "$RVD_EXPECTED" ] || verification_failure "Backing LV is smaller than requested target ($RVD_AFTER < $RVD_EXPECTED bytes)."
        [ "$RVD_AFTER" -gt "$RVD_BEFORE" ] || verification_failure "Backing LV did not grow."
        journal_note after_bytes "$RVD_AFTER"
        ok "VM disk grew to $RVD_AFTER bytes. Guest partitions/filesystems are unchanged."
    fi
}
...
usage() {
    cat <<EOF
resize-vm-disk.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  resize-vm-disk.sh <VMID> <slot> <new-size|+increment> [dryrun|--preflight]
  resize-vm-disk.sh --vmid VMID --slot SLOT --size SIZE [dryrun|--preflight]

DESCRIPTION
  Grow-only Proxmox disk resize for an LVM-backed QEMU disk. Absolute and
  relative IEC sizes are validated before qm resize. The exact backing LV UUID
  must survive and its resulting byte size must meet the requested target.
  Partitions/filesystems are not changed; grow them separately when appropriate.
EOF
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --vmid) [ "$#" -ge 2 ] || usage_error "--vmid requires a value."; VMID="$2"; shift 2 ;;
      --slot) [ "$#" -ge 2 ] || usage_error "--slot requires a value."; SLOT="$2"; shift 2 ;;
      --size) [ "$#" -ge 2 ] || usage_error "--size requires a value."; SIZE="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; elif [ -z "$SIZE" ]; then SIZE="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$VMID" ] && [ -n "$SLOT" ] && [ -n "$SIZE" ] || usage_error "VMID, slot and size are required."
    case "$SIZE" in -*) refuse "Shrinking is not supported." ;; esac
}
