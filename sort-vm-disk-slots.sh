...
# Sorts device slots by managed backing disk number.
############################################################
...
main() {
    ...
    need_commands qm cp awk sed grep sort mktemp date
    require_qemu_vm "$VMID"; [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    SVDS_CFG="/etc/pve/qemu-server/${VMID}.conf"; ! grep -qE '^\[' "$SVDS_CFG" || refuse "VM snapshots are present."
    SVDS_ITEMS="$(mktemp "${TMPDIR:-/tmp}/lt-slot-items.XXXXXX")"; SVDS_PLAN="$(mktemp "${TMPDIR:-/tmp}/lt-slot-plan.XXXXXX")"; SVDS_OUT="$(mktemp "${TMPDIR:-/tmp}/lt-slot-out.XXXXXX")"
    trap 'rm -f "$SVDS_ITEMS" "$SVDS_PLAN" "$SVDS_OUT"' 0 HUP INT TERM
    : > "$SVDS_ITEMS"; : > "$SVDS_PLAN"
    SVDS_SEQ=0
    while IFS='|' read -r SVDS_SLOT SVDS_VALUE; do
     [ -n "$SVDS_SLOT" ] || continue
     SVDS_VOLID="${SVDS_VALUE%%,*}"; SVDS_LV="${SVDS_VOLID#*:}"
     SVDS_NUM="$(printf '%s\n' "$SVDS_LV" | sed -n 's/^\(vm\|base\)-[0-9][0-9]*-disk-\([0-9][0-9]*\)$/\2/p')"
     if [ -z "$SVDS_NUM" ]; then SVDS_KEY=$((1000000 + SVDS_SEQ)); else SVDS_KEY="$SVDS_NUM"; fi
     printf '%s|%s|%s\n' "$SVDS_KEY" "$SVDS_SEQ" "$SVDS_SLOT" >> "$SVDS_ITEMS"
     SVDS_SEQ=$((SVDS_SEQ + 1))
    done <<EOF
    $(awk -v bus="$BUS" -F': ' '$1 ~ ("^"bus"[0-9]+$") {print $1 "|" $2}' "$SVDS_CFG")
EOF
    SVDS_TARGET=0
    sort -t'|' -k1,1n -k2,2n "$SVDS_ITEMS" | while IFS='|' read -r SVDS_KEY SVDS_SEQ SVDS_OLD; do
     SVDS_NEW="${BUS}${SVDS_TARGET}"
     [ "$SVDS_OLD" = "$SVDS_NEW" ] || printf '%s|%s\n' "$SVDS_OLD" "$SVDS_NEW"
     SVDS_TARGET=$((SVDS_TARGET + 1))
    done > "$SVDS_PLAN"
    SVDS_COUNT="$(awk 'END{print NR+0}' "$SVDS_PLAN")"
    if [ "$SVDS_COUNT" -eq 0 ]; then ok "Slots already sort by managed backing disk number."; return 0; fi
    info "Planned moves:"; cat "$SVDS_PLAN"
    SVDS_BOOT="$(rs_remap_boot_order "$SVDS_CFG" "$SVDS_PLAN")"; rs_rewrite_config "$SVDS_CFG" "$SVDS_PLAN" "$SVDS_OUT" "$SVDS_BOOT"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin sort-vm-disk-slots; journal_note vmid "$VMID"; journal_note bus "$BUS"; journal_note moves "$SVDS_COUNT"
    SVDS_BACKUP="/root/${VMID}.conf.before-slot-sort.$(date +%Y%m%d-%H%M%S)"
    if [ "$DRYRUN" -eq 1 ]; then print_command cp "$SVDS_CFG" "$SVDS_BACKUP"; printf '[DRYRUN] replace config with sorted slot map\n'; return 0; fi
    cp "$SVDS_CFG" "$SVDS_BACKUP"
    SVDS_TX_ACTIVE=1
    trap 'svds_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cat "$SVDS_OUT" > "$SVDS_CFG"
    if ! qm config "$VMID" >/dev/null 2>&1; then
        cat "$SVDS_BACKUP" > "$SVDS_CFG"
        SVDS_TX_ACTIVE=0
        trap - 0 HUP INT TERM
        verification_failure "Proxmox rejected rewritten config; backup restored."
    fi
    SVDS_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    journal_note backup "$SVDS_BACKUP"; ok "Sorted $BUS slots by managed backing disk number."
}
...
usage() {
    cat <<EOF
sort-vm-disk-slots.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --bus) [ "$#" -ge 2 ] || usage_error "--bus requires a value."; BUS="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) [ -z "$VMID" ] || usage_error "Unexpected argument: $1"; VMID="$1"; shift ;;
     esac
    done
    [ -n "$VMID" ] || usage_error "VMID is required."
    case "$BUS" in ide|sata|scsi|virtio) : ;; *) usage_error "Invalid bus: $BUS" ;; esac
}

# rs_rewrite_config CONFIG PLAN OUTPUT BOOTORDER
# Call: rs_rewrite_config CONFIG PLAN OUTPUT BOOTORDER
# Rewrites slot keys in one pass so temporary/cascading slot collisions cannot occur.
rs_rewrite_config() {
    rs_rewrite_config_cfg="$1"; rs_rewrite_config_plan="$2"; rs_rewrite_config_out="$3"; rs_rewrite_config_boot="$4"
    awk -F'|' -v plan="$rs_rewrite_config_plan" -v bootorder="$rs_rewrite_config_boot" '
      BEGIN { while ((getline < plan) > 0) { if ($1!="" && $2!="") map[$1]=$2 } close(plan) }
      {
        line=$0
        key=line
        sub(/:.*/, "", key)
        if (key in map) sub("^" key ":", map[key] ":", line)
        if (bootorder != "" && line ~ /^boot:[[:space:]]/ && line ~ /order=/) sub(/order=[^,]*/, "order=" bootorder, line)
        print line
      }
    ' "$rs_rewrite_config_cfg" > "$rs_rewrite_config_out"
}

# rs_remap_boot_order CONFIG PLAN
# Call: rs_remap_boot_order CONFIG PLAN
# Prints boot order with each original slot independently remapped.
rs_remap_boot_order() {
    rs_remap_boot_order_cfg="$1"; rs_remap_boot_order_plan="$2"
    rs_remap_boot_order_raw="$(sed -n 's/^boot:.*order=\([^,]*\).*/\1/p' "$rs_remap_boot_order_cfg" | head -n1)"
    [ -n "$rs_remap_boot_order_raw" ] || return 0
    rs_remap_boot_order_out=""; rs_remap_boot_order_oldifs=$IFS; IFS=';'
    for rs_remap_boot_order_dev in $rs_remap_boot_order_raw; do
        IFS=$rs_remap_boot_order_oldifs
        rs_remap_boot_order_new="$(awk -F'|' -v d="$rs_remap_boot_order_dev" '$1==d {print $2; found=1; exit} END{if(!found) print d}' "$rs_remap_boot_order_plan")"
        if [ -z "$rs_remap_boot_order_out" ]; then rs_remap_boot_order_out="$rs_remap_boot_order_new"; else rs_remap_boot_order_out="$rs_remap_boot_order_out;$rs_remap_boot_order_new"; fi
        IFS=';'
    done
    IFS=$rs_remap_boot_order_oldifs
    printf '%s\n' "$rs_remap_boot_order_out"
}

# svds_restore_on_exit
# Restores the complete QEMU config if a slot-sort transaction is interrupted.
svds_restore_on_exit() {
    svds_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${SVDS_TX_ACTIVE:-0}" -eq 1 ] && [ -f "${SVDS_BACKUP:-}" ]; then
        cat "$SVDS_BACKUP" > "$SVDS_CFG" || :
        warn "Restored VM config after interrupted disk-slot sort."
    fi
    [ "$svds_restore_status" -eq 0 ] || exit "$svds_restore_status"
    return 0
}
