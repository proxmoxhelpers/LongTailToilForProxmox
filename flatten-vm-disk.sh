...
############################################################
# flatten-vm-disk.sh
# Severs thin-snapshot lineage by verified independent copy.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs lvcreate lvremove lvchange blockdev dd cmp awk sed grep mktemp date cut head find sort

    require_qemu_vm "$VMID"
    [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    FVD_CFG="/etc/pve/qemu-server/${VMID}.conf"
    qemu_config_has_snapshots "$FVD_CFG" &&
        refuse "VM configuration contains snapshot sections; flatten refuses direct config-reference rewrites while snapshots exist."

    FVD_OLD_VALUE="$(disk_value "$VMID" "$SLOT")"
    [ -n "$FVD_OLD_VALUE" ] || refuse "Slot $SLOT is not configured."
    FVD_OLD_VOLID="${FVD_OLD_VALUE%%,*}"
    FVD_STORAGE="${FVD_OLD_VOLID%%:*}"

    FVD_SOURCE="$(resolve_volid_path "$FVD_OLD_VOLID")" ||
        refuse "Cannot resolve $FVD_OLD_VOLID."
    FVD_ID="$(lvm_identity "$FVD_SOURCE")" ||
        refuse "Flatten currently requires an LVM-thin source."
    FVD_VG="$(printf '%s\n' "$FVD_ID" | cut -d'|' -f1)"
    FVD_LV="$(printf '%s\n' "$FVD_ID" | cut -d'|' -f2)"
    FVD_OLD_UUID="$(printf '%s\n' "$FVD_ID" | cut -d'|' -f3)"
    FVD_POOL="$(printf '%s\n' "$FVD_ID" | cut -d'|' -f5)"
    FVD_ORIGIN="$(printf '%s\n' "$FVD_ID" | cut -d'|' -f6)"
    [ -n "$FVD_POOL" ] || refuse "Source is not a thin LV."
    [ -n "$FVD_ORIGIN" ] || refuse "Source has no thin origin and is already independent."

    FVD_FAMILY="$(printf '%s\n' "$FVD_LV" | sed -n 's/^\(vm\|base\)-[0-9][0-9]*-disk-[0-9][0-9]*$/\1/p')"
    [ -n "$FVD_FAMILY" ] || FVD_FAMILY=vm

    FVD_N=0
    while lvs "$FVD_VG/vm-${VMID}-disk-${FVD_N}" >/dev/null 2>&1 ||
          lvs "$FVD_VG/base-${VMID}-disk-${FVD_N}" >/dev/null 2>&1; do
        FVD_N=$((FVD_N + 1))
    done
    FVD_NEW_LV="${FVD_FAMILY}-${VMID}-disk-${FVD_N}"
    FVD_NEW_VOLID="$FVD_STORAGE:$FVD_NEW_LV"
    FVD_NEW_PATH="/dev/$FVD_VG/$FVD_NEW_LV"
    FVD_SIZE="$(lvs --noheadings --units b --nosuffix -o lv_size "$FVD_VG/$FVD_LV" 2>/dev/null |
        awk 'NF {printf "%.0f\n",$1; exit}')"
    [ -n "$FVD_SIZE" ] || refuse "Could not determine source LV size from LVM metadata."

    FVD_SOURCE_WAS_INACTIVE=0
    FVD_SOURCE_ACTIVATED=0
    FVD_NEW_UUID=""
    FVD_DETACHED=0
    FVD_NEW_ATTACHED=0
    FVD_COMPLETE=0
    if [ ! -b "$FVD_SOURCE" ]; then
        FVD_SOURCE_WAS_INACTIVE=1
        info "Source LV is inactive; it will be temporarily activated with -K for the copy window."
    fi

    info "Flatten: $FVD_OLD_VOLID (origin $FVD_ORIGIN) -> $FVD_NEW_VOLID"
    [ "$PREFLIGHT" -eq 0 ] || return 0

    journal_begin flatten-vm-disk
    journal_note vmid "$VMID"
    journal_note slot "$SLOT"
    journal_note old "$FVD_OLD_VOLID"
    journal_note old_uuid "$FVD_OLD_UUID"
    journal_note new "$FVD_NEW_VOLID"
    ...
    if [ "$FVD_SOURCE_WAS_INACTIVE" -eq 1 ]; then
        if [ "$DRYRUN" -eq 1 ]; then
            print_command lvchange -ay -K "$FVD_VG/$FVD_LV"
            FVD_SOURCE_ACTIVATED=1
        else
            run_lvm_filtered lvchange -ay -K "$FVD_VG/$FVD_LV" ||
                die "Could not activate source LV."
            FVD_SOURCE_ACTIVATED=1
            [ -b "$FVD_SOURCE" ] ||
                die "Source LV activation succeeded but no block device appeared: $FVD_SOURCE"
        fi
    fi

    run_mutation lvcreate -V "${FVD_SIZE}B" -T "$FVD_VG/$FVD_POOL" -n "$FVD_NEW_LV"
    if [ "$DRYRUN" -eq 1 ]; then
        print_command dd "if=$FVD_SOURCE" "of=$FVD_NEW_PATH" bs=4M iflag=fullblock conv=sparse,fsync status=progress
        print_command qm set "$VMID" --delete "$SLOT"
        print_command qm set "$VMID" "--$SLOT" "$FVD_NEW_VOLID${FVD_OLD_VALUE#"$FVD_OLD_VOLID"}"
        if [ "$FVD_SOURCE_ACTIVATED" -eq 1 ]; then
            print_command lvchange -an "$FVD_VG/$FVD_LV"
            FVD_SOURCE_ACTIVATED=0
        fi
        FVD_COMPLETE=1
        return 0
    ...
    FVD_NEW_UUID="$(lvs --noheadings -o lv_uuid "$FVD_VG/$FVD_NEW_LV" 2>/dev/null |
        awk 'NF {$1=$1; print; exit}')"
    [ -n "$FVD_NEW_UUID" ] || verification_failure "Could not capture UUID of newly created flattened LV."
    journal_note new_uuid "$FVD_NEW_UUID"

    dd if="$FVD_SOURCE" of="$FVD_NEW_PATH" bs=4M iflag=fullblock conv=sparse,fsync status=progress
    cmp -n "$FVD_SIZE" "$FVD_SOURCE" "$FVD_NEW_PATH"

    if [ "$FVD_SOURCE_ACTIVATED" -eq 1 ]; then
        lvchange -an "$FVD_VG/$FVD_LV" ||
            verification_failure "Copy verified, but source LV could not be restored to inactive state."
        FVD_SOURCE_ACTIVATED=0
    fi

    FVD_NEW_ID="$(lvm_identity "$FVD_NEW_PATH")"
    [ "$(printf '%s\n' "$FVD_NEW_ID" | cut -d'|' -f3)" = "$FVD_NEW_UUID" ] ||
        verification_failure "Flattened LV UUID changed unexpectedly."
    [ -z "$(printf '%s\n' "$FVD_NEW_ID" | cut -d'|' -f6)" ] ||
        verification_failure "New LV unexpectedly has an origin."
    FVD_RESOLVED="$(resolve_volid_path "$FVD_NEW_VOLID" || :)"
    [ -n "$FVD_RESOLVED" ] || verification_failure "Proxmox storage cannot resolve newly created LV."
    FVD_RESOLVED_ID="$(lvm_identity "$FVD_RESOLVED" || :)"
    [ -n "$FVD_RESOLVED_ID" ] &&
    [ "$(printf '%s\n' "$FVD_RESOLVED_ID" | cut -d'|' -f3)" = "$FVD_NEW_UUID" ] ||
        verification_failure "Proxmox storage does not resolve the new volume to the LV created by this invocation."

    qm set "$VMID" --delete "$SLOT"
    FVD_DETACHED=1
    FVD_UNUSED="$(qm config "$VMID" | awk -v v="$FVD_OLD_VOLID" -F': ' '
        $1 ~ /^unused[0-9]+$/ {
            split($2, parts, ",")
            if (parts[1] == v) { print $1; exit }
        }')"
    [ -n "$FVD_UNUSED" ] ||
        verification_failure "Old disk was detached but no preserving unusedN reference was found."

    FVD_NEW_VALUE="$FVD_NEW_VOLID${FVD_OLD_VALUE#"$FVD_OLD_VOLID"}"
    qm set "$VMID" "--$SLOT" "$FVD_NEW_VALUE" ||
        verification_failure "Could not attach flattened disk; rollback will restore the old slot."
    FVD_NEW_ATTACHED=1

    [ "$(disk_volid "$VMID" "$SLOT")" = "$FVD_NEW_VOLID" ] ||
        verification_failure "Final slot does not reference flattened LV."
    [ "$(volid_lv_uuid "$(disk_volid "$VMID" "$SLOT")" 2>/dev/null || :)" = "$FVD_NEW_UUID" ] ||
        verification_failure "Final slot does not resolve to the flattened LV UUID."

    FVD_COMPLETE=1
    FVD_DETACHED=0
    ok "Flattened disk attached at $SLOT; original linked volume remains preserved as $FVD_UNUSED."
}
...
usage() {
    cat <<EOF
flatten-vm-disk.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    [ -n "$VMID" ] && [ -n "$SLOT" ] || usage_error "VMID and slot are required."
    case "$VMID" in *[!0-9]*|"") usage_error "VMID must be numeric." ;; esac
    valid_disk_slot "$SLOT" || usage_error "Slot must be a valid QEMU disk slot (ide0..3, sata0..5, scsi0..30, virtio0..15)."
}

# fvd_remove_unused VMID VOLID
# Call: fvd_remove_unused VMID VOLID
# Removes only the exact unusedN line from config; storage is never freed.
# fvd_remove_unused VMID VOLID
# Removes exact config-only unusedN references to VOLID without freeing storage.
fvd_remove_unused() {
    fvd_remove_unused_vmid="$1"
    fvd_remove_unused_volid="$2"
    fvd_remove_unused_cfg="/etc/pve/qemu-server/${fvd_remove_unused_vmid}.conf"
    fvd_remove_unused_tmp="$(mktemp "${TMPDIR:-/tmp}/lt-flatten-unused.XXXXXX")" || return 1
    awk -v v="$fvd_remove_unused_volid" -F': ' '
        $1 ~ /^unused[0-9]+$/ {
            split($2, parts, ",")
            if (parts[1] == v) next
        }
        { print }
    ' "$fvd_remove_unused_cfg" > "$fvd_remove_unused_tmp" || {
        rm -f "$fvd_remove_unused_tmp"
        return 1
    }
    cat "$fvd_remove_unused_tmp" > "$fvd_remove_unused_cfg" || {
        rm -f "$fvd_remove_unused_tmp"
        return 1
    }
    rm -f "$fvd_remove_unused_tmp"
...
# fvd_restore_old
# Call: fvd_restore_old
# Restores the original disk reference/LV identity during flatten rollback.
# fvd_restore_old
# Restores the exact original slot value and removes config-only unused entries.
fvd_restore_old() {
    # If the replacement currently occupies the slot, detach it first so it is
    # not silently overwritten. Its generated unusedN entry is removed only
    # config-side; storage cleanup is UUID-gated separately.
    if [ "$(disk_volid "$VMID" "$SLOT" 2>/dev/null || :)" = "${FVD_NEW_VOLID:-}" ]; then
        qm set "$VMID" --delete "$SLOT" >/dev/null 2>&1 || {
            warn "Could not detach replacement while restoring original slot."
            return 1
        }
    fi

    fvd_remove_unused "$VMID" "${FVD_NEW_VOLID:-}" || :
    if [ "$(disk_value "$VMID" "$SLOT" 2>/dev/null || :)" != "$FVD_OLD_VALUE" ]; then
        qm set "$VMID" "--$SLOT" "$FVD_OLD_VALUE" >/dev/null 2>&1 || {
            warn "Could not restore original slot automatically."
            return 1
        }
    fi
    fvd_remove_unused "$VMID" "$FVD_OLD_VOLID" || :
    [ "$(disk_value "$VMID" "$SLOT" 2>/dev/null || :)" = "$FVD_OLD_VALUE" ] || {
        warn "Original slot value was not restored exactly."
        return 1
    }
    FVD_DETACHED=0
    FVD_NEW_ATTACHED=0
...
# fvd_cleanup STATUS
# Call: fvd_cleanup STATUS
# Runs flatten rollback/cleanup for transaction-owned staged state on exit.
# fvd_cleanup STATUS
# Restores source activation and original VM slot on any incomplete transaction.
# The new LV is removed only when its captured UUID still owns the name and no
# Proxmox guest physically references it.
fvd_cleanup() {
    fvd_cleanup_status="$1"
    trap - 0 HUP INT TERM

    if [ "${FVD_SOURCE_ACTIVATED:-0}" -eq 1 ]; then
        lvchange -an "$FVD_VG/$FVD_LV" >/dev/null 2>&1 ||
            warn "Could not restore source LV to inactive state."
        FVD_SOURCE_ACTIVATED=0
    fi

    if [ "${FVD_COMPLETE:-0}" -ne 1 ] && [ "${FVD_DETACHED:-0}" -eq 1 ]; then
        fvd_restore_old || warn "Automatic rollback could not fully restore the original VM slot."
    fi

    if [ "${FVD_COMPLETE:-0}" -ne 1 ] && [ -n "${FVD_NEW_UUID:-}" ]; then
        if lvs "$FVD_VG/$FVD_NEW_LV" >/dev/null 2>&1; then
            fvd_cleanup_uuid="$(lvs --noheadings -o lv_uuid "$FVD_VG/$FVD_NEW_LV" 2>/dev/null | awk 'NF {$1=$1; print; exit}')"
            if [ "$fvd_cleanup_uuid" = "$FVD_NEW_UUID" ]; then
                if [ "$(lv_uuid_reference_count "$FVD_NEW_UUID")" -eq 0 ]; then
                    lvremove -y "$FVD_VG/$FVD_NEW_LV" >/dev/null 2>&1 ||
                        warn "Could not remove incomplete flattened LV."
                else
                    warn "Incomplete flattened LV is still referenced; preserving it."
                fi
            else
                warn "Flattened LV name no longer has the UUID created by this invocation; preserving it."
            fi
        fi
    fi

    [ "$fvd_cleanup_status" -eq 0 ] || exit "$fvd_cleanup_status"
}
