...
############################################################
# compare-vm-disks.sh
# Read-only structural/content comparison of two block devices.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs lvchange blockdev dd sha256sum cmp awk sed grep mktemp cut
    CD_ACTIVATIONS="$(mktemp "${TMPDIR:-/var/tmp}/lt-compare-activations.XXXXXX")"
    : > "$CD_ACTIVATIONS"
    trap 'cd_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    ...
    CD_A_RAW="$(cd_resolve_selector "$DISK_A")" || refuse "Cannot resolve first disk selector: $DISK_A"
    CD_B_RAW="$(cd_resolve_selector "$DISK_B")" || refuse "Cannot resolve second disk selector: $DISK_B"
    if prepare_readable_path "$CD_A_RAW" "$CD_ACTIVATIONS"; then CD_A="$READABLE_PATH"; else
        CD_RC=$?; [ "$CD_RC" -ne 2 ] || refuse "First LV is inactive; dry-run/preflight will not change activation state."
        refuse "First selected device is not readable: $CD_A_RAW"
    fi
    if prepare_readable_path "$CD_B_RAW" "$CD_ACTIVATIONS"; then CD_B="$READABLE_PATH"; else
        CD_RC=$?; [ "$CD_RC" -ne 2 ] || refuse "Second LV is inactive; dry-run/preflight will not change activation state."
        refuse "Second selected device is not readable: $CD_B_RAW"
    fi

    CD_SIZE_A="$(blockdev --getsize64 "$CD_A")"; CD_SIZE_B="$(blockdev --getsize64 "$CD_B")"
    CD_HEAD_A="$(dd if="$CD_A" bs=1M count=4 status=none 2>/dev/null | sha256sum | awk '{print $1}')"
    CD_HEAD_B="$(dd if="$CD_B" bs=1M count=4 status=none 2>/dev/null | sha256sum | awk '{print $1}')"
    printf 'A: %s\nB: %s\n' "$CD_A" "$CD_B"
    printf 'Size: %s / %s %s\n' "$CD_SIZE_A" "$CD_SIZE_B" "$( [ "$CD_SIZE_A" = "$CD_SIZE_B" ] && printf MATCH || printf DIFFER )"
    printf 'First 4 MiB SHA-256: %s / %s %s\n' "$CD_HEAD_A" "$CD_HEAD_B" "$( [ "$CD_HEAD_A" = "$CD_HEAD_B" ] && printf MATCH || printf DIFFER )"
    CD_DIFF=0
    [ "$CD_SIZE_A" = "$CD_SIZE_B" ] || CD_DIFF=1
    [ "$CD_HEAD_A" = "$CD_HEAD_B" ] || CD_DIFF=1
    if [ "$FULL" -eq 1 ]; then
        [ "$PREFLIGHT" -eq 0 ] || { ok "Preflight passed; full byte comparison not started."; return 0; }
        if cmp "$CD_A" "$CD_B" >/dev/null 2>&1; then ok "Full byte comparison matches."
        else CD_DIFF=1; warn "Full byte comparison differs."; fi
    fi
    [ "$CD_DIFF" -eq 0 ] || exit 4
}
...
usage() {
    cat <<EOF
compare-vm-disks.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            *) if [ -z "$DISK_A" ]; then DISK_A="$1"; elif [ -z "$DISK_B" ]; then DISK_B="$1"; else usage_error "Too many disk selectors."; fi; shift ;;
        esac
    done
    [ -n "$DISK_A" ] && [ -n "$DISK_B" ] || usage_error "Two disk selectors are required."
}

# cd_resolve_selector SELECTOR
# Call: cd_resolve_selector SELECTOR
# Accepts /dev/VG/LV or VMID:SLOT.
cd_resolve_selector() {
    cd_resolve_selector_selector="$1"
    case "$cd_resolve_selector_selector" in
        /*) [ -e "$cd_resolve_selector_selector" ] || lvs "$cd_resolve_selector_selector" >/dev/null 2>&1 || return 1; printf '%s\n' "$cd_resolve_selector_selector" ;;
        *:*)
            cd_resolve_selector_vmid="${cd_resolve_selector_selector%%:*}"
            cd_resolve_selector_slot="${cd_resolve_selector_selector#*:}"
            require_qemu_vm "$cd_resolve_selector_vmid"
            valid_disk_slot "$cd_resolve_selector_slot" || usage_error "Invalid or out-of-range disk slot: $cd_resolve_selector_slot"
            cd_resolve_selector_volid="$(disk_volid "$cd_resolve_selector_vmid" "$cd_resolve_selector_slot")" || return 1
            resolve_volid_path "$cd_resolve_selector_volid"
            ;;
        *) return 1 ;;
    esac
}
...
# cd_cleanup STATUS
# Restores temporary inactive-LV activations used for read-only comparison.
# Call: cd_cleanup STATUS
cd_cleanup() {
    cd_cleanup_status="$1"
    trap - 0 HUP INT TERM
    restore_tracked_activations "${CD_ACTIVATIONS:-}"
    rm -f "${CD_ACTIVATIONS:-}" 2>/dev/null || :
    [ "$cd_cleanup_status" -eq 0 ] || exit "$cd_cleanup_status"
    return 0
}
