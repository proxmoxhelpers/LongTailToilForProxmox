...
[Back to helper list](../README.md) · [View script](../remove-vm-disk-reference-only.sh) · [Raw usage](./remove-vm-disk-reference-only.sh.usage)
...
```sh
./remove-vm-disk-reference-only.sh --help
```

<!-- BEGIN LIVE HELP -->
```text
remove-vm-disk-reference-only.sh 3.7.1 (project 3.7.1)
...
The same output is stored verbatim in [`remove-vm-disk-reference-only.sh.usage`](./remove-vm-disk-reference-only.sh.usage) and is checked byte-for-byte by the static suite.
