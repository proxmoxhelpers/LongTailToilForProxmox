...
[Back to helper list](../README.md) · [View script](../plan-vm-storage-move.sh) · [Raw usage](./plan-vm-storage-move.sh.usage)
...
```sh
./plan-vm-storage-move.sh --help
```

<!-- BEGIN LIVE HELP -->
```text
plan-vm-storage-move.sh 3.7.1 (project 3.7.1)
...
The same output is stored verbatim in [`plan-vm-storage-move.sh.usage`](./plan-vm-storage-move.sh.usage) and is checked byte-for-byte by the static suite.
