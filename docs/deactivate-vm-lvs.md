...
[Back to helper list](../README.md) · [View script](../deactivate-vm-lvs.sh) · [Raw usage](./deactivate-vm-lvs.sh.usage)
...
```sh
./deactivate-vm-lvs.sh --help
```

<!-- BEGIN LIVE HELP -->
```text
deactivate-vm-lvs.sh 3.7.1 (project 3.7.1)
...
The same output is stored verbatim in [`deactivate-vm-lvs.sh.usage`](./deactivate-vm-lvs.sh.usage) and is checked byte-for-byte by the static suite.
