...
[Back to helper list](../README.md) · [View script](../for-each-vm.sh) · [Raw usage](./for-each-vm.sh.usage)
...
<!-- BEGIN LIVE HELP -->
```text
for-each-vm.sh 3.7.1 (project 3.7.1)
...
The same output is stored verbatim in [`for-each-vm.sh.usage`](./for-each-vm.sh.usage) and is checked byte-for-byte by the static suite.
