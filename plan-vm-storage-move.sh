...
############################################################
# plan-vm-storage-move.sh
# Read-only VM storage migration capacity/topology planner.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs blockdev awk sed grep cut head find sort
    require_qemu_vm "$VMID"

    PVSM_STATUS="$(pvesm status 2>/dev/null | awk -v s="$DEST" '$1==s {print; found=1} END{if(!found)exit 1}')" ||
        refuse "Destination storage is unavailable: $DEST"
    printf 'Destination: %s\n' "$PVSM_STATUS"

    PVSM_TOTAL=0; PVSM_COUNT=0; PVSM_WARN=0
    printf '%-10s %-38s %-14s %-14s %-18s %s\n' SLOT SOURCE SIZE POOL ORIGIN NOTES
    for PVSM_SLOT in $(qm config "$VMID" | awk -F: '/^(ide|sata|scsi|virtio)[0-9]+:/ {print $1}'); do
        [ -z "$SLOT" ] || [ "$PVSM_SLOT" = "$SLOT" ] || continue
        valid_disk_slot "$PVSM_SLOT" || {
            warn "Unexpected disk slot in VM config: $PVSM_SLOT"
            PVSM_WARN=$((PVSM_WARN + 1))
            continue
        }
        PVSM_VOLID="$(disk_volid "$VMID" "$PVSM_SLOT" || :)"
        [ -n "$PVSM_VOLID" ] || continue
        PVSM_PATH="$(resolve_volid_path "$PVSM_VOLID" || :)"
        [ -n "$PVSM_PATH" ] || {
            warn "$PVSM_SLOT cannot be resolved."
            PVSM_WARN=$((PVSM_WARN + 1))
            continue
        }

        PVSM_ID="$(lvm_identity "$PVSM_PATH" || :)"
        PVSM_POOL="-"; PVSM_ORIGIN="-"; PVSM_NOTE="-"
        if [ -n "$PVSM_ID" ]; then
            # LVM metadata remains authoritative even when activation-skip means
            # no block device node currently exists.
            PVSM_SIZE="$(printf '%s\n' "$PVSM_ID" | cut -d'|' -f4)"
            PVSM_POOL="$(printf '%s\n' "$PVSM_ID" | cut -d'|' -f5)"
            PVSM_ORIGIN="$(printf '%s\n' "$PVSM_ID" | cut -d'|' -f6)"
            [ -z "$PVSM_ORIGIN" ] || PVSM_NOTE="snapshot/clone lineage"
        else
            PVSM_SIZE="$(blockdev --getsize64 "$PVSM_PATH" 2>/dev/null || printf 0)"
        fi
        case "$PVSM_SIZE" in *[!0-9]*|"") PVSM_SIZE=0 ;; esac
        PVSM_TOTAL=$((PVSM_TOTAL + PVSM_SIZE))
        PVSM_COUNT=$((PVSM_COUNT + 1))
        printf '%-10s %-38s %-14s %-14s %-18s %s\n' \
            "$PVSM_SLOT" "$PVSM_VOLID" "$PVSM_SIZE" "${PVSM_POOL:--}" "${PVSM_ORIGIN:--}" "$PVSM_NOTE"
    done

    printf 'Disks: %s\nVirtual bytes to move: %s\n' "$PVSM_COUNT" "$PVSM_TOTAL"
    [ "$PVSM_COUNT" -gt 0 ] || exit 4
    [ "$PVSM_WARN" -eq 0 ] || exit 4
}
...
usage() {
    cat <<EOF
plan-vm-storage-move.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  plan-vm-storage-move.sh <VMID> <destination-storage> [--slot SLOT]

DESCRIPTION
  Read-only migration planner. Shows every selected disk, virtual byte count,
  thin-pool/origin lineage and destination storage status before any move.
  LVM sizes come from metadata, so inactive activation-skip/template LVs can be
  planned without being activated. --slot requires an exact QEMU disk slot.
EOF
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            *)
                ...
                else usage_error "Too many arguments."
                fi
                shift
                ;;
        esac
    ...
    [ -z "$SLOT" ] || valid_disk_slot "$SLOT" || usage_error "--slot must be an exact QEMU disk slot."
    single_line_value "$DEST" || usage_error "Destination storage must be a single-line ID."
