...
############################################################
# verify-vm-storage-consistency.sh
# Read-only cross-check of guest configuration, Proxmox storage mapping and LVM identity.
############################################################
...
main() {
    ...
    need_commands qm pct pvesm lvs find grep sed awk sort basename
    VSC_ISSUES=0
    if [ "$OUTPUT_FORMAT" = text ]; then
        printf '%-8s %-10s %-42s %-16s %s\n' VMID SLOT VOLUME PROBLEM DETAIL
    ...
    if [ -n "$TARGET" ]; then
        VSC_KIND="$(guest_kind "$TARGET")" || refuse "Guest $TARGET does not exist on this node."
        VSC_CFG="$(guest_config_path "$TARGET")"
        vsc_check_config "$VSC_CFG" "$VSC_KIND" "$TARGET"
    else
        for VSC_CFG in /etc/pve/qemu-server/*.conf; do
            [ -f "$VSC_CFG" ] || continue
            VSC_VMID="$(basename "$VSC_CFG" .conf)"
            vsc_check_config "$VSC_CFG" qemu "$VSC_VMID"
        done
        for VSC_CFG in /etc/pve/lxc/*.conf; do
            [ -f "$VSC_CFG" ] || continue
            VSC_VMID="$(basename "$VSC_CFG" .conf)"
            vsc_check_config "$VSC_CFG" lxc "$VSC_VMID"
        done
    fi
    ...
    if [ "$OUTPUT_FORMAT" = json ]; then
        ...
        if [ "$VSC_ISSUES" -eq 0 ]; then ok "Storage consistency checks passed."
        else warn "$VSC_ISSUES storage consistency issue(s) detected."; fi
    fi
    [ "$VSC_ISSUES" -eq 0 ] || exit 4
}
...
usage() {
    cat <<EOF
verify-vm-storage-consistency.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  verify-vm-storage-consistency.sh [VMID] [--json|--tsv] [--quiet] [--no-color]

DESCRIPTION
  Performs a read-only consistency audit of QEMU and LXC storage references.
  Checks pvesm resolution, backing-path existence, managed LV VMID identity,
  and configured multi-guest references. unusedN references are inspected but
  are not treated as writable shared-disk conflicts.

EXIT STATUS
  0  consistent
  2  command-line usage error
  3  requested guest is unavailable/refused
  4  one or more consistency findings were detected
EOF
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            *) [ -z "$TARGET" ] || usage_error "Only one optional VMID is accepted."; TARGET="$1"; shift ;;
        esac
    done
    if [ -n "$TARGET" ]; then case "$TARGET" in *[!0-9]*|'') usage_error "VMID must be numeric." ;; esac; fi
}
...
# vsc_emit_issue VMID SLOT VOLID PROBLEM DETAIL
# Call: vsc_emit_issue VMID SLOT VOLID PROBLEM DETAIL
# Records and prints one storage-consistency finding without mutating guest state.
vsc_emit_issue() {
    vsc_emit_issue_vmid="$1"; vsc_emit_issue_slot="$2"; vsc_emit_issue_volid="$3"
    vsc_emit_issue_problem="$4"; vsc_emit_issue_detail="$5"
    VSC_ISSUES=$((VSC_ISSUES + 1))
    case "$OUTPUT_FORMAT" in
        json)
            [ "$VSC_JSON_FIRST" -eq 1 ] || printf ','
            VSC_JSON_FIRST=0
            printf '{"vmid":"%s","slot":"%s","volume":"%s","problem":"%s","detail":"%s"}' \
                "$(json_escape "$vsc_emit_issue_vmid")" "$(json_escape "$vsc_emit_issue_slot")" \
                "$(json_escape "$vsc_emit_issue_volid")" "$(json_escape "$vsc_emit_issue_problem")" \
                "$(json_escape "$vsc_emit_issue_detail")"
            ;;
        tsv) printf '%s\t%s\t%s\t%s\t%s\n' "$vsc_emit_issue_vmid" "$vsc_emit_issue_slot" "$vsc_emit_issue_volid" "$vsc_emit_issue_problem" "$vsc_emit_issue_detail" ;;
        *) printf '%-8s %-10s %-42s %-16s %s\n' "$vsc_emit_issue_vmid" "$vsc_emit_issue_slot" "$vsc_emit_issue_volid" "$vsc_emit_issue_problem" "$vsc_emit_issue_detail" ;;
    esac
}

# vsc_check_config CONFIG KIND VMID
# Call: vsc_check_config CONFIG KIND VMID
# Checks one guest configuration reference against storage and LVM metadata.
vsc_check_config() {
    vsc_check_config_cfg="$1"; vsc_check_config_kind="$2"; vsc_check_config_vmid="$3"
    while IFS='|' read -r vsc_check_config_slot vsc_check_config_value; do
        [ -n "$vsc_check_config_slot" ] || continue
        vsc_check_config_volid="${vsc_check_config_value%%,*}"
        case "$vsc_check_config_volid" in *:*) : ;; *) continue ;; esac
        vsc_check_volume "$vsc_check_config_vmid" "$vsc_check_config_slot" "$vsc_check_config_volid"
    done <<EOF
$(awk -F': ' '
    /^(ide|sata|scsi|virtio)[0-9]+: / || /^(efidisk|tpmstate)[0-9]+: / || /^unused[0-9]+: / || /^rootfs: / || /^mp[0-9]+: / {
        key=$1
        sub(/^[^:]+:[[:space:]]*/, "", $0)
        print key "|" $0
    }' "$vsc_check_config_cfg")
EOF
}

# vsc_check_volume VMID SLOT VOLID
# Call: vsc_check_volume VMID SLOT VOLID
# Checks one LVM volume for ownership/reference consistency against local guest configs.
vsc_check_volume() {
    vsc_check_volume_vmid="$1"; vsc_check_volume_slot="$2"; vsc_check_volume_volid="$3"
    vsc_check_volume_path="$(resolve_volid_path "$vsc_check_volume_volid" || :)"
    if [ -z "$vsc_check_volume_path" ]; then
        vsc_emit_issue "$vsc_check_volume_vmid" "$vsc_check_volume_slot" "$vsc_check_volume_volid" "UNRESOLVED" "pvesm cannot resolve volume"
        return 0
    fi
    if [ ! -e "$vsc_check_volume_path" ] && ! lvs "$vsc_check_volume_path" >/dev/null 2>&1; then
        vsc_emit_issue "$vsc_check_volume_vmid" "$vsc_check_volume_slot" "$vsc_check_volume_volid" "MISSING_PATH" "$vsc_check_volume_path"
        return 0
    fi

    vsc_check_volume_identity="$(lvm_identity "$vsc_check_volume_path" || :)"
    if [ -n "$vsc_check_volume_identity" ]; then
        vsc_check_volume_lv="$(printf '%s\n' "$vsc_check_volume_identity" | cut -d'|' -f2)"
        case "$vsc_check_volume_lv" in
            vm-[0-9]*-disk-[0-9]*|base-[0-9]*-disk-[0-9]*)
                vsc_check_volume_embedded="$(printf '%s\n' "$vsc_check_volume_lv" | sed -n 's/^\(vm\|base\)-\([0-9][0-9]*\)-disk-[0-9][0-9]*$/\2/p')"
                if [ -n "$vsc_check_volume_embedded" ] && [ "$vsc_check_volume_embedded" != "$vsc_check_volume_vmid" ]; then
                    vsc_emit_issue "$vsc_check_volume_vmid" "$vsc_check_volume_slot" "$vsc_check_volume_volid" "VMID_MISMATCH" "LV embeds VMID $vsc_check_volume_embedded"
                fi
                ;;
        esac
    fi

    case "$vsc_check_volume_slot" in unused*) return 0 ;; esac
    vsc_check_volume_refs="$(volid_reference_count "$vsc_check_volume_volid")"
    if [ "$vsc_check_volume_refs" -gt 1 ]; then
        vsc_emit_issue "$vsc_check_volume_vmid" "$vsc_check_volume_slot" "$vsc_check_volume_volid" "SHARED_REF" "$vsc_check_volume_refs guest configs reference this volume"
    fi
}
