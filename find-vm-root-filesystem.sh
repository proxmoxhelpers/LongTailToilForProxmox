...
# Read-only Linux root-filesystem candidate finder.
############################################################
...
main() {
    ...
    FVR_ACTIVATIONS="$(mktemp "${TMPDIR:-/var/tmp}/lt-root-candidates-activations.XXXXXX")"; : > "$FVR_ACTIVATIONS"
    trap 'fvr_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    FVR_FOUND=0
    for FVR_SLOT in $(qm config "$VMID" | awk -F: '/^(ide|sata|scsi|virtio)[0-9]+:/ {print $1}'); do
        FVR_VOLID="$(disk_volid "$VMID" "$FVR_SLOT" || :)"; [ -n "$FVR_VOLID" ] || continue
        FVR_RAW="$(resolve_volid_path "$FVR_VOLID" || :)"; [ -n "$FVR_RAW" ] || continue
        if prepare_readable_path "$FVR_RAW" "$FVR_ACTIVATIONS"; then FVR_PATH="$READABLE_PATH"; else
            FVR_RC=$?; [ "$FVR_RC" -ne 2 ] || refuse "$FVR_SLOT is inactive; dry-run/preflight will not activate it."
            refuse "Cannot read $FVR_SLOT backing device: $FVR_RAW"
        fi
        FVR_SS="$(blockdev --getss "$FVR_PATH")"
        while read -r FVR_NR FVR_START FVR_SECTORS FVR_TYPE; do
            [ -n "${FVR_NR:-}" ] || continue
            FVR_OFF=$((FVR_START * FVR_SS)); FVR_LEN=$((FVR_SECTORS * FVR_SS))
            FVR_FS="$(blkid -p -o value -s TYPE -O "$FVR_OFF" -S "$FVR_LEN" "$FVR_PATH" 2>/dev/null || :)"
            case "$FVR_FS" in
                ext2|ext3|ext4|xfs|btrfs)
                    printf '%s part%s %s offset=%s bytes=%s\n' "$FVR_SLOT" "$FVR_NR" "$FVR_FS" "$FVR_OFF" "$FVR_LEN"
                    FVR_FOUND=$((FVR_FOUND + 1))
                    ;;
            esac
        done <<EOF
$(partx --show --noheadings -o NR,START,SECTORS,TYPE "$FVR_PATH" 2>/dev/null || :)
EOF
        FVR_WHOLE="$(blkid -p -o value -s TYPE "$FVR_PATH" 2>/dev/null || :)"
        case "$FVR_WHOLE" in
            ext2|ext3|ext4|xfs|btrfs)
                printf '%s whole %s offset=0 bytes=%s\n' "$FVR_SLOT" "$FVR_WHOLE" "$(blockdev --getsize64 "$FVR_PATH")"
                FVR_FOUND=$((FVR_FOUND + 1))
                ;;
        esac
    done
    [ "$FVR_FOUND" -gt 0 ] || exit 4
    warn "Candidates are filesystem-signature based; this command does not mount them to prove which contains /."
}
...
usage() {
    cat <<EOF
find-vm-root-filesystem.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# fvr_cleanup STATUS
# Restores temporary inactive-LV activations used for signature inspection.
# Call: fvr_cleanup STATUS
fvr_cleanup() {
    fvr_cleanup_status="$1"
    trap - 0 HUP INT TERM
    restore_tracked_activations "${FVR_ACTIVATIONS:-}"
    rm -f "${FVR_ACTIVATIONS:-}" 2>/dev/null || :
    [ "$fvr_cleanup_status" -eq 0 ] || exit "$fvr_cleanup_status"
    return 0
}
