...
# Exports one guest filesystem as a portable file-tree archive.
############################################################
...
main() {
    ...
    need_commands qm pvesm lvs lvchange partx blkid kpartx mount umount findmnt tar mktemp mkdir awk sed grep sha256sum dirname basename mv ln rm readlink cut
    ...
    [ "$(guest_status "$VMID" qemu)" = "stopped" ] ||
        refuse "VM $VMID must be stopped for filesystem export."
    if [ -n "$PART" ]; then
        case "$PART" in ''|*[!0-9]*) usage_error "--partition must be a positive integer." ;; esac
        [ "$PART" -gt 0 ] || usage_error "--partition must be greater than zero."
    fi
    single_line_value "$OUT" || usage_error "Output path must be a single line."
    [ ! -d "$OUT" ] || refuse "Output path is a directory: $OUT"
    [ ! -e "$OUT" ] || [ "$FORCE" -eq 1 ] || refuse "Output already exists; use --force: $OUT"
    EVF_OUT_DIR="$(dirname "$OUT")"
    EVF_OUT_BASE="$(basename "$OUT")"
    [ -d "$EVF_OUT_DIR" ] || refuse "Output directory does not exist: $EVF_OUT_DIR"

    EVF_VOLID="$(disk_volid "$VMID" "$SLOT")" || refuse "Slot is not a storage-backed disk."
    EVF_RAW_PATH="$(resolve_volid_path "$EVF_VOLID")" || refuse "Cannot resolve $EVF_VOLID."
    EVF_ACTIVATIONS="$(mktemp "${TMPDIR:-/var/tmp}/lt-fs-export-activations.XXXXXX")"; : > "$EVF_ACTIVATIONS"
    EVF_MAPPER_CREATED=0; EVF_MOUNTED=0
    EVF_TMP="$(mktemp -d "${TMPDIR:-/var/tmp}/lt-fs-export.XXXXXX")"
    EVF_MNT="$EVF_TMP/mnt"; mkdir -p "$EVF_MNT"
    EVF_OUT_TMP=""
    trap 'evf_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    ...
    if prepare_readable_path "$EVF_RAW_PATH" "$EVF_ACTIVATIONS"; then
        EVF_PATH="$READABLE_PATH"
    else
        EVF_READ_RC=$?
        [ "$EVF_READ_RC" -ne 2 ] ||
            refuse "Selected LV is inactive; dry-run/preflight will not change activation state."
        refuse "Selected backing device is not readable: $EVF_RAW_PATH"
    fi
    EVF_PARTS="$(partx --show --noheadings -o NR "$EVF_PATH" 2>/dev/null | awk 'NF{print $1}')"

    if [ -n "$EVF_PARTS" ]; then
        [ -n "$PART" ] || refuse "Disk is partitioned; specify --partition N."
        printf '%s\n' "$EVF_PARTS" | grep -qx "$PART" || refuse "Partition $PART does not exist."
        EVF_MAP="$(kpartx -l "$EVF_PATH" 2>/dev/null | awk -v p="$PART" '$1 ~ ("p"p"$") {print $1; exit}')"
        [ -n "$EVF_MAP" ] || refuse "Cannot derive mapper name for partition $PART."
        EVF_DEV="/dev/mapper/$EVF_MAP"
        [ ! -e "$EVF_DEV" ] && [ ! -L "$EVF_DEV" ] ||
            refuse "Partition mapper already exists before this invocation; ownership is ambiguous: $EVF_DEV"
    else
        [ -z "$PART" ] || refuse "--partition was specified but the disk has no partition table."
        EVF_DEV="$EVF_PATH"
    fi
    info "Filesystem source: $EVF_DEV"
    [ "$PREFLIGHT" -eq 0 ] || return 0

    if [ -n "$EVF_PARTS" ]; then
        if [ "$DRYRUN" -eq 1 ]; then
            print_command kpartx -av "$EVF_PATH"
        else
            kpartx -av "$EVF_PATH"
            EVF_MAPPER_CREATED=1
            [ -b "$EVF_DEV" ] || verification_failure "Expected mapper device did not appear: $EVF_DEV"
        fi
    fi

    if [ "$DRYRUN" -eq 1 ]; then
        print_command mount -o ro "$EVF_DEV" "$EVF_MNT"
        printf '[DRYRUN] create archive atomically -> %s\n' "$OUT"
        return 0
    fi

    EVF_FS="$(blkid -p -o value -s TYPE "$EVF_DEV" 2>/dev/null || :)"
    [ -n "$EVF_FS" ] || refuse "No recognizable filesystem signature."
    case "$EVF_FS" in
        ext3|ext4) EVF_MOUNT_OPTS="ro,noload" ;;
        xfs) EVF_MOUNT_OPTS="ro,norecovery,nouuid" ;;
        *) EVF_MOUNT_OPTS="ro" ;;
    esac
    mount -o "$EVF_MOUNT_OPTS" "$EVF_DEV" "$EVF_MNT"
    EVF_MOUNTED=1
    EVF_MOUNT_SOURCE="$(findmnt -rn -M "$EVF_MNT" -o SOURCE 2>/dev/null | head -n1 || :)"
    [ -n "$EVF_MOUNT_SOURCE" ] || verification_failure "Temporary mountpoint was not created."
    [ "$(readlink -f "$EVF_MOUNT_SOURCE" 2>/dev/null || :)" = "$(readlink -f "$EVF_DEV" 2>/dev/null || :)" ] ||
        verification_failure "Temporary mount source does not match selected device."

    EVF_OUT_TMP="$(mktemp "$EVF_OUT_DIR/.${EVF_OUT_BASE}.longtail.XXXXXXXX")" ||
        die "Could not create atomic output staging file."
    case "$OUT" in
        *.tar.gz|*.tgz) tar -czf "$EVF_OUT_TMP" -C "$EVF_MNT" . ;;
        *.tar.zst)
            command -v zstd >/dev/null 2>&1 || die "zstd is required for .tar.zst"
            tar -cf - -C "$EVF_MNT" . | zstd -T0 -f -o "$EVF_OUT_TMP"
            ;;
        *) tar -cf "$EVF_OUT_TMP" -C "$EVF_MNT" . ;;
    esac
    [ -s "$EVF_OUT_TMP" ] || verification_failure "Export archive is empty."
    sha256sum "$EVF_OUT_TMP"
    if [ "$FORCE" -eq 1 ]; then
        mv -Tf "$EVF_OUT_TMP" "$OUT" ||
            verification_failure "Could not atomically replace output archive."
        EVF_OUT_TMP=""
    else
        # Staging is created in the output directory, so an atomic hard link
        # reserves the previously absent final pathname without a TOCTOU
        # overwrite window.
        ln "$EVF_OUT_TMP" "$OUT" ||
            verification_failure "Output path appeared before commit; refusing to overwrite it."
        rm -f "$EVF_OUT_TMP"
        EVF_OUT_TMP=""
    fi
    ok "Filesystem exported from stopped VM to $OUT."
}
...
usage() {
    cat <<EOF
export-vm-filesystem.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --partition) [ "$#" -ge 2 ] || usage_error "--partition requires N."; PART="$2"; shift 2 ;;
      --force) FORCE=1; shift ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; elif [ -z "$OUT" ]; then OUT="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$VMID" ] && [ -n "$SLOT" ] && [ -n "$OUT" ] || usage_error "VMID, slot and output archive are required."
}

# evf_cleanup STATUS
# Call: evf_cleanup STATUS
# Removes export-owned temporary files and restores any temporary activation state on exit.
evf_cleanup() {
    evf_cleanup_status="$1"
    trap - 0 HUP INT TERM
    if [ "${EVF_MOUNTED:-0}" -eq 1 ]; then
        umount "$EVF_MNT" >/dev/null 2>&1 || warn "Could not unmount temporary filesystem."
        EVF_MOUNTED=0
    fi
    if [ "${EVF_MAPPER_CREATED:-0}" -eq 1 ]; then
        kpartx -d "$EVF_PATH" >/dev/null 2>&1 || warn "Could not remove temporary partition mapping."
        EVF_MAPPER_CREATED=0
    fi
    [ -z "${EVF_OUT_TMP:-}" ] || rm -f "$EVF_OUT_TMP" 2>/dev/null || :
    restore_tracked_activations "${EVF_ACTIVATIONS:-}"
    rm -f "${EVF_ACTIVATIONS:-}" 2>/dev/null || :
    rm -rf "${EVF_TMP:-}" 2>/dev/null || :
    [ "$evf_cleanup_status" -eq 0 ] || exit "$evf_cleanup_status"
    return 0
}
