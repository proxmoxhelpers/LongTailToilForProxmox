...
# Unmounts filesystems created beneath a mount-vm-filesystems root.
############################################################
...
# main ARGS...
# Call: main "$@"
# Unmounts only targets beneath the exact supplied root and then removes idle
# partition mappings for disks belonging to the selected VM.
main() {
    [ "$APP_ELEVATED" = "true" ] || self_elevate "$@"
    need_commands qm lvs lvchange findmnt umount kpartx rmdir find awk sed grep sort readlink ls rm
    require_qemu_vm "$VMID"
    case "$ROOT" in /*) : ;; *) usage_error "Mount root must be an absolute path." ;; esac
    ...
    UVFS_STATE="$ROOT/.longtailtoil-mounts-${VMID}.state"
    [ -f "$UVFS_STATE" ] && [ ! -L "$UVFS_STATE" ] ||
        refuse "No regular ownership state file exists for this VM/root: $UVFS_STATE"
    UVFS_OWNER="$(ls -nd "$UVFS_STATE" 2>/dev/null | awk '{print $3}')"
    [ "$UVFS_OWNER" = "0" ] || refuse "Ownership state file must be owned by root: $UVFS_STATE"
    grep -qxF "VMID|$VMID" "$UVFS_STATE" || refuse "Ownership state VMID does not match."
    grep -qxF "ROOT|$ROOT" "$UVFS_STATE" || refuse "Ownership state mount root does not match."

    # Validate every recorded mount identity before unmounting the first one.
    UVFS_COUNT=0
    while IFS='|' read -r UVFS_KIND UVFS_SOURCE UVFS_TARGET; do
        [ "$UVFS_KIND" = "MOUNT" ] || continue
        UVFS_COUNT=$((UVFS_COUNT + 1))
        UVFS_ACTUAL="$(findmnt -rn -M "$UVFS_TARGET" -o SOURCE 2>/dev/null | head -n1 || :)"
        if [ -z "$UVFS_ACTUAL" ]; then
            warn "Recorded mount is already absent: $UVFS_TARGET"
            continue
        fi
        UVFS_ACTUAL_CANON="$(readlink -f "$UVFS_ACTUAL" 2>/dev/null || :)"
        [ "$UVFS_ACTUAL_CANON" = "$UVFS_SOURCE" ] ||
            refuse "Recorded mount source changed at $UVFS_TARGET; refusing unrelated unmount."
    done < "$UVFS_STATE"

    info "Invocation-owned mounted targets: $UVFS_COUNT"
    if [ "$PREFLIGHT" -eq 1 ]; then
        awk -F'|' '$1=="MOUNT"{print "  "$3" <- "$2}' "$UVFS_STATE"
        return 0
    fi
    if [ "$DRYRUN" -eq 1 ]; then
        awk -F'|' '$1=="MOUNT"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
        while IFS='|' read -r UVFS_KIND UVFS_SOURCE UVFS_TARGET; do print_command umount "$UVFS_TARGET"; done
        awk -F'|' '$1=="MAPPER"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
        while IFS='|' read -r UVFS_KIND UVFS_SOURCE; do print_command kpartx -d "$UVFS_SOURCE"; done
        awk -F'|' '$1=="ACTIVATE"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
        while IFS='|' read -r UVFS_KIND UVFS_VG UVFS_LV UVFS_UUID; do print_command lvchange -an "$UVFS_VG/$UVFS_LV"; done
        return 0
    fi

    awk -F'|' '$1=="MOUNT"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
    while IFS='|' read -r UVFS_KIND UVFS_SOURCE UVFS_TARGET; do
        if findmnt -rn -M "$UVFS_TARGET" >/dev/null 2>&1; then
            info "Unmounting: $UVFS_TARGET"
            umount "$UVFS_TARGET"
            ! findmnt -rn -M "$UVFS_TARGET" >/dev/null 2>&1 ||
                verification_failure "Mount remains after unmount: $UVFS_TARGET"
        fi
    done

    awk -F'|' '$1=="MAPPER"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
    while IFS='|' read -r UVFS_KIND UVFS_SOURCE; do
        # Refuse mapper removal if any partition source from this disk remains mounted.
        UVFS_BUSY=0
        for UVFS_MAP in $(kpartx -l "$UVFS_SOURCE" 2>/dev/null | awk '{print $1}'); do
            if findmnt -rn -S "/dev/mapper/$UVFS_MAP" >/dev/null 2>&1; then UVFS_BUSY=1; fi
        done
        [ "$UVFS_BUSY" -eq 0 ] || verification_failure "A recorded mapper remains mounted for $UVFS_SOURCE."
        UVFS_MAP_EXISTS=0
        for UVFS_MAP in $(kpartx -l "$UVFS_SOURCE" 2>/dev/null | awk '{print $1}'); do
            if [ -e "/dev/mapper/$UVFS_MAP" ] || [ -L "/dev/mapper/$UVFS_MAP" ]; then UVFS_MAP_EXISTS=1; fi
        done
        [ "$UVFS_MAP_EXISTS" -eq 0 ] || kpartx -d "$UVFS_SOURCE"
    done

    awk -F'|' '$1=="ACTIVATE"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
    while IFS='|' read -r UVFS_KIND UVFS_VG UVFS_LV UVFS_UUID; do
        UVFS_NOW_UUID="$(lvs --noheadings -o lv_uuid "$UVFS_VG/$UVFS_LV" 2>/dev/null | awk '{$1=$1;print;exit}')"
        [ "$UVFS_NOW_UUID" = "$UVFS_UUID" ] || verification_failure "Recorded activation UUID no longer matches $UVFS_VG/$UVFS_LV."
        UVFS_NOW_ATTR="$(lvs --noheadings -o lv_attr "$UVFS_VG/$UVFS_LV" 2>/dev/null | awk '{$1=$1;print;exit}')"
        case "$UVFS_NOW_ATTR" in
            ????a*) lvchange -an "$UVFS_VG/$UVFS_LV" || verification_failure "Could not restore inactive state for $UVFS_VG/$UVFS_LV." ;;
            *) : ;;
        esac
    done

    UVFS_ROOT_CREATED="$(awk -F'|' '$1=="ROOT_CREATED"{print $2; exit}' "$UVFS_STATE")"
    awk -F'|' '$1=="DIR"{line[NR]=$0} END{for(i=NR;i>=1;i--)if(line[i]!="")print line[i]}' "$UVFS_STATE" |
    while IFS='|' read -r UVFS_KIND UVFS_DIR; do
        [ "$UVFS_KIND" = "DIR" ] || continue
        rmdir "$UVFS_DIR" 2>/dev/null || :
    done
    rm -f "$UVFS_STATE"
    if [ "$UVFS_ROOT_CREATED" = "1" ]; then rmdir "$ROOT" 2>/dev/null || :; fi
    ok "Unmounted $UVFS_COUNT invocation-owned filesystem target(s) and removed owned mapper state."
}
...
usage() {
    cat <<EOF
unmount-all-vm-drives.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# Parses the public command line.
parse_arguments() {
    VMID=""; ROOT=""
    while [ "$#" -gt 0 ]; do
        case "$1" in
