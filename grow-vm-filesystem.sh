...
# Filesystem growth kept separate from block-device growth.
############################################################
...
main() {
    ...
    need_commands qm pvesm blkid partx findmnt lvs awk sed grep date cut
    GVF_PATH="$(gvf_resolve "$SELECTOR")" || refuse "Cannot resolve selector: $SELECTOR"
    [ -b "$GVF_PATH" ] || refuse "Selected filesystem device is not an active block device: $GVF_PATH"
    GVF_PARTS="$(partx --show --noheadings -o NR "$GVF_PATH" 2>/dev/null | awk 'NF{n++} END{print n+0}')"
    [ "$GVF_PARTS" -eq 0 ] || refuse "Partitioned disks require partition growth first; this helper intentionally handles whole-device filesystems only."
    GVF_FS="$(blkid -p -o value -s TYPE "$GVF_PATH" 2>/dev/null || :)"
    case "$GVF_FS" in
     ext2|ext3|ext4)
       need_commands resize2fs
       GVF_MOUNTED="$(findmnt -n -o TARGET --source "$GVF_PATH" 2>/dev/null || :)"
       [ -z "$GVF_MOUNTED" ] || refuse "Offline ext filesystem growth requires the device to be unmounted."
       info "Will grow $GVF_FS on $GVF_PATH with resize2fs."
       [ "$PREFLIGHT" -eq 0 ] || return 0
       journal_begin grow-vm-filesystem; journal_note device "$GVF_PATH"; journal_note filesystem "$GVF_FS"
       run_mutation resize2fs "$GVF_PATH"
       ;;
     xfs)
       need_commands xfs_growfs readlink df
       [ -n "$MOUNTPOINT" ] || usage_error "XFS growth requires --mountpoint PATH."
       case "$MOUNTPOINT" in /*) : ;; *) usage_error "XFS mountpoint must be an absolute path." ;; esac
       GVF_MOUNT_SOURCE="$(findmnt -rn -M "$MOUNTPOINT" -o SOURCE 2>/dev/null | head -n1 || :)"
       [ -n "$GVF_MOUNT_SOURCE" ] || refuse "Path is not an exact mounted filesystem mountpoint: $MOUNTPOINT"
       GVF_EXPECTED_SOURCE="$(readlink -f "$GVF_PATH" 2>/dev/null || :)"
       GVF_ACTUAL_SOURCE="$(readlink -f "$GVF_MOUNT_SOURCE" 2>/dev/null || :)"
       [ -n "$GVF_EXPECTED_SOURCE" ] && [ "$GVF_ACTUAL_SOURCE" = "$GVF_EXPECTED_SOURCE" ] ||
           refuse "XFS mountpoint $MOUNTPOINT is not backed by selected device $GVF_PATH."
       GVF_BEFORE_FS_BYTES="$(df -B1 --output=size "$MOUNTPOINT" 2>/dev/null | awk 'NR==2{print $1}')"
       info "Will grow XFS on $GVF_PATH at exact mountpoint $MOUNTPOINT with xfs_growfs."
       [ "$PREFLIGHT" -eq 0 ] || return 0
       journal_begin grow-vm-filesystem; journal_note device "$GVF_PATH"; journal_note filesystem xfs; journal_note mountpoint "$MOUNTPOINT"
       run_mutation xfs_growfs "$MOUNTPOINT"
       if [ "$DRYRUN" -eq 0 ]; then
           GVF_AFTER_SOURCE="$(findmnt -rn -M "$MOUNTPOINT" -o SOURCE 2>/dev/null | head -n1 || :)"
           [ "$(readlink -f "$GVF_AFTER_SOURCE" 2>/dev/null || :)" = "$GVF_EXPECTED_SOURCE" ] ||
               verification_failure "XFS mount source changed during growth."
           GVF_AFTER_FS_BYTES="$(df -B1 --output=size "$MOUNTPOINT" 2>/dev/null | awk 'NR==2{print $1}')"
           [ -n "$GVF_AFTER_FS_BYTES" ] || verification_failure "Could not verify XFS filesystem size after growth."
           [ "$GVF_AFTER_FS_BYTES" -ge "${GVF_BEFORE_FS_BYTES:-0}" ] || verification_failure "XFS filesystem size decreased unexpectedly."
       fi
       ;;
     *) refuse "Unsupported or undetected whole-device filesystem: ${GVF_FS:-none}. Supported: ext2/3/4 and mounted XFS." ;;
    esac
    [ "$DRYRUN" -eq 1 ] || ok "Filesystem grow command completed."
}
...
usage() {
    cat <<EOF
grow-vm-filesystem.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --mountpoint) [ "$#" -ge 2 ] || usage_error "--mountpoint requires a value."; MOUNTPOINT="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) [ -z "$SELECTOR" ] || usage_error "One selector is accepted."; SELECTOR="$1"; shift ;;
     esac
    done
    [ -n "$SELECTOR" ] || usage_error "A /dev/VG/LV or VMID:SLOT selector is required."
}

# gvf_resolve SELECTOR
# Call: gvf_resolve SELECTOR
# Resolves the selected guest filesystem/device and prints the canonical path used for growth.
# Call: gvf_resolve SELECTOR
# Resolves a direct device or VMID:SLOT and records guest context when present.
gvf_resolve() {
    gvf_resolve_selector="$1"
    GVF_GUEST_VMID=""
    case "$gvf_resolve_selector" in
        /*)
            gvf_resolve_id="$(lvm_identity "$gvf_resolve_selector" 2>/dev/null || :)"
            if [ -n "$gvf_resolve_id" ]; then
                gvf_resolve_uuid="$(printf '%s\n' "$gvf_resolve_id" | cut -d'|' -f3)"
                [ "$(lv_uuid_reference_count "$gvf_resolve_uuid")" -eq 0 ] ||
                    refuse "Direct LV selector is guest-referenced; use VMID:SLOT so stopped-guest state can be proven."
            fi
            printf '%s\n' "$gvf_resolve_selector"
            ;;
        *:*)
            gvf_resolve_vmid="${gvf_resolve_selector%%:*}"
            gvf_resolve_slot="${gvf_resolve_selector#*:}"
            case "$gvf_resolve_vmid" in *[!0-9]*|"") usage_error "VMID in selector must be numeric." ;; esac
            require_qemu_vm "$gvf_resolve_vmid"
            valid_disk_slot "$gvf_resolve_slot" ||
                usage_error "Invalid or out-of-range disk slot: $gvf_resolve_slot"
            [ "$(guest_status "$gvf_resolve_vmid" qemu)" = stopped ] ||
                refuse "Filesystem growth requires the selected VM to be stopped."
            GVF_GUEST_VMID="$gvf_resolve_vmid"
            gvf_resolve_volid="$(disk_volid "$gvf_resolve_vmid" "$gvf_resolve_slot")" || return 1
            resolve_volid_path "$gvf_resolve_volid"
            ;;
        *) return 1 ;;
    esac
}
