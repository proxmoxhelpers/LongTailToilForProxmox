...
# Compacts QEMU device slot positions without renaming storage.
############################################################
...
main() {
    ...
    need_commands qm cp awk sed grep mktemp date
    require_qemu_vm "$VMID"; [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM must be stopped."
    RDS_CFG="/etc/pve/qemu-server/${VMID}.conf"
    ! grep -qE '^\[' "$RDS_CFG" || refuse "VM snapshots are present; device-slot renumbering is refused."
    RDS_PLAN="$(mktemp "${TMPDIR:-/tmp}/lt-slot-plan.XXXXXX")"; RDS_OUT="$(mktemp "${TMPDIR:-/tmp}/lt-slot-out.XXXXXX")"
    trap 'rm -f "$RDS_PLAN" "$RDS_OUT"' 0 HUP INT TERM
    : > "$RDS_PLAN"
    for RDS_BUS in ide sata scsi virtio; do
     [ "$BUS" = all ] || [ "$BUS" = "$RDS_BUS" ] || continue
     case "$RDS_BUS" in ide) RDS_MAX=3 ;; sata) RDS_MAX=5 ;; scsi) RDS_MAX=30 ;; virtio) RDS_MAX=15 ;; esac
     RDS_TARGET=0; RDS_I=0
     while [ "$RDS_I" -le "$RDS_MAX" ]; do
       if grep -qE "^${RDS_BUS}${RDS_I}:" "$RDS_CFG"; then
         RDS_OLD="${RDS_BUS}${RDS_I}"; RDS_NEW="${RDS_BUS}${RDS_TARGET}"
         [ "$RDS_OLD" = "$RDS_NEW" ] || printf '%s|%s\n' "$RDS_OLD" "$RDS_NEW" >> "$RDS_PLAN"
         RDS_TARGET=$((RDS_TARGET + 1))
       fi
       RDS_I=$((RDS_I + 1))
     done
    done
    RDS_COUNT="$(awk 'END{print NR+0}' "$RDS_PLAN")"
    if [ "$RDS_COUNT" -eq 0 ]; then ok "Device slots are already compact."; return 0; fi
    info "Planned slot moves:"
    cat "$RDS_PLAN"
    RDS_BOOT="$(rs_remap_boot_order "$RDS_CFG" "$RDS_PLAN")"
    rs_rewrite_config "$RDS_CFG" "$RDS_PLAN" "$RDS_OUT" "$RDS_BOOT"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin renumber-vm-device-slots; journal_note vmid "$VMID"; journal_note moves "$RDS_COUNT"
    RDS_BACKUP="/root/${VMID}.conf.before-device-renumber.$(date +%Y%m%d-%H%M%S)"
    if [ "$DRYRUN" -eq 1 ]; then print_command cp "$RDS_CFG" "$RDS_BACKUP"; printf '[DRYRUN] replace config with one-pass slot remap\n'; return 0; fi
    cp "$RDS_CFG" "$RDS_BACKUP"
    RDS_TX_ACTIVE=1
    trap 'rds_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cat "$RDS_OUT" > "$RDS_CFG"
    if ! qm config "$VMID" >/dev/null 2>&1; then
        cat "$RDS_BACKUP" > "$RDS_CFG"
        RDS_TX_ACTIVE=0
        trap - 0 HUP INT TERM
    ...
    RDS_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    journal_note backup "$RDS_BACKUP"; ok "Renumbered $RDS_COUNT device slot(s); backing volumes were not renamed."
}
...
usage() {
    cat <<EOF
renumber-vm-device-slots.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --bus) [ "$#" -ge 2 ] || usage_error "--bus requires ide|sata|scsi|virtio|all."; BUS="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) [ -z "$VMID" ] || usage_error "Unexpected argument: $1"; VMID="$1"; shift ;;
     esac
    done
    [ -n "$VMID" ] || usage_error "VMID is required."
    case "$BUS" in all|ide|sata|scsi|virtio) : ;; *) usage_error "Invalid bus: $BUS" ;; esac
}
...
# rds_restore_on_exit
# Restores the complete QEMU config if a slot-renumber transaction is interrupted.
rds_restore_on_exit() {
    rds_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${RDS_TX_ACTIVE:-0}" -eq 1 ] && [ -f "${RDS_BACKUP:-}" ]; then
        cat "$RDS_BACKUP" > "$RDS_CFG" || :
        warn "Restored VM config after interrupted device-slot renumber."
    fi
    [ "$rds_restore_status" -eq 0 ] || exit "$rds_restore_status"
    return 0
}
