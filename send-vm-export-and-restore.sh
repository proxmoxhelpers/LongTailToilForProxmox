...
# Transfers one .ltvm file to root@Proxmox host and restores it there.
############################################################
...
# sver_remote_preflight EXPECT_ABSENT
# Validates remote root identity, required restore commands and destination
# directory. EXPECT_ABSENT=1 additionally refuses an existing/symlink target.
main() {
    [ "$APP_ELEVATED" = "true" ] || self_elevate "$@"
    need_commands scp ssh tar grep awk sha256sum sort uniq sed
    [ -f "$ARCHIVE" ] || refuse "Archive not found: $ARCHIVE"
    sver_validate_archive "$ARCHIVE"

    SVER_LOCAL_HASH="$(sha256sum "$ARCHIVE" | awk '{print $1}')"
    info "Archive SHA-256: $SVER_LOCAL_HASH"
    info "Destination: $DEST:$REMOTE_PATH"

    SVER_RESTORE_ARGS=""
    [ -z "$TARGET_VMID" ] || SVER_RESTORE_ARGS="$SVER_RESTORE_ARGS --vmid $TARGET_VMID"
    [ -z "$STORAGE" ] || SVER_RESTORE_ARGS="$SVER_RESTORE_ARGS --storage $STORAGE"
    [ "$START_GUEST" -eq 0 ] || SVER_RESTORE_ARGS="$SVER_RESTORE_ARGS --start"
    [ "$VERIFY_CONTENT" -eq 1 ] || SVER_RESTORE_ARGS="$SVER_RESTORE_ARGS --no-content-verify"
    [ "$QUIET" -eq 0 ] || SVER_RESTORE_ARGS="$SVER_RESTORE_ARGS --quiet"

    if [ "$DRYRUN" -eq 1 ]; then
        printf '[DRYRUN] reserve previously absent remote path %s\n' "$DEST:$REMOTE_PATH"
        print_command scp -- "$ARCHIVE" "$DEST:$REMOTE_PATH"
        printf '[DRYRUN] ssh %s verify SHA-256 %s then execute archive restore.sh%s\n' \
            "$DEST" "$SVER_LOCAL_HASH" "$SVER_RESTORE_ARGS"
        return 0
    fi

    if ! sver_remote_preflight 1; then
        refuse "Destination failed root/Proxmox/restore-command, directory-writability, or remote-path-availability preflight."
    fi
    if [ "$PREFLIGHT" -eq 1 ]; then
        ok "Remote preflight passed. No archive was transferred and no restore was started."
        return 0
    fi

    # Atomically reserve the validated target so scp cannot silently overwrite
    # a pre-existing remote file or symlink between preflight and transfer.
    if ! ssh "$DEST" "umask 077; set -C; : > '$REMOTE_PATH'"; then
        refuse "Remote archive path appeared before transfer; refusing to overwrite it: $REMOTE_PATH"
    fi

    if ! scp -- "$ARCHIVE" "$DEST:$REMOTE_PATH"; then
        warn "Archive transfer failed; reserved remote path is retained for diagnosis: $DEST:$REMOTE_PATH"
        return 4
    fi

    SVER_REMOTE_HASH="$(ssh "$DEST" "sha256sum '$REMOTE_PATH' | awk '{print \\\$1}'")" || {
        warn "Remote SHA-256 command failed; archive retained at $DEST:$REMOTE_PATH"
        return 4
    }
    if [ "$SVER_REMOTE_HASH" != "$SVER_LOCAL_HASH" ]; then
        warn "Remote archive SHA-256 does not match; refusing restore. Archive retained at $DEST:$REMOTE_PATH"
        return 4
    fi

    SVER_REMOTE_CMD="/bin/sh -s -- '$REMOTE_PATH'$SVER_RESTORE_ARGS"
    if tar -xOf "$ARCHIVE" ltvm/restore.sh | ssh "$DEST" "$SVER_REMOTE_CMD"; then
        ok "Remote restore completed and verified."
        if [ "$KEEP_REMOTE" -eq 0 ]; then
            ssh "$DEST" "rm -f -- '$REMOTE_PATH'" ||
                warn "Restore succeeded but remote archive cleanup failed: $REMOTE_PATH"
        fi
    else
        warn "Remote restore failed; archive intentionally retained at $DEST:$REMOTE_PATH for diagnosis."
        return 4
    fi
}
...
usage() {
    cat <<EOF
send-vm-export-and-restore.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
# sver_remote_preflight
# Validates the remote root/tool/storage target and optional archive-path absence before transfer or restore.
# Call: sver_remote_preflight EXPECT_ABSENT
sver_remote_preflight() {
    sver_remote_preflight_absent="$1"
    SVER_KIND="$(tar -xOf "$ARCHIVE" ltvm/manifest.tsv 2>/dev/null |
        awk -F '\t' '$1=="kind"{print $2;exit}')"
    case "$SVER_KIND" in
        qemu) SVER_REMOTE_NEEDS="qm qmrestore" ;;
        lxc) SVER_REMOTE_NEEDS="pct" ;;
        *) refuse "Archive guest kind is invalid: $SVER_KIND" ;;
    esac
    SVER_REMOTE_DIR="${REMOTE_PATH%/*}"
    [ -n "$SVER_REMOTE_DIR" ] || SVER_REMOTE_DIR="/"
    SVER_REMOTE_CHECK="test \"\$(id -u)\" -eq 0 && test -d '$SVER_REMOTE_DIR' && test -w '$SVER_REMOTE_DIR' && command -v tar >/dev/null && command -v sha256sum >/dev/null && command -v pvesm >/dev/null"
    for SVER_NEED in $SVER_REMOTE_NEEDS; do
        SVER_REMOTE_CHECK="$SVER_REMOTE_CHECK && command -v $SVER_NEED >/dev/null"
    done
    if [ "$sver_remote_preflight_absent" -eq 1 ]; then
        SVER_REMOTE_CHECK="$SVER_REMOTE_CHECK && test ! -e '$REMOTE_PATH' && test ! -L '$REMOTE_PATH'"
    fi
    # Preflight must not modify the local SSH trust database. Require an
    # already-trusted host key instead of accepting/recording a new one here.
    ssh -o StrictHostKeyChecking=yes "$DEST" "$SVER_REMOTE_CHECK"
}
...
# sver_validate_archive ARCHIVE
# Call: sver_validate_archive ARCHIVE
# Validates the outer tar namespace, member types, duplicate paths and every
# checksum-listed member before embedded restore code is trusted or executed.
sver_validate_archive() {
    sver_validate_archive_file="$1"
    SVER_LISTING="$(tar -tf "$sver_validate_archive_file" 2>/dev/null)" || refuse "Archive is not a readable tar file."
    printf '%s\n' "$SVER_LISTING" | while IFS= read -r sver_validate_archive_member; do
        case "${sver_validate_archive_member}" in /*|..|../*|*/..|*/../*) exit 8 ;; esac
        case "${sver_validate_archive_member}" in ltvm|ltvm/*) : ;; *) exit 8 ;; esac
        printf '%s\n' "${sver_validate_archive_member}" | grep -Eq '^[A-Za-z0-9._/-]+$' || exit 8
    done || refuse "Archive contains an unsafe path."
    [ -z "$(printf '%s\n' "$SVER_LISTING" | sort | uniq -d)" ] || refuse "Archive contains duplicate member paths."
    SVER_TYPES="$(tar -tvf "$sver_validate_archive_file" 2>/dev/null)" || refuse "Archive member metadata cannot be read."
    printf '%s\n' "$SVER_TYPES" | awk 'NF && substr($1,1,1)!="-" && substr($1,1,1)!="d"{bad=1} END{exit bad}' || refuse "Archive contains a symlink, hardlink, device, FIFO, or other non-regular member."
    for sver_validate_archive_required in ltvm/manifest.tsv ltvm/config.conf ltvm/storage-map.tsv ltvm/checksums.sha256 ltvm/restore.sh; do
        [ "$(printf '%s\n' "$SVER_LISTING" | grep -Fxc "$sver_validate_archive_required" || :)" -eq 1 ] || refuse "Archive must contain exactly one ${sver_validate_archive_required}."
    done
    SVER_CHECKSUMS="$(tar -xOf "$sver_validate_archive_file" ltvm/checksums.sha256 2>/dev/null)" || refuse "Archive checksum list cannot be read."
    [ -n "$SVER_CHECKSUMS" ] || refuse "Archive checksum list is empty."
    printf '%s\n' "$SVER_CHECKSUMS" | grep -Eq '^[[:xdigit:]]{64}  ltvm/[A-Za-z0-9._/-]+$' || refuse "Archive checksum list has invalid syntax."
    [ "$(printf '%s\n' "$SVER_CHECKSUMS" | awk '$2=="ltvm/restore.sh"{n++} END{print n+0}')" -eq 1 ] || refuse "Embedded restore program is not covered exactly once by archive checksums."
    printf '%s\n' "$SVER_CHECKSUMS" | while IFS= read -r sver_validate_archive_line; do
        sver_validate_archive_expected="${sver_validate_archive_line%% *}"
        sver_validate_archive_member="${sver_validate_archive_line#*  }"
        printf '%s\n' "${sver_validate_archive_expected}" | grep -Eq '^[0-9a-f]{64}$' || exit 8
        printf '%s\n' "${sver_validate_archive_member}" | grep -Eq '^ltvm/[A-Za-z0-9._/-]+$' || exit 8
        case "${sver_validate_archive_member}" in ltvm/*) : ;; *) exit 8 ;; esac
        case "${sver_validate_archive_member}" in *'/../'*|../*|*/..|..|/*) exit 8 ;; esac
        [ "$(printf '%s\n' "$SVER_LISTING" | grep -Fxc "${sver_validate_archive_member}" || :)" -eq 1 ] || exit 8
        sver_validate_archive_actual="$(tar -xOf "$sver_validate_archive_file" "${sver_validate_archive_member}" 2>/dev/null | sha256sum | awk '{print $1}')"
        [ "${sver_validate_archive_actual}" = "${sver_validate_archive_expected}" ] || exit 9
    done || refuse "Archive checksum paths or content verification failed."
    printf '%s\n' "$SVER_LISTING" | while IFS= read -r sver_validate_archive_member; do
        case "$sver_validate_archive_member" in */|ltvm/checksums.sha256) continue ;; esac
        printf '%s\n' "$SVER_CHECKSUMS" | awk -v m="$sver_validate_archive_member" '$2==m{n++} END{exit !(n==1)}' || exit 10
...
parse_arguments() {
    ARCHIVE=""; DEST=""; TARGET_VMID=""; STORAGE=""; START_GUEST=0; VERIFY_CONTENT=1; KEEP_REMOTE=0; REMOTE_PATH=""
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --vmid) [ "$#" -ge 2 ] || usage_error "--vmid requires a value."; TARGET_VMID="$2"; shift 2 ;;
      --storage) [ "$#" -ge 2 ] || usage_error "--storage requires an ID."; STORAGE="$2"; shift 2 ;;
      --start) START_GUEST=1; shift ;;
      --no-content-verify) VERIFY_CONTENT=0; shift ;;
      --keep-remote) KEEP_REMOTE=1; shift ;;
      --remote-path) [ "$#" -ge 2 ] || usage_error "--remote-path requires an absolute path."; REMOTE_PATH="$2"; shift 2 ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$ARCHIVE" ]; then ARCHIVE="$1"; elif [ -z "$DEST" ]; then DEST="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$ARCHIVE" ] && [ -n "$DEST" ] || usage_error "Archive and root@destination are required."
    single_line_value "$ARCHIVE" || usage_error "Archive path must not contain a newline."
    printf '%s\n' "$DEST" | grep -Eq '^root@[A-Za-z0-9._:-]+$' || usage_error "Destination must be root@host with no shell metacharacters."
    [ -z "$TARGET_VMID" ] || case "$TARGET_VMID" in *[!0-9]*|"") usage_error "--vmid must be numeric." ;; esac
    [ -z "$STORAGE" ] || printf '%s\n' "$STORAGE" | grep -Eq '^[A-Za-z0-9._-]+$' || usage_error "Unsafe storage ID."
    [ -n "$REMOTE_PATH" ] || REMOTE_PATH="/var/tmp/longtailtoil-restore-$$.ltvm"
    printf '%s\n' "$REMOTE_PATH" | grep -Eq '^/[A-Za-z0-9_./-]+$' || usage_error "Unsafe --remote-path."
}
