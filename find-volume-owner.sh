...
usage() {
    cat <<EOF
find-volume-owner.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
find_owners() {
    FOUND=0
    fo_refs_file="$(mktemp)" || die "Unable to create temporary reference file."
    trap 'rm -f "$fo_refs_file"' 0 HUP INT TERM
    guest_volume_references > "$fo_refs_file"
    ...
    rm -f "$fo_refs_file"; trap - 0 HUP INT TERM
}
