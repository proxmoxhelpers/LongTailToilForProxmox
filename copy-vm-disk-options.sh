...
# Copies selected disk policy options without changing backing storage.
############################################################
...
main() {
    ...
    need_commands qm awk sed grep sort date
    require_qemu_vm "$SRC_VM"; require_qemu_vm "$DST_VM"
    valid_disk_slot "$SRC_SLOT" || usage_error "Invalid or out-of-range source slot: $SRC_SLOT"
    valid_disk_slot "$DST_SLOT" || usage_error "Invalid or out-of-range destination slot: $DST_SLOT"
    [ "$(guest_status "$DST_VM" qemu)" = stopped ] || refuse "Destination VM must be stopped."
    CVDO_SRC="$(disk_value "$SRC_VM" "$SRC_SLOT")"; CVDO_DST="$(disk_value "$DST_VM" "$DST_SLOT")"
    [ -n "$CVDO_SRC" ] && [ -n "$CVDO_DST" ] || refuse "Both source and destination slots must exist."
    CVDO_NEW="$(cvdo_merge "$CVDO_SRC" "$CVDO_DST")"
    case "$DST_SLOT" in
        ide*|sata*)
            if printf '%s\n' "$CVDO_NEW" | grep -q ',iothread='; then
                warn "Not copying iothread to incompatible destination bus $DST_SLOT."
                CVDO_NEW="$(printf '%s\n' "$CVDO_NEW" | sed 's/,iothread=[^,]*//g')"
            fi
            ;;
    esac
    info "$DST_VM/$DST_SLOT old: $CVDO_DST"
    info "$DST_VM/$DST_SLOT new: $CVDO_NEW"
    [ "$CVDO_NEW" != "$CVDO_DST" ] || { ok "Destination already has the selected source options."; return 0; }
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin copy-vm-disk-options; journal_note destination "$DST_VM/$DST_SLOT"; journal_note old "$CVDO_DST"; journal_note new "$CVDO_NEW"
    if [ "$DRYRUN" -eq 1 ]; then
        run_mutation qm set "$DST_VM" "--$DST_SLOT" "$CVDO_NEW"
        return 0
    fi
    CVDO_TX_ACTIVE=1
    trap 'cvdo_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    qm set "$DST_VM" "--$DST_SLOT" "$CVDO_NEW"
    CVDO_ACTUAL="$(disk_value "$DST_VM" "$DST_SLOT")"
    if ! cvdo_values_equivalent "$CVDO_NEW" "$CVDO_ACTUAL"; then
        qm set "$DST_VM" "--$DST_SLOT" "$CVDO_DST" >/dev/null 2>&1 || :
        CVDO_TX_ACTIVE=0
        trap - 0 HUP INT TERM
        verification_failure "Destination options did not match the planned semantic value; original value was restored."
    fi
    CVDO_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    ok "Copied compatible disk options."
}
...
usage() {
    cat <<EOF
copy-vm-disk-options.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    SRC_VM=""; SRC_SLOT=""; DST_VM=""; DST_SLOT=""
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      *) if [ -z "$SRC_VM" ]; then SRC_VM="$1"; elif [ -z "$SRC_SLOT" ]; then SRC_SLOT="$1"; elif [ -z "$DST_VM" ]; then DST_VM="$1"; elif [ -z "$DST_SLOT" ]; then DST_SLOT="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
    [ -n "$SRC_VM" ] && [ -n "$SRC_SLOT" ] && [ -n "$DST_VM" ] && [ -n "$DST_SLOT" ] || usage_error "Source VM/slot and destination VM/slot are required."
}

# cvdo_merge SOURCE_VALUE DESTINATION_VALUE
# Call: cvdo_merge SOURCE_VALUE DESTINATION_VALUE
# Copies selected policy options while preserving destination volume/size/others.
cvdo_merge() {
    cvdo_merge_src="$1"; cvdo_merge_dst="$2"
    awk -v src="$cvdo_merge_src" -v dst="$cvdo_merge_dst" '
    function selected(k){return k=="cache"||k=="discard"||k=="iothread"||k=="ssd"||k=="backup"||k=="replicate"||k=="ro"||k=="readonly"}
    BEGIN{
      nd=split(dst,d,","); out=d[1]
      for(i=2;i<=nd;i++){k=d[i];sub(/=.*/,"",k);if(!selected(k))out=out","d[i]}
      ns=split(src,s,",")
      for(i=2;i<=ns;i++){k=s[i];sub(/=.*/,"",k);if(selected(k))out=out","s[i]}
      print out
    }'
}


# cvdo_canonical_value VALUE
# Call: cvdo_canonical_value VALUE
# Prints a stable representation of one Proxmox disk value. The backing volume
# remains position-sensitive while comma-separated option fields are sorted so
# Proxmox option reordering does not create a false verification failure.
cvdo_canonical_value() {
    cvdo_canonical_value_input="$1"
    cvdo_canonical_value_volume="${cvdo_canonical_value_input%%,*}"
    printf 'volume=%s\n' "$cvdo_canonical_value_volume"
    printf '%s\n' "$cvdo_canonical_value_input" |
        awk -F, '{for (i=2; i<=NF; i++) if ($i != "") print $i}' |
        LC_ALL=C sort |
        sed 's/^/option=/'
}

# cvdo_values_equivalent EXPECTED ACTUAL
# Call: cvdo_values_equivalent EXPECTED ACTUAL
# Succeeds when two disk values have the same backing volume and same option
# key/value tokens regardless of Proxmox's canonical comma-field ordering.
cvdo_values_equivalent() {
    [ "$(cvdo_canonical_value "$1")" = "$(cvdo_canonical_value "$2")" ]
}

# cvdo_restore_on_exit
# Restores the destination slot if an abnormal exit occurs after mutation.
cvdo_restore_on_exit() {
    cvdo_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${CVDO_TX_ACTIVE:-0}" -eq 1 ] && [ -n "${CVDO_DST:-}" ]; then
        qm set "$DST_VM" "--$DST_SLOT" "$CVDO_DST" >/dev/null 2>&1 || :
        warn "Restored destination disk options after interrupted update."
    fi
    [ "$cvdo_restore_status" -eq 0 ] || exit "$cvdo_restore_status"
    return 0
}
