...
############################################################
# deactivate-vm-lvs.sh
# Safely deactivates idle LVs referenced by a stopped guest.
############################################################
...
main() {
    ...
    need_commands qm pct pvesm lvs lvchange findmnt dmsetup awk sed grep sort readlink cut head find mktemp
    DVL_KIND="$(guest_kind "$VMID")" || refuse "Guest $VMID does not exist."
    [ "$(guest_status "$VMID" "$DVL_KIND")" = stopped ] || refuse "Guest $VMID must be stopped before referenced LVs can be deactivated."
    DVL_CFG="$(guest_config_path "$VMID")"
    DVL_PLAN="$(mktemp "${TMPDIR:-/var/tmp}/lt-deactivate-plan.XXXXXX")"
    DVL_DONE="$(mktemp "${TMPDIR:-/var/tmp}/lt-deactivate-done.XXXXXX")"
    : > "$DVL_PLAN"; : > "$DVL_DONE"
    trap 'dvl_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    ...
    # Preflight every distinct active LV, including mount/open checks, before
    # deactivating the first one.
    while IFS= read -r DVL_VOLID; do
        [ -n "$DVL_VOLID" ] || continue
        DVL_PATH="$(resolve_volid_path "$DVL_VOLID" || :)"; [ -n "$DVL_PATH" ] || continue
        DVL_ID="$(lvm_identity "$DVL_PATH" || :)"; [ -n "$DVL_ID" ] || continue
        DVL_VG="$(printf '%s\n' "$DVL_ID" | cut -d'|' -f1)"
        DVL_LV="$(printf '%s\n' "$DVL_ID" | cut -d'|' -f2)"
        DVL_UUID="$(printf '%s\n' "$DVL_ID" | cut -d'|' -f3)"
        DVL_ATTR="$(printf '%s\n' "$DVL_ID" | cut -d'|' -f7)"
        grep -qF "|$DVL_UUID|" "$DVL_PLAN" 2>/dev/null && continue
        case "$DVL_ATTR" in ????a*) : ;; *) continue ;; esac
        ! findmnt -rn -S "$DVL_PATH" >/dev/null 2>&1 || refuse "$DVL_PATH is mounted."
        DVL_DM="$(readlink -f "$DVL_PATH" 2>/dev/null || :)"
        if [ -n "$DVL_DM" ]; then
            DVL_OPEN="$(dmsetup info -c --noheadings -o open "$DVL_DM" 2>/dev/null | awk '{$1=$1;print}' || printf 0)"
            [ "${DVL_OPEN:-0}" -eq 0 ] || refuse "$DVL_PATH has device-mapper open count $DVL_OPEN."
        fi
        printf '%s|%s|%s\n' "$DVL_VG" "$DVL_LV" "$DVL_UUID" >> "$DVL_PLAN"
    done <<EOF
$(config_volids "$DVL_CFG" | sort -u)
EOF

    DVL_COUNT="$(awk 'END{print NR+0}' "$DVL_PLAN")"
    while IFS='|' read -r DVL_VG DVL_LV DVL_UUID; do
        [ -n "$DVL_VG" ] || continue
        info "Deactivate: $DVL_VG/$DVL_LV"
        if [ "$PREFLIGHT" -eq 1 ]; then continue; fi
        if [ "$DRYRUN" -eq 1 ]; then print_command lvchange -an "$DVL_VG/$DVL_LV"; continue; fi
        lvchange -an "$DVL_VG/$DVL_LV"
        DVL_NOW="$(lvs --noheadings -o lv_attr "$DVL_VG/$DVL_LV" 2>/dev/null | awk '{$1=$1;print;exit}')"
        case "$DVL_NOW" in ????a*) verification_failure "LV remained active: $DVL_VG/$DVL_LV" ;; *) : ;; esac
        printf '%s|%s|%s\n' "$DVL_VG" "$DVL_LV" "$DVL_UUID" >> "$DVL_DONE"
    done < "$DVL_PLAN"

    DVL_COMPLETE=1
    ok "Deactivation plan contains $DVL_COUNT active, idle referenced LV(s)."
}
...
usage() {
    cat <<EOF
deactivate-vm-lvs.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  deactivate-vm-lvs.sh <VMID> [dryrun|--preflight]

DESCRIPTION
  Deactivates referenced LVs only when the guest is stopped and each LV is
  unmounted with zero device-mapper open count. Safety refusal is exit 3.
EOF
...
# dvl_cleanup STATUS
# Re-activates only LVs deactivated by this invocation after an abnormal exit.
# Call: dvl_cleanup STATUS
dvl_cleanup() {
    dvl_cleanup_status="$1"
    trap - 0 HUP INT TERM
    if [ "${DVL_COMPLETE:-0}" -ne 1 ] && [ -s "${DVL_DONE:-}" ]; then
        warn "Deactivation did not complete; restoring LVs changed by this invocation."
        awk '{line[NR]=$0} END{for(i=NR;i>=1;i--)print line[i]}' "$DVL_DONE" |
        while IFS='|' read -r dvl_cleanup_vg dvl_cleanup_lv dvl_cleanup_uuid; do
            dvl_cleanup_now="$(lvs --noheadings -o lv_uuid "$dvl_cleanup_vg/$dvl_cleanup_lv" 2>/dev/null | awk '{$1=$1;print;exit}')"
            [ "$dvl_cleanup_now" = "$dvl_cleanup_uuid" ] || continue
            lvchange -ay -K "$dvl_cleanup_vg/$dvl_cleanup_lv" >/dev/null 2>&1 ||
                warn "Could not roll back deactivation: $dvl_cleanup_vg/$dvl_cleanup_lv"
        done
    fi
    rm -f "${DVL_PLAN:-}" "${DVL_DONE:-}" 2>/dev/null || :
    [ "$dvl_cleanup_status" -eq 0 ] || exit "$dvl_cleanup_status"
    return 0
}
