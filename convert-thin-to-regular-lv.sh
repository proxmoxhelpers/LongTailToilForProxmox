...
# Copy-and-verify conversion to an independent regular LV.
############################################################
...
setup() {
    ...
    CTR_COMPLETE=0; CTR_SOURCE_ACTIVATED_BY_US=0
...
main() {
    ...
    need_commands lvs vgs lvcreate lvremove lvchange blockdev dd cmp awk sed basename date cut head find
    ...
    CTR_ID="$(lvm_identity "$SOURCE")" || refuse "Source LV does not exist."
    CTR_SOURCE_VG="$(printf '%s\n' "$CTR_ID" | cut -d'|' -f1)"
    CTR_SOURCE_LV="$(printf '%s\n' "$CTR_ID" | cut -d'|' -f2)"
    SOURCE="/dev/$CTR_SOURCE_VG/$CTR_SOURCE_LV"
    CTR_POOL="$(printf '%s\n' "$CTR_ID" | cut -d'|' -f5)"
    [ -n "$CTR_POOL" ] || warn "Source does not appear thin; an independent regular copy can still be created."
    CTR_SIZE="$(lvs --noheadings --units b --nosuffix -o lv_size "$CTR_SOURCE_VG/$CTR_SOURCE_LV" 2>/dev/null | awk 'NF {printf "%.0f\n", $1; exit}')"
    [ -n "$CTR_SIZE" ] || refuse "Could not determine source LV size."
    ! lvs "$DEST_VG/$DEST_NAME" >/dev/null 2>&1 || refuse "Destination LV already exists."
    info "Independent regular copy: $SOURCE -> /dev/$DEST_VG/$DEST_NAME ($CTR_SIZE bytes)"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin convert-thin-to-regular-lv; journal_note source "$SOURCE"; journal_note destination "$DEST_VG/$DEST_NAME"
    trap 'ctr_cleanup "$?"' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    ctr_ensure_source_device
    run_mutation lvcreate -L "${CTR_SIZE}B" -n "$DEST_NAME" "$DEST_VG"
    if [ "$DRYRUN" -eq 0 ]; then
        CTR_DEST_UUID="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        [ -n "${CTR_DEST_UUID}" ] || die "Created destination LV but could not capture its UUID."
    fi
    if [ "$DRYRUN" -eq 1 ]; then print_command dd "if=$SOURCE" "of=/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=fsync status=progress; ctr_release_source_device; CTR_COMPLETE=1; return 0; fi
    dd if="$SOURCE" of="/dev/$DEST_VG/$DEST_NAME" bs=4M iflag=fullblock conv=fsync status=progress
    cmp -n "$CTR_SIZE" "$SOURCE" "/dev/$DEST_VG/$DEST_NAME"
    ctr_release_source_device
    CTR_COMPLETE=1
    ok "Independent regular LV created and byte-verified."
}
...
usage() {
    cat <<EOF
convert-thin-to-regular-lv.sh $SCRIPT_VERSION (project $PROJECT_VERSION)
...
parse_arguments() {
    ...
    [ -n "$DEST_NAME" ] || DEST_NAME="$(basename "$SOURCE")-regular"
}

# ctr_ensure_source_device
# Call: ctr_ensure_source_device
...
ctr_ensure_source_device() {
    ...
    if [ "$DRYRUN" -eq 1 ]; then
        dryrun_cmd lvchange -ay -K "$CTR_SOURCE_VG/$CTR_SOURCE_LV"
        CTR_SOURCE_ACTIVATED_BY_US=1
        dryrun_verify "Source LV would be active and readable for the copy"
        return 0
    fi
    run_lvm_filtered lvchange -ay -K "$CTR_SOURCE_VG/$CTR_SOURCE_LV" || die "Could not activate source LV for block copying: $CTR_SOURCE_VG/$CTR_SOURCE_LV"
    CTR_SOURCE_ACTIVATED_BY_US=1
    [ -b "$SOURCE" ] || die "Source LV activation succeeded but no block device appeared: $SOURCE"
}

# ctr_release_source_device
# Call: ctr_release_source_device
# Restores an LV activated by this helper to its original inactive state.
ctr_release_source_device() {
    [ "${CTR_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ] || return 0
    info "Restoring source LV to its original inactive state..."
    if [ "$DRYRUN" -eq 1 ]; then dryrun_cmd lvchange -an "$CTR_SOURCE_VG/$CTR_SOURCE_LV"
    else run_lvm_filtered lvchange -an "$CTR_SOURCE_VG/$CTR_SOURCE_LV" || { warn "Could not restore source LV to inactive state: $CTR_SOURCE_VG/$CTR_SOURCE_LV"; return 1; }; fi
    CTR_SOURCE_ACTIVATED_BY_US=0
}

# ctr_cleanup STATUS
# Call: ctr_cleanup STATUS
# Restores activation state and removes only conversion-owned temporary LVs on exit.
ctr_cleanup() {
    ctr_cleanup_status="$1"
    trap - 0 HUP INT TERM

    # Restore source activation state before doing anything that may fail.
    if [ "${CTR_SOURCE_ACTIVATED_BY_US:-0}" -eq 1 ]; then
        if run_lvm_filtered lvchange -an "${CTR_SOURCE_VG}/${CTR_SOURCE_LV}" >/dev/null 2>&1; then
            CTR_SOURCE_ACTIVATED_BY_US=0
        else
            warn "Source LV could not be restored to inactive state: ${CTR_SOURCE_VG}/${CTR_SOURCE_LV}"
        fi
    fi
    ...
    if [ "${CTR_COMPLETE:-0}" -ne 1 ] && [ -n "${CTR_DEST_UUID:-}" ]; then
        ctr_cleanup_uuid="$(lvs --noheadings -o lv_uuid "$DEST_VG/$DEST_NAME" 2>/dev/null | awk '{$1=$1; if(NF)print; exit}')"
        if [ -n "${ctr_cleanup_uuid}" ] && [ "${ctr_cleanup_uuid}" = "${CTR_DEST_UUID}" ]; then
        ...
        elif [ -n "${ctr_cleanup_uuid}" ]; then
            warn "Destination name now refers to a different LV UUID; preserving it."
        fi
    fi

    [ "${ctr_cleanup_status}" -eq 0 ] || exit "${ctr_cleanup_status}"
    return 0
}
