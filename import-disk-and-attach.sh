...
# Call: validate_import ARG1 [ARG2]
validate_import() {
...
attach_imported_disk() {
    ...
    aid_new="$(
        while IFS= read -r aid_line; do
            grep -Fx "$aid_line" "$BEFORE_FILE" >/dev/null 2>&1 || printf '%s\n' "$aid_line"
        done < "$AFTER_FILE" | tail -n1
    )"
    [ -n "$aid_new" ] || die "Import finished but the new unused disk could not be identified."
    UKEY="${aid_new%%:*}"
