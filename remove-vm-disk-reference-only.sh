...
############################################################
# remove-vm-disk-reference-only.sh
# Advanced recovery primitive for config-only disk-reference removal.
############################################################
...
main() {
    ...
    need_commands qm cp awk grep mktemp date
    require_qemu_vm "$VMID"
    RVR_CFG="/etc/pve/qemu-server/${VMID}.conf"
    [ "$(guest_status "$VMID" qemu)" = stopped ] || refuse "VM $VMID must be stopped."
    ! qemu_config_has_snapshots "$RVR_CFG" || refuse "VM snapshots are present; direct config editing is refused."
    if valid_unused_slot "$SLOT"; then
      :
    elif valid_disk_slot "$SLOT" || [ "$SLOT" = "efidisk0" ] || [ "$SLOT" = "tpmstate0" ]; then
      [ "$ALLOW_ACTIVE" -eq 1 ] || refuse "Active/device slot removal requires explicit --active."
    else
      usage_error "Unsupported or out-of-range slot: $SLOT"
    fi
    RVR_LINE="$(grep -E "^${SLOT}:" "$RVR_CFG" | head -n1 || :)"
    [ -n "$RVR_LINE" ] || refuse "Slot $SLOT is not present."
    info "Config-only removal: VM $VMID $SLOT"
    info "Preserved line: $RVR_LINE"
    [ "$PREFLIGHT" -eq 0 ] || return 0
    journal_begin remove-vm-disk-reference-only; journal_note vmid "$VMID"; journal_note slot "$SLOT"; journal_note original_line "$RVR_LINE"
    RVR_BACKUP="/root/${VMID}.conf.before-reference-remove.$(date +%Y%m%d-%H%M%S)"
    RVR_TMP="$(mktemp "${TMPDIR:-/tmp}/lt-ref-remove.XXXXXX")"; trap 'rm -f "$RVR_TMP"' 0 HUP INT TERM
    awk -v slot="$SLOT" '$0 !~ ("^" slot ":") {print}' "$RVR_CFG" > "$RVR_TMP"
    if [ "$DRYRUN" -eq 1 ]; then
     print_command cp "$RVR_CFG" "$RVR_BACKUP"
     printf '[DRYRUN] rewrite %s without %s; no storage API is called\n' "$RVR_CFG" "$SLOT"
     return 0
    fi
    cp "$RVR_CFG" "$RVR_BACKUP"
    RVR_TX_ACTIVE=1
    trap 'rvr_restore_on_exit' 0
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    cat "$RVR_TMP" > "$RVR_CFG"
    if ! qm config "$VMID" >/dev/null 2>&1 || grep -qE "^${SLOT}:" "$RVR_CFG"; then
     cat "$RVR_BACKUP" > "$RVR_CFG"
     RVR_TX_ACTIVE=0
     trap - 0 HUP INT TERM
    ...
    RVR_TX_ACTIVE=0
    trap - 0 HUP INT TERM
    journal_note backup "$RVR_BACKUP"
    ok "Removed config reference only. Backing storage was not freed. Backup: $RVR_BACKUP"
}
...
usage() {
    cat <<EOF
remove-vm-disk-reference-only.sh $SCRIPT_VERSION (project $PROJECT_VERSION)

USAGE
  remove-vm-disk-reference-only.sh <VMID> <unusedN> [dryrun|--preflight]
  remove-vm-disk-reference-only.sh <VMID> <active-slot> --active [dryrun|--preflight]

DESCRIPTION
  Removes exactly one QEMU config line without calling qm --delete or pvesm
  free, so the backing volume cannot be freed as a side effect. VM must be
  stopped and snapshot-free. Active-slot removal requires explicit --active.
EOF
...
parse_arguments() {
    ...
    while [ "$#" -gt 0 ]; do
     case "$1" in
      ...
      --quiet) QUIET=1; shift ;;
      --active) ALLOW_ACTIVE=1; shift ;;
      --*) usage_error "Unknown option: $1" ;;
      *) if [ -z "$VMID" ]; then VMID="$1"; elif [ -z "$SLOT" ]; then SLOT="$1"; else usage_error "Too many arguments."; fi; shift ;;
     esac
    done
...
# rvr_restore_on_exit
# Restores the original config after an interrupted direct-reference removal.
rvr_restore_on_exit() {
    rvr_restore_status=$?
    trap - 0 HUP INT TERM
    if [ "${RVR_TX_ACTIVE:-0}" -eq 1 ] && [ -n "${RVR_BACKUP:-}" ] && [ -f "$RVR_BACKUP" ]; then
        cat "$RVR_BACKUP" > "$RVR_CFG" || :
        warn "Restored VM config after interrupted reference removal."
    fi
    [ "$rvr_restore_status" -eq 0 ] || exit "$rvr_restore_status"
    return 0
}
